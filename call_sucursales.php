...
if($_POST['ajax']==1) {
		...
		if ($_POST['maneja_callcenter']!="all") $select.=" AND a.maneja_callcenter='".$_POST['maneja_callcenter']."'";
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			echo '<tr bgcolor="#E9F2F8"><th>&nbsp;</th><th>Nombre</th><th>Localidad</th><th>Sucursal</th><th>Direccion</th><th>CallCenter</th><th>Numero Lineas</th></tr>';
			while($row=mysql_fetch_array($res)) {
				rowb();
				...
				echo '<td>'.htmlentities($array_nosi[$row['maneja_callcenter']]).'</td>';
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		echo '<tr><th align="left">CallCenter</th><td><input type="checkbox" name="maneja_callcenter" id="maneja_callcenter" value="1"';
		if($row['maneja_callcenter']==1) echo ' checked';
		echo '></td></tr>';
		echo '<tr><th align="left">Registro Web</th><td><input type="checkbox" name="maneja_web" id="maneja_web" value="1"';
...
/*** PAGINA PRINCIPAL **************************************************/
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr><td>CallCenter</td><td><select name="maneja_callcenter" id="maneja_callcenter"><option value="all" selected>Todos</option>';
		...
		<Script language="javascript">
			...
			function buscarRegistros()
			{
				...
				if (objeto.readyState != 0) {
					...
					objeto.send("ajax=1&nom="+document.getElementById("nom").value+"&sucursal="+document.getElementById("sucursal").value+"&maneja_callcenter="+document.getElementById("maneja_callcenter").value+"&numeroPagina="+document.getElementById("numeroPagina").value);
