...
if($_POST['cmd']==100) {
	...
	echo '<h1>Listado de Devolucion de Certificados de la Plaza '.$array_plaza[$_POST['plazausuario']].' del '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'</h1>';
		//Listado de plazas
...
if($_POST['ajax']==2){
	$res = mysql_query("SELECT a.cve as ticket,a.fecha,a.placa,a.engomado,a.monto as monto_venta, b.cve as folioentrega, b.engomado as engomadoentrega, b.certificado, b.entregado FROM cobro_engomado a LEFT JOIN certificados b ON a.plaza = b.plaza AND a.cve = b.ticket AND b.estatus!='C' WHERE a.plaza='".$_POST['plazausuario']."' AND a.cve = '".$_POST['ticket']."' AND a.estatus!='C' AND a.factura=0 ORDER BY a.cve DESC LIMIT 1");
	if($row=mysql_fetch_array($res)){
		if($row['fecha']==fechaLocal()){
			echo '-4|';
		}
		elseif($row['entregado']==1 || $row['monto_venta']==0){
...
if ($_POST['cmd']==2) {
	...
	$res = mysql_query("SELECT cve FROM devolucion_certificado WHERE plaza='".$_POST['plazausuario']."' AND ticket='".$_POST['ticket']."' AND estatus!='C'");
	if(mysql_num_rows($res)==0){
							...
							objeto.onreadystatechange = function()
							{
								if (objeto.readyState==4)
								{
									...
									if(datos[0]=="-1"){
										...
										alert("Ya se le hizo la devolucion al ticket");
										document.forma.placa.value="";
										document.forma.engomado.value="";
										document.forma.nomengomado.value="";
										document.forma.ticket.value="";
										...
										alert("El ticket no tiene certificado entregado");
										document.forma.placa.value="";
										document.forma.engomado.value="";
										document.forma.nomengomado.value="";
										document.forma.ticket.value="";
										...
										alert("El ticket es del mismo dia cancelar la entrega y ticket y generar uno nuevo");
										document.forma.placa.value="";
										document.forma.engomado.value="";
										document.forma.nomengomado.value="";
										document.forma.ticket.value="";
