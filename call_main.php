...
if($_SESSION['CallCveUsuario']!=1 && $_POST['loginUser']!="root"){
	...
	if($Cerrado['cerrar_sistema']=='S'){
		echo '<script>window.location="call_index.php";</script>';
	}
}
...
if (isset($_POST['loginUser']) && isset($_POST['loginPassword'])) {
	...
	$redirectLoginSuccess = "call_citas.php";
	$redirectLoginFailed = "call_index.php?ErrLogUs=true";
	...
	$LoginRS_query = sprintf("SELECT * FROM call_usuarios WHERE usuario = %s AND password = %s AND estatus!='I'",
	...
	if ($loginFoundUser) {
		...
		mysql_query("INSERT call_registros_sistema SET usuario='".$Usuario['cve']."',entrada='".$fechahora."',ip='$ip'");
...
function top($SESSION,$enter=0) {
	...
	$menuRS=mysql_query("SELECT * FROM call_menu WHERE cve='".$_POST['cvemenu']."'");
	...
	<title>:: VERIFICENTROS CALLCENTER ::</title>
	...
	<script>
	if(top.window.location.href!="http://verificentros.net/callcenter/" && top.window.location.href!="http://vereficentros.hgaribay.com/callcenter/")
		top.window.location.href="http://verificentros.net/callcenter";
	...
	<table width="100%" height="50" border="0" cellpadding="0" cellspacing="0">
	  ...
	  <tr>
	   ...
	   <td background="images/bannertop-bg.gif"><span class="whiteText17">VERIFICENTROS CALLCENTER</span></td>
	...
	if($Menu['tipo']>$_POST['tipousuario']){
		echo '<script>alert("No tiene acceso al menu");document.forma.cvemenu.value=1;atcr("call_citas.php","",0,0);</script>';
	}
	...
	<td valign="top" bgcolor="#FFFFFF"><table width="100%" border="0" cellspacing="0" cellpadding="0">
	  ...
	  <tr>
		...
		<td background="images/optionHeader.gif"><div align="right">Bienvenido '.$SESSION['CallNomUsuario'].'</div></td>
		
		<td background="images/optionHeader.gif"><div align="center" id="idreloj">'.fechaLocal().' '.horaLocal().'</div></td>

		<td width="15%" background="images/optionHeader.gif" align="center" nowrap><a href="call_logout.php">Cerrar Sesion</a></td>
...
function menuppal2($SESSION) {
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="3">
		<tr><td height="20" bgcolor="#666666"><span class="style1">Menu</span></td></tr>';
		$rs=mysql_query("SELECT * FROM call_menu WHERE tipo<='".$_POST['tipousuario']."' ORDER BY orden");
