...
top($_SESSION);
	if($_POST['plazausuario']){
		...
		if($row['localidad_id']==1){
			echo '<script>atcr(\'verificentros_df/inicio.php\',\'\',\'\',\'\');</script>';
		}
		else{
		...
		if($_POST['cveusuario']==1 || $_SESSION['TipoUsuario']==1)
			$res = mysql_query("SELECT a.cve,a.numero,a.nombre FROM plazas a LEFT JOIN datosempresas b on a.cve = b.plaza WHERE a.estatus!='I' AND a.tipo_plaza != 3 ORDER BY b.localidad_id, a.numero");
		else
			$res = mysql_query("SELECT a.cve,a.numero,a.nombre FROM plazas a INNER JOIN usuario_accesos b ON a.cve=b.plaza AND b.usuario='".$_POST['cveusuario']."' AND b.acceso>0 LEFT JOIN datosempresas c on a.cve = c.plaza WHERE a.estatus!='I' AND a.tipo_plaza != 3 GROUP BY a.cve ORDER BY c.localidad_id,a.numero");
		$localidad = "";
		while($row = mysql_fetch_array($res)){
			...
			if($array_localidad_id_plaza[$row['cve']] == 1)
				echo '<li><a href="#" onClick="document.forma.plazausuario.value='.$row['cve'].';atcr(\'verificentros_df/inicio.php\',\'\',\'\',\'\');">'.$row['numero'].' '.$row['nombre'].' '.$array_localidad_plaza[$row['cve']].'</li>';
			else
		...
		while($row = mysql_fetch_array($res)){
			...
			if($array_localidad_id_plaza[$row['cve']] == 1)
				echo '<li><a href="#" onClick="document.forma.plazausuario.value='.$row['cve'].';atcr(\'verificentros_df/inicio.php\',\'\',\'\',\'\');">'.$row['numero'].' -'.$row['municipio'].'- '.$row['nombre'].' '.$array_localidad_plaza[$row['cve']].'</li>';
			else
