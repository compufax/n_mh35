...
if($_POST['cmd']==100){
	echo '<h1>'.$array_plaza[$_POST['plazausuario']].'<br>Reporte de Certificados Utilizados</h1>';
	...
	while($fecha<=$_POST['fecha_fin']){
		...
		foreach($array_engomado['normal'] as $k=>$v){
			...
			WHERE plaza='".$_POST['plazausuario']."' AND fecha='".$fecha."' AND engomado IN (".$v['cves'].") AND estatus!='C'";
			if($_POST['anio'] != 'all') $select .= " AND anio = '".$_POST['anio']."'";
			$select .= ") as a ORDER BY CAST(a.certificado as UNSIGNED)";
			...
			while($row = mysql_fetch_array($res)){
				...
				if($row['engomadoventa']==23 && $v['nombre']!='RECHAZO'){
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.send("ajax=1&anio="+document.getElementById("anio").value+"&fecha_ini="+document.getElementById("fecha_ini").value+"&fecha_fin="+document.getElementById("fecha_fin").value+"&cveusuario="+document.getElementById("cveusuario").value+"&cvemenu="+document.getElementById("cvemenu").value+"&plazausuario="+document.getElementById("plazausuario").value+"&numeroPagina="+document.getElementById("numeroPagina").value);
