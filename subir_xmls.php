	...
	if(is_uploaded_file ($_FILES['archivo']['tmp_name'])){
		if(substr($_FILES['archivo']['name'],-3)=='zip'){
			...
			if ($zip->open($_FILES['archivo']['tmp_name']) === TRUE){
				$nombre_carpeta = 'zip_'.date('Y_m_d_H_i_s');
				...
				for($i = 0; $i < $zip->numFiles; $i++)
				{
					...
					if(substr($filename,-3)=='xml'){
						...
						if($rowempresa['rfc']==$rfc_emisor){
							...
							$res = mysql_query("SELECT * FROM facturas_xml WHERE plaza='".$_POST['plazausuario']."' AND uuid='".$datos['uuid']."'");
							if(mysql_num_rows($res)==0){
								...
								foreach($conceptos as $concepto){
									...
									mysql_query("INSERT facturasmov_xml SET plaza='".$_POST['plazausuario']."',cvefact='".$cvefact."'".$campos);
								}
								copy($arch,"xmls/cfdi_".$_POST['plazausuario']."_".$cvefact.".xml");
								chmod("xmls/cfdi_".$_POST['plazausuario']."_".$cvefact.".xml", 0777);
								$arch2=substr($arch,0,(count($arch)-4)).'pdf';
								$nombres_nuevos[$arch2] = "xmls/cfdi_".$_POST['plazausuario']."_".$cvefact;
							}
						}
					}
				}
...
echo '<table>
<tr><th>Archivo</th><td><input type="file" name="archivo" id="archivo" class="textField">&nbsp;&nbsp;<a href="#" onClick="atcr(\'subir_xmls.php\',\'\',2,0);"><img src="images/guardar.gif" border="0">&nbsp;Guardar</a></td></tr>
</table>';
