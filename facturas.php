...
if($_POST['cmd']==110){
	...
	if(mysql_num_rows($rsabonos)>0) {
		...
		header("Expires: 0");
		echo '<h1>Facturas '.$Plaza['numero'].' '.$Plaza['nombre'].' del dia '.$_POST['fecha'].'</h1>';
...
if($_POST['cmd']==30){
	...
	if($Factura = mysql_fetch_array($resfactura)){
		...
		$i=0;
			...
			engomado='".$rowD['engomado']."',claveprodsat='78181505',claveunidadsat='E48'");
...
if($_POST['cmd']==2){
	...
	foreach($_POST['cant'] as $k=>$v){
		if($v>0){
			...
			engomado='".$_POST['engomado_id'][$k]."',claveprodsat='78181505',claveunidadsat='E48'");
