		...
		if($_POST['plaza']>0){
			...
			while($hora<$row['horafin'].':00'){
				...
				while($fecha<=$fecha_fin){
					...
					//if($fecha<=date("Y-m-d") || $fecha=='2015-05-05' || $fecha=='2015-04-04' || $fecha=='2015-05-01' || ($dia == 0 && $fecha!='2015-09-13') || $fecha>date( "Y-m-d" , strtotime ( "+ ".$diausuario." day" , strtotime(date("Y-m-d")) ) )){
					if($fecha<=date("Y-m-d") || intval($array_fechas_conf[$fecha])==1 || ($dia == 0 && intval($array_fechas_conf[$fecha])!=2) || $fecha>date( "Y-m-d" , strtotime ( "+ ".$diausuario." day" , strtotime(date("Y-m-d")) ) )){
	...
	if($_POST['cmd']==0){
		...
		if($_POST['plaza']>0){
			...
			for($i=$dia;$fec<=$fecultima;$i++){
				...
				//if($fec<=date("Y-m-d") || $fec=='2015-05-05' || $fec=='2015-05-01' || ($dia == 0 && $fec!='2015-09-13') || $fec>date( "Y-m-d" , strtotime ( "+ ".$diausuario." day" , strtotime(date("Y-m-d")) ) )){
				if($fec<=date("Y-m-d") || intval($array_fechas_conf[$fec])==1 || ($dia == 0 && intval($array_fechas_conf[$fec])!=2) || $fec>date( "Y-m-d" , strtotime ( "+ ".$diausuario." day" , strtotime(date("Y-m-d")) ) )){
