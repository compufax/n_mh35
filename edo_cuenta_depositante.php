...
if($_POST['ajax']==1){
	...
	if(mysql_num_rows($rsconductor)>0) {
		...
		while($row=mysql_fetch_array($res)){
			...
			$saldo_anterior = saldo_depositante($row['cve'],1,0,$_POST['fecha_ini'],"",$_POST['tipo_pago']);
			$cargo = saldo_depositante($row['cve'],2,1,$_POST['fecha_ini'],$_POST['fecha_fin'],$_POST['tipo_pago']);
			$abono = saldo_depositante($row['cve'],2,2,$_POST['fecha_ini'],$_POST['fecha_fin'],$_POST['tipo_pago']);
...
if($_POST['ajax']==2){
	...
	$saldo = saldo_depositante($row['cve'],1,0,$fecha,"", $_POST['tipo_pago']);
	echo '<td align=center>&nbsp;'.$fecha.'</td>';
	echo '<td align=left>&nbsp;Saldo Anterior</td>';
	echo '<td align="right">&nbsp;</td>';
	echo '<td align="right">&nbsp;</td>';
...
if($_POST['cmd']==1){
	...
	echo '<tr><td>Tipo de Pago</td><td><select name="tipo_pago" id="tipo_pago"><option value="0" selected>Todos</option>';
	...
	function buscar_cargos(depositante)
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.send("ajax=2&tipo_pago="+document.getElementById("tipo_pago").value+"&depositante="+depositante+"&fecha_ini="+document.getElementById("fecha_ini").value+"&fecha_fin="+document.getElementById("fecha_fin").value+"&plazausuario="+document.forma.plazausuario.value);
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","edo_cuenta_depositante.php",true);
			objeto.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
			objeto.send("ajax=1&tipo_pago="+document.getElementById("tipo_pago").value+"&nombre="+document.getElementById("nombre").value+"&fecha_ini="+document.getElementById("fecha_ini").value+"&fecha_fin="+document.getElementById("fecha_fin").value+"&numeroPagina="+document.getElementById("numeroPagina").value+"&plazausuario="+document.getElementById("plazausuario").value);
			objeto.onreadystatechange = function()
			{
				if (objeto.readyState==4)
