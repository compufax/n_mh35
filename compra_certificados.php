...
if($_POST['cmd']==20){
	...
	echo '</table>';
	echo '<h1>Entregas sin compra</h1>';
	echo '<table>';
	$t=0;
...
if($_POST['cmd'] == 10){
	...
	//	echo '<td>'.$row1['folio'].'</td>';
		...
		AND a.engomado = '".$row['engomado']."' AND CAST(a.certificado AS UNSIGNED) = '".$row1['folio']."' AND a.estatus != 'C'");
		if($row2 = mysql_fetch_array($res2)){
			...
			$res2 = mysql_query("SELECT * FROM certificados_cancelados WHERE plaza='".$_POST['plazausuario']."' AND engomado = '".$row['engomado']."' 
			AND CAST(certificado AS UNSIGNED) = '".$row1['folio']."' AND estatus != 'C'");
			if($row2 = mysql_fetch_array($res2)){
			if($_POST['fill']==2){rowb();	echo '<td>'.$row1['folio'].'</td>';
				echo '<td>Cancelacion</td>';
				echo '<td>'.$row2['cve'].'</td>';
