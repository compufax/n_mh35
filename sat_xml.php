...
if($_POST['cmd']==10){
	...
	if(count($_POST['xmls'])>0){
		...
		foreach($_POST['xmls'] as $folio){
			...
			$folios = explode("_",$folio);
			$pdf->SetFillColor(240,240,240);
			$res = mysql_query("SELECT * FROM sat_xml WHERE empresa='".$folios[0]."' AND cve='".$folios[1]."'");
			$row = mysql_fetch_array($res);
			if($row['tipo_xml'] == 1){
				$pdf->SetFont("Arial","",8);
				$pdf->SetXY(122.5,5);
				...
				$y=$pdf->GetY();
				$pdf->SetFont('Arial','',6);
				$res2 = mysql_query("SELECT * FROM satmov_xml WHERE empresa='".$folios[0]."' AND cvefact='".$folios[1]."'");
				while($row2 = mysql_fetch_array($res2)){
					$pdf->SetXY(5,$y);
				...
				if(!file_exists("../xmls/barcodesat_".$row['empresa'].'_'.$row['cve'].".png")) QRcode::png($codigo,"../xmls/barcodesat_".$row['empresa'].'_'.$row['cve'].".png","L",4,0);
				if(file_exists("../xmls/barcodesat_".$row['empresa'].'_'.$row['cve'].".png")) $pdf->Image("../xmls/barcodesat_".$row['empresa'].'_'.$row['cve'].".png",10,$y,34,34);
			}
		}
		$pdf->Output();
	}
...
if($_POST['ajax']==1){
	...
	if(mysql_num_rows($rsabonos)>0) {
		...
		while ($Abono=mysql_fetch_array($rsabonos)){	
			...
			rowb();
			$estatus='';
			echo '<td align="center"><input type="checkbox" class="chks" name="xmls[]" value="'.$Abono['empresa'].'_'.$Abono['cve'].'"></td>';
