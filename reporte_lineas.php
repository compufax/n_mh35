...
$array_lineas=array();
$res=mysql_query("SELECT * FROM cat_lineas WHERE plaza='".$_POST['plazausuario']."' ORDER BY nombre");
...
if($_POST['ajax']==1){
	$select= " SELECT a.*,b.nombre FROM reporte_lineas a WHERE 1 ";
...
if($_POST['cmd']==12){
	if($_POST['obs']!=""){
		mysql_query("INSERT reporte_lineas_obs SET obs='".$_POST['obs']."',usuario='".$_POST['cveusuario']."',fecha=NOW()");
	}
	foreach($_POST['refacciones'] as $k=>$v){
		if($v>0){
			mysql_query("INSERT reporte_lineas_refacciones SET refaccion='".$v."',usuario='".$_POST['cveusuario']."',fecha=NOW()");		
		}
	}
	foreach($_POST['cvefolios'] as $k=>$v){
		...
		elseif($_POST['folios'][$k]>0){
			mysql_query("INSERT reporte_lineas_folios SET folio='".$_POST['folios'][$k]."',estatus='".$_POST['estatusfolios'][$k]."',usuario='".$_POST['cveusuario']."',fecha=NOW()".$campos);		
		}
	}
	$_POST['cmd']=0;
}
...
if($_POST['cmd']==11){
	...
	echo '<tr><th align="left">Refacciones</th><td><table id="trefacciones"><tr id="rref" style="display:none;"><td><select name="refacciones[]"><option value="0">Seleccione</option>';
	...
	echo '<tr><th align="left">Folios</th><td><table id="tfolios"><tr><th>Folio</th><th>Estatus Folio</th></tr><tr id="rfol" style="display:none;">
	...
	echo '</table><br><input type="button" class="textField" value="Agregar Refaccion" onClick="agregarFolio()"></td></tr>';
	echo '</table>';
...
if($_POST['cmd']==2){
	mysql_query("INSERT reporte_lineas SET fecha=CURDATE(), hora=CURTIME(), plaza='".$_POST['plazausuario']."',linea='".$_POST['reg']."',
...
if($_POST['cmd']==1){
	...
	while($row=mysql_fetch_array($res)){
		...
		foreach($array_tipo) echo '<option value="'.$k.'">'.$v.'</option>';
		echo '</select></td>';
		...
		foreach($array_proveedores) echo '<option value="'.$k.'">'.$v.'</option>';
		echo '</select></td>';
		...
		foreach($array_nomenclaturas) echo '<option value="'.$k.'">'.$v.'</option>';
		echo '</select></td>';
