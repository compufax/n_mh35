      ...
      <style type="text/css">
		...
		$reporte.='<table border="0" style="font-size:13px" width="100%"><tr>';
			foreach($array_lineas as $row) {
				$porcen=number_format($row[2]*100/$total,1);
				$cve_=$row['cve'];
				$nom_=$row['nombre'];
//				$indice_=$array_lineas[0];
...
/*checar filtro*/			$filtro_engomado .= " AND a.engomado = '".$_POST['engomado']."'";
		}
//		$reporte.= '</h2>';
		...
		if($z==4){$reporte.='<tr><td colspan="3"><table width="100%" border=""><tr><td rowspan="">&nbsp;</td></tr><tr><td>&nbsp;</td></tr><tr><th align="center" style="font-size:26px">Soporte del Centro: '.$array_plazas[$_POST['plazausuario']].'</th></tr><tr><th align="right">Fecha: '.fechaLocal().'  '.horaLocal().'</th></tr><tr><td>&nbsp;</td></tr>
		 <tr><td>&nbsp;</td></tr><tr><td>&nbsp;</td></tr></table></td></tr>';}
		 if($z==2 || $z==4 || $z==6 ||$z==8 || $z==$indice_){$reporte.='<tr>';}
		$reporte.='<td><table><tr><th style="font-size:15px">'.$nom_.' '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;% '.$porcen.'</th></tr></table>
...
//		rowb();
		...
		$reporte.= '<tr gcolor="#E9F2F8"><th>Totales</th><th align="right">'.number_format($t1,0).'</th><!--<td align="right">'.number_format($t2,2).'</td>--></tr>';	
...
if($_POST['cmd']==-102) {
      ...
      <style type="text/css">
		 ...
		 </head><body><table width="100%"><tr><th align="center" style="font-size:26px">Soporte del Centro: '.$array_plazas[$_POST['plazausuario']].'</th><th align="right">Fecha: '.fechaLocal().'  '.horaLocal().'</th></tr><tr><td></td></tr></table></br>';
		//Listado de plazas
		$select= " SELECT a.cve,a.nombre, COUNT(b.cve), SUM(b.monto) FROM cat_lineas a INNER JOIN certificados b ON a.plaza = b.plaza AND a.cve = b.linea  
		INNER JOIN cobro_engomado c ON c.plaza = b.plaza AND c.cve = b.ticket WHERE a.plaza='".$_POST['plazausuario']."' ";
		if ($_POST['engomado']!="") { $select.=" AND b.engomado='".$_POST['engomado']."' "; }	
...
//		$reporte.= '</h2>';
		...
		$reporte.= '<table><tr><th style="font-size:15px">'.$nom_.' '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'</th></tr></table>
				<table width="100%" border="1" style="font-size:8px"><tr bgclor="#E9F2F8"><th>&nbsp;</th><th>Cantidad de Certificados</th><th>Importe</th></tr>';
//		rowb();
		$reporte.= '<tr><td>AFORO</td><td align="right">'.number_format($total1,0).'</td><td align="right">'.number_format($total2,2).'</td></tr>';
		$reporte.= '<tr><td colspan="3">&nbsp;</td></tr>';
//		$reporte.= '<tr><td colspan="3">&nbsp;</td></tr>';
...
//		rowb();
		$reporte.= '<tr><td>Cancelados</td><td align="right">'.number_format($row[0],0).'</td><td align="right">'.number_format(0,2).'</td></tr>';
		$reporte.= '<tr gcolor="#E9F2F8"><th>Totales</th><td align="right">'.number_format($t1,0).'</th><td align="right">'.number_format($t2,2).'</th></tr>';
		$reporte.= '</table>
';
...
//		$html.= '</h2>';
		...
		$html.= '<table width="100%" border="1" style="font-size:13px"><tr bgclor="#E9F2F8"><th>&nbsp;</th><th>Cantidad de Certificados</th><th>Importe</th></tr>';
//		rowb();
...
//		rowb();
		...
		$html.= '<tr gcolor="#E9F2F8"><th>Totales</th><td align="right">'.number_format($t1,0).'</th><td align="right">'.number_format($t2,2).'</th></tr>';
...
if($_POST['cmd']==100) {
			require_once('../dompdf/dompdf_config.inc.php');
				if ($_POST['engomado']!="") { $fil=" del Certificado ".$array_engomado[$_POST['engomado']].""; }else{$fil="";}	
		$reporte.='<html><head>
      <style type="text/css">
	                    top  lado      ladoiz
		 @page{ margin: 5in 0.5in 1px 0.5in;}
		</style>
		 </head><table width="100%"><tr><th align="center" style="font-size:26px">Plaza: '.$array_plazas[$_POST['plazausuario']].'</th></tr><tr><th align="right">Fecha: '.fechaLocal().'  '.horaLocal().'</th></tr><tr><th align="left" style="font-size:15px">Productividad de Lineas del: '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].''.$fil.'</th></tr>
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			$reporte.= '<table width="100%"><tr><td><table border="" width=""><tr><td>&nbsp;</td><td>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td></tr></table></td><td><table width="70%" border="1" style="font-size:15px">';
//			$reporte.= '<tr><td gcolor="#E9F2F8" colspan="4"></td></tr>';
			$reporte.= '<tr bgclor="#E9F2F8"><th>Linea</th><th>Aforo</th><th>%</th><!--<th>Importe</th>-->';
...
//				$reporte.= '<td align="right">'.number_format($row[3],0).'</td>';
			...
			$reporte.= '	
				<tr>
				<td bgcolr="#E9F2F8" align="right" colspan="4">&nbsp;</td>
				</tr>
				<tr>
				<td bgcolr="#E9F2F8" align="right">Total</td>
...
top($_SESSION);
	...
	echo'<input type="hidden" id="engomado" name="engomado" value="'.$_POST['engomado'].'">
		...
		$res=mysql_query("SELECT * FROM cat_lineas WHERE plaza='".$_POST['plazausuario']."' AND cve='".$_POST['reg']."'");
		$row = mysql_fetch_assoc($res);
		echo '<h2>Productividad de la  '.$row['nombre'].' del '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'];
		$filtro_engomado = "";
		if($_POST['engomado'] > 0){
