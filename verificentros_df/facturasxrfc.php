...
//	$select="SELECT * FROM clientes WHERE 1";
	$select="SELECT count(a.cve) as t_facturas,b.*,b.rfc as rfccliente FROM facturas as a LEFT JOIN clientes b ON b.plaza = a.plaza AND b.cve = a.cliente LEFT JOIN cobro_engomado as c ON a.plaza = c.plaza AND a.cve = c.factura WHERE a.tipo_serie = '0' and a.fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."'";
	//if($_POST['cveusuario'] != 1) $select .= " AND usuario>=0";
//	if($_POST['nombre']!="") $select.=" AND nombre LIKE '%".$_POST['nombre']."%'";
	if($_POST['rfc']!="") $select.=" AND b.rfc LIKE '%".$_POST['rfc']."%'";
	$select.=" GROUP BY b.rfc ORDER BY t_facturas desc";
...
//echo''.$select.'';		
	if(mysql_num_rows($res)>0) 
	{
		...
		echo '<thead><tr bgcolor="#E9F2F8"><td colspan="3">'.mysql_num_rows($res).' Registro(s)</td></tr>';
...
//					$roww1=mysql_fetch_array($res1);
		//		if($row['t_facturas']>=10){
				rowb();
				echo '<td align="left">';
				if($row['rfccliente']=="" || $row['nombre']=="" || $row['calle']=="" || $row['numexterior']=="" || $row['colonia']=="" || $row['municipio']=="" || $row['codigopostal']==""){
						echo '<font color="re">'.utf8_encode($row['nombre']).'</font></td>';
				}
				else{
					echo utf8_encode($row['nombre']).'</td>';	
				}
				echo '<td align="center">'.utf8_encode($row['rfccliente']).'</td>';
				echo '<td align="center">'.$row['t_facturas'].'</td>';
				$t_fc=($t_fc)+($row['t_facturas']);
				echo '</tr>';
		}
...
/*if($_POST['cmd']==1){
	echo '<script>
			...
			function validar_rfc_repetido(reg){
				...
				if (objeto.readyState != 0) {
					...
					objeto.open("POST","facturasxrfc.php",true);
	...
	echo '<td><a href="#" onClick="$(\'#panel\').show();atcr(\'facturasxrfc.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver</a></td><td>&nbsp;</td>';
	echo '</tr>';
	echo '</table>';
	echo '<br>';
	$class='textField';
...
if ($_POST['cmd']<1) {
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","facturasxrfc.php",true);
			objeto.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
			objeto.send("ajax=1&rfc="+document.getElementById("rfc").value+"&nombre="+document.getElementById("nombre").value+"&numeroPagina="+document.getElementById("numeroPagina").value+"&plazausuario="+document.getElementById("plazausuario").value+"&cveusuario="+document.getElementById("cveusuario").value+"&fecha_ini="+document.getElementById("fecha_ini").value+"&fecha_fin="+document.getElementById("fecha_fin").value);
			objeto.onreadystatechange = function()
			{
				if (objeto.readyState==4)
