...
header("Expires: 0");
	...
	echo '<h1 align="center" style="font-size:24px">Reporte de la Secretaria por Color</h2>';
	if($_POST['plazausuario']>0) echo '<h1 style="font-size:20px">Plaza: '.$array_plazas[$_POST['plazausuario']];
	echo '</h2>
	<h1 style="font-size:15px">Periodo; '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'</h2>';
	...
	while($row=mysql_fetch_array($res)){
		echo '<h1 style="font-size:17px">'.$row['nombre'].'</h2>';
...
if($_POST['cmd']==100){
	echo '<h2>Reporte de la Secretaria por Color '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'];
...
top($_SESSION);
	...
	if($_POST['cmd']==2){
		...
		echo '<h2>Reporte de la Secretaria por color de '.$array_engomado[$datos[1]].' Cancelados del dia '.$datos[0].'</h2>';
		$select= " SELECT * FROM certificados_cancelados WHERE plaza IN  IN (".$_POST['plaza'].") AND fecha = '".$datos[0]."' AND engomado = '".$datos[1]."' AND estatus!='C'";
	...
	if($_POST['cmd']==1){
		...
		$datos = explode("|",$_POST['reg']);
		echo '<h2>Reporte de la Secretaria por color de '.$array_engomado[$datos[1]].' tipo '.$array_color[$datos[2]].' del dia '.$datos[0].'</h2>';
		$select= " SELECT * FROM certificados WHERE plaza  IN (".$_POST['plaza'].") AND fecha = '".$datos[0]."' AND engomado = '".$datos[1]."' AND estatus!='C'";
		$select.=" ORDER BY cve DESC";
		$res=mysql_query($select);
		$totalRegistros = mysql_num_rows($res);
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr>
				...
				echo'<td><a href="#" onclick="if(document.forma.localidad.value==\'all\'){
					alert(\'Necesita seleccionar una localidad\');
				}
				else{';
				if($_POST['plazausuario']==0) echo 'document.forma.plaza.value=$(\'#plazas\').multipleSelect(\'getSelects\');';
				echo 'atcr(\'rep_secretaria_color.php\',\'_blank\',101,0);}"><img src="images/b_print.png" border="0">&nbsp;&nbsp;Excell</a>&nbsp;&nbsp;</td>';
		echo '</tr>';
		echo '</table>';
		echo '<table>';
