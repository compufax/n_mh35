...
//	echo '<h3>Total de Intentos de 250.00: '.number_format($row[0],2).'</h3>';
	...
	echo'<table width="100%" border="0">
	     <tr bgcolor="#E9F2F8">
		 <th>Fecha</th><th>Efectivo</th><th>Pagos Anticipados</th><th>Recuperacion de Creditos</th><th>Devoluciones</th><th>Vales de Descuento</th><th>Cometra</th>
		 <th>Pagos Anticipados Banco</th><th>Recuperacion Creditos Banco</th><th>Tarjeta de Credito</th><th>Tarjeta de Debito</th><th>Copias</th><th>Bancos</th><th>Creditos</th>
...
//	$sel="SELECT sum(monto_verificacion) as efectivo from cobro_engomado where estatus='A' and tipo_venta in('0','3') and tipo_pago='1' and fecha='".$fecha."' and plaza='".$_POST['plazausuario']."'";
	$sel="SELECT sum(monto) as efectivo from cobro_engomado where estatus!='C' and tipo_venta in('0','3') and tipo_pago='1' and fecha='".$fecha."' and plaza='".$_POST['plazausuario']."'";
	$rs=mysql_query($sel) or die(mysql_error());
	$row=mysql_fetch_array($rs) or die(mysql_error());
...
//	$sel3="SELECT sum(monto_verificacion) as t_credito from cobro_engomado where estatus='A'	
	$sel3="SELECT sum(monto) as t_credito from cobro_engomado where estatus='A' 
	and tipo_venta in('0','3') and tipo_pago='5' and fecha='".$fecha."' and plaza='".$_POST['plazausuario']."'";
	$rs3=mysql_query($sel3) or die(mysql_error());
	$row3=mysql_fetch_array($rs3) or die(mysql_error());
...
//	$row7=mysql_fetch_array($rs7) or die(mysql_error());
	...
	$Copias = mysql_fetch_assoc(mysql_query("SELECT SUM(copias) as copias FROM cobro_engomado WHERE fecha='{$fecha}' AND plaza='{$_POST['plazausuario']}'"));
	
	$cometra=$row['efectivo']+$row1['p_anticipado']+$row7['r_credito']-$row2['m_devoluvion']-$row9['vale_descuento'];
	$bancos=$cometra+$Copias['copias']+$row3['t_credito']+$row8['t_debito']+$row1['bancos']+$row7['bancos'];
	$t_venta=$bancos+$row5['credito']-$row7['r_credito']-$row7['bancos'];
	$t_ingreso=$t_venta+$row7['r_credito'];
	...
	echo'<td align="right">'.number_format($Copias['copias'],2).'</td>';
...
//	echo'<td align="right">'.number_format($t_ingreso,2).'</td>';
	...
	echo'<tr bgcolor="#E9F2F8">
		 ...
		 <td align="right">'.number_format($t13,2).'</td>
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","resumeningreso2.php",true);
