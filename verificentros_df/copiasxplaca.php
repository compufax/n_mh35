...
//		echo''.$select.'';
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			echo '<tr bgcolor="#E9F2F8"><th></th><th>Folio</th><th>Fecha</th><th>Fecha</br>Venta</th><th>Placa</th><th>Cantidad</th><th>Total</th><th>Usuario</th>';
			...
			while($row=mysql_fetch_array($res)) {
				...
				echo '<td align="center">'.htmlentities(utf8_encode($row['fecha'].'  '.$row['hora'])).'</td>';
//				echo '<td align="center">'.htmlentities(utf8_encode($row['hora'])).'</td>';
				$selec= " SELECT placa,fecha FROM cobro_engomado WHERE plaza='".$_POST['plazausuario']."' and cve='".$row['placa']."'";
			//$select.=" ORDER BY cve desc";
//		echo''.$select.'';
		$re=mysql_query($selec)or die(mysql_error());
		$row1=mysql_fetch_array($re);
				
				echo '<td align="center">'.htmlentities(utf8_encode($row1['fecha'])).'</td>';
...
if($_POST['ajax']==2) {
		//Listado de plazas
		$select= " SELECT cve,fecha FROM cobro_engomado WHERE plaza='".$_POST['plazausuario']."' and placa='".$_POST['cod']."'";
		$select.=" ORDER BY cve desc";
//		echo''.$select.'';
		$res=mysql_query($select)or die(mysql_error());
		$row=mysql_fetch_array($res);
