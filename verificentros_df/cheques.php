		...
		while($row=mysql_fetch_array($res)) {
			...
			if($row['estatus']=='C'){
				...
				if($_POST['cveusuario']==1){
					echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'cheques.php\',\'\',\'1\','.$row['cve'].')"><img src="images/modificar.gif" border="0" title="Cancelar '.$row['cve'].'"></a>';
				}
				if(nivelUsuario()>1)
...
if ($_POST['cmd']==2) {
	if($_POST['reg']>0){
		mysql_query("UPDATE cheques SET fecha_corte='".$_POST['fecha_corte']."',motivo='".$_POST['motivo']."',beneficiario='".$_POST['beneficiario']."',monto='".$_POST['monto']."',concepto='".$_POST['concepto']."' WHERE cve='".$_POST['reg']."'");
	}
	else{
		$intentos = 0;
		$folio = $_POST['folio'];
...
if ($_POST['cmd']==1) {
	...
	if($_POST['plazausuario']==0 && $row['plaza']==0){
		...
		if($row['cuenta']>0){
			$res1 = mysql_query("SELECT cve,cuenta,banco,folio_siguiente FROM cuentas WHERE cve='".$row['cuenta']."'");
		}
		elseif($_POST['plazausuario']>0)
