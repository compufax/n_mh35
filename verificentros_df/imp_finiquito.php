...
if($_POST['cmd']==1){
	$fecha=$_POST['fec_nom'];
	$reg=$_POST['reg'];
	/*$select= " SELECT b.*,a.nombre,if(b.rfc='',a.rfc,b.rfc) as rfc,a.homoclave,a.imss,a.puesto,a.folio as numpersonal,a.departamento,a.plaza as plazaempleado,a.metodo_pago,a.recibo_honorarios FROM personal_nomina as b 
	INNER JOIN personal as a ON (b.personal=a.cve)
	WHERE b.cve IN (".implode(",",$_POST['cvepersonal']).")";*/
	$select= " SELECT b.*,a.nombre,if(b.rfc='',a.rfc,b.rfc) as rfc,a.imss,a.puesto,a.cve as numpersonal,a.departamento,a.plaza as plazaempleado,a.metodo_pago FROM personal_nomina as b 
	INNER JOIN personal as a ON (b.personal=a.cve)
	WHERE b.cve = ".$_POST['reg'];
...
'.$Plaza['calle'].' '.$Plaza['numexterior'].', '.$Plaza['colonia'],0,'J');   
		$pdf->Cell(190,5,$Plaza['municipio'].', '.$Plaza['estado'].', C.P. '.$Plaza['codigopostal'],'B',0,'L');
		$pdf->Ln();
		//$pdf->SetFont('Arial','B',12);
		...
		$res2=mysql_query("SELECT b.cve, b.nombre, a.total FROM personal_nomina_deduccion a INNER JOIN cat_deducciones b on a.deduccion=b.cve WHERE a.nomina='".$Personal['cve']."' AND a.total>0");
		...
		$pdf->Ln(10);
		$pdf->SetFont('Arial','',10);
		$pdf->MultiCell(190,4,'Recibo el presente por concepto de mi finiquito, sin que la Empresa me adeude nada al respecto. Al recibir a mi entera satisfacción la anterior cantidad doy por terminada la relación de trabajo, manifestando que no me reservo ninguna acción o derecho de cualquier naturaleza que ejercitar con posterioridad en contra de la misma, ni de quien sus legítimos intereses represente.',0,'J');
		$pdf->MultiCell(190,4,'Así mismo, hago constar que no sufri riesgo o enfermedad laboral alguna durante el tiempo que presté mis servicios, y recibí todas las prestaciones legales y contractuales a las que tuve derecho, en virtud de estar totalmente pagado de las mismas, por lo que otorgo el finiquito mas amplio que en derecho proceda.',0,'J');
		$pdf->Ln();
		$pdf->SetFont('Arial','',10);
...
INNER JOIN personal_nomina as b ON (b.personal=a.cve AND b.periodo='".$_POST['periodo']."' AND b.tipo='1')
