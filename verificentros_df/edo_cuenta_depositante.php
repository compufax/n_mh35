...
if($_POST['ajax']==2){
	...
	$array_detalle = array();
	while($fecha<=$_POST['fecha_fin']){
		$res=mysql_query("SELECT * FROM cobro_engomado WHERE depositante='".$_POST['depositante']."' AND estatus!='C' AND tipo_pago IN (2,6) AND fecha='$fecha'");
		while($row=mysql_fetch_array($res)){
			if($row['tipo_venta']==2){
		...
		$res=mysql_query("SELECT a.* FROM recuperacion_certificado a INNER JOIN cobro_engomado b on b.plaza = a.plaza AND b.cve = a.ticket WHERE a.estatus!='C' AND a.fecha='$fecha' AND b.depositante='".$_POST['depositante']."' AND b.estatus!='C' AND b.tipo_pago IN (2,6)");
		while($row=mysql_fetch_array($res)){
			if($row['tipo_pago']==6){
		...
		$res=mysql_query("SELECT * FROM pagos_caja WHERE depositante='".$_POST['depositante']."' AND estatus!='C' AND tipo_pago IN (2,6) AND fecha='$fecha'");
		while($row=mysql_fetch_array($res)){
			if($row['tipo_pago']==6){
				$vales = '';
		...
		$res=mysql_query("SELECT * FROM ajuste_saldos WHERE depositante='".$_POST['depositante']."' AND estatus!='C' AND fecha='$fecha'");
		while($row=mysql_fetch_array($res)){
			if($row['monto']>0){
		...
		$res=mysql_query("SELECT a.* FROM devolucion_certificado a INNER JOIN cobro_engomado b on b.plaza = a.plaza AND b.cve = a.ticket WHERE a.estatus!='C' AND a.fecha='$fecha' AND b.depositante='".$_POST['depositante']."' AND b.estatus!='C' AND b.tipo_pago IN (2,6)");
		while($row=mysql_fetch_array($res)){
			if($row['tipo_pago']==6){
