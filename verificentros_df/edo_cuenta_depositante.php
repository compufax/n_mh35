	...
	$totales=array();
		for($i=0;$i<count($_POST['depositantes']);$i++){
	     ...
	     // $pdf->Ln();
			...
			$saldo_anterior = saldo_depositante($row['cve'],1,0,$_POST['fecha_ini'],"");
			$cargo = saldo_depositante($row['cve'],2,1,$_POST['fecha_ini'],$_POST['fecha_fin']);
			$abono = saldo_depositante($row['cve'],2,2,$_POST['fecha_ini'],$_POST['fecha_fin']);
	...
	while($fecha<=$_POST['fecha_fin']){
		...
		while($row=mysql_fetch_array($res)){
			if($row['tipo_pago']==6){
				...
				$cargo+=abs($row['monto']);
				$saldo-=abs($row['monto']);
				$tipo_venta =($row['tipo_venta']==2)?'Cortesia':'Venta';
				rowb();
				echo '<td align=center>&nbsp;'.$fecha.'</td>';
				echo '<td align=left>'.$tipo_venta.' de Engomado a Credito # '.$row['cve'].'&nbsp;</td>';
				echo '<td align="right">'.number_format(abs($row['monto']),2).'</td>';
				echo '<td align="right">&nbsp;</td>';
				echo '<td align="right">'.number_format($saldo,2).'</td>';
				echo '<td align="left">&nbsp;</td>';
...
if($_POST['ajax']==1){
	...
	$select= " SELECT * FROM depositantes WHERE plaza='".$_POST['plazausuario']."' AND solo_contado=0 AND edo_cuenta=1 ";
	if ($_POST['nombre']!="") { $select.=" AND nombre LIKE '%".$_POST['nombre']."%'"; }
	if ($_POST['tipo']>0) { $select.=" AND tipo = '".$_POST['tipo']."'"; }
	...
	if(mysql_num_rows($rsconductor)>0) {
		...
		echo '<tr bgcolor="#E9F2F8">';
		echo '<th><input type="checkbox" onClick="if(this.checked) $(\'.chks\').attr(\'checked\',\'checked\'); else $(\'.chks\').removeAttr(\'checked\');"></th><th>Nombre</th><th>Saldo Anterior</th><th>Cargos<th>Abonos</th><th>Saldo</th>';
		echo '</tr>';
		$i=0;
		$x=0;
		$totales=array();
		while($row=mysql_fetch_array($res)){
			...
			echo '<td align="right"><a href="#" onClick="atcr(\'edo_cuenta_depositante.php\',\'\',1,'.$row['cve'].')">'.number_format($saldo_anterior+$abono-$cargo,2).'</a></td>';
			echo '</tr>';
			$totales[0]+=$saldo_anterior;
			$totales[1]+=$cargo;
			$totales[2]+=$abono;
...
if($_POST['ajax']==2){
	...
	$array_detalle = array();
	while($fecha<=$_POST['fecha_fin']){
		$res=mysql_db_query($base,"SELECT * FROM cobro_engomado WHERE depositante='".$_POST['depositante']."' AND estatus!='C' AND tipo_pago IN (2,6) AND fecha='$fecha'");
		while($row=mysql_fetch_array($res)){
			if($row['tipo_venta']==2){
				$row['placa'] = '<font color="YELLOW">'.$row['placa'].'</font>';
			}
			if($row['tipo_pago']==6){
				

				$array_detalle[$fecha.' '.$row['hora'].'_1_'.$row['cve']]['cargo'] = abs($row['monto']);
				$array_detalle[$fecha.' '.$row['hora'].'_1_'.$row['cve']]['fecha'] = $fecha.' '.$row['hora'];
				$array_detalle[$fecha.' '.$row['hora'].'_1_'.$row['cve']]['tipo'] = 0;
				$array_detalle[$fecha.' '.$row['hora'].'_1_'.$row['cve']]['concepto'] = $row['obs'].'&nbsp;';
				...
				$array_detalle[$fecha.' '.$row['hora'].'_1_'.$row['cve']]['tipo'] = 0;
				$array_detalle[$fecha.' '.$row['hora'].'_1_'.$row['cve']]['concepto'] = $row['obs'].'&nbsp;';
				$array_detalle[$fecha.' '.$row['hora'].'_1_'.$row['cve']]['movimiento'] = $tipo_venta.' de Engomado a Credito # '.$row['cve'].'&nbsp;Placa: '.$row['placa'];
		...
		$res=mysql_db_query($base,"SELECT a.* FROM recuperacion_certificado a INNER JOIN cobro_engomado b on b.plaza = a.plaza AND b.cve = a.ticket WHERE a.estatus!='C' AND a.fecha='$fecha' AND b.depositante='".$_POST['depositante']."' AND b.estatus!='C' AND b.tipo_pago IN (2,6)");
		while($row=mysql_fetch_array($res)){
			if($row['tipo_pago']==6){
				$array_detalle[$fecha.' '.$row['hora'].'_2_'.$row['cve']]['cargo'] = abs($row['recuperacion']);
				$array_detalle[$fecha.' '.$row['hora'].'_2_'.$row['cve']]['fecha'] = $fecha.' '.$row['hora'];
				$array_detalle[$fecha.' '.$row['hora'].'_2_'.$row['cve']]['tipo'] = 0;
				$array_detalle[$fecha.' '.$row['hora'].'_2_'.$row['cve']]['concepto'] = '&nbsp;';
				$array_detalle[$fecha.' '.$row['hora'].'_2_'.$row['cve']]['movimiento'] = 'Recuperacion por Diferencia # '.$row['cve'].'&nbsp;Placa: '.$row['placa'];
		...
		$res=mysql_db_query($base,"SELECT * FROM pagos_caja WHERE depositante='".$_POST['depositante']."' AND estatus!='C' AND tipo_pago IN (2,6) AND fecha='$fecha'");
		while($row=mysql_fetch_array($res)){
			if($row['tipo_pago']==6){
				$vales = '';
				...
				$array_detalle[$fecha.' '.$row['hora'].'_3_'.$row['cve']]['abono'] = abs($row['monto']);
				...
				$array_detalle[$fecha.' '.$row['hora'].'_3_'.$row['cve']]['movimiento'] = 'Pago de Engomado Anticipado # '.$row['cve'];
				...
				$array_detalle[$fecha.' '.$row['hora'].'_3_'.$row['cve']]['fecha'] = $fecha.' '.$row['hora'];
				$array_detalle[$fecha.' '.$row['hora'].'_3_'.$row['cve']]['tipo'] = 1;
				$array_detalle[$fecha.' '.$row['hora'].'_3_'.$row['cve']]['concepto'] = '&nbsp;';
				$array_detalle[$fecha.' '.$row['hora'].'_3_'.$row['cve']]['movimiento'] = 'Pago de Engomado a Credito # '.$row['cve'];
		...
		$res=mysql_db_query($base,"SELECT * FROM ajuste_saldos WHERE depositante='".$_POST['depositante']."' AND estatus!='C' AND fecha='$fecha'");
		while($row=mysql_fetch_array($res)){
			if($row['monto']>0){
				$array_detalle[$fecha.' '.$row['hora'].'_4_'.$row['cve']]['abono'] = abs($row['monto']);
				$array_detalle[$fecha.' '.$row['hora'].'_4_'.$row['cve']]['fecha'] = $fecha.' '.$row['hora'];
				$array_detalle[$fecha.' '.$row['hora'].'_4_'.$row['cve']]['tipo'] = 1;
				$array_detalle[$fecha.' '.$row['hora'].'_4_'.$row['cve']]['concepto'] = $row['obs'].'&nbsp;';
				$array_detalle[$fecha.' '.$row['hora'].'_4_'.$row['cve']]['movimiento'] = 'Ajuste de Saldos # '.$row['cve'];
				
				/*$abono+=abs($row['monto']);
				$saldo+=abs($row['monto']);
				rowb();
				echo '<td align=center>&nbsp;'.$fecha.'</td>';
				echo '<td align=left>Ajuste de Saldos # '.$row['cve'].'&nbsp;</td>';
				echo '<td align="right">&nbsp;</td>';
				echo '<td align="right">'.number_format(abs($row['monto']),2).'</td>';
				echo '<td align="right">'.number_format($saldo,2).'</td>';
				...
				$array_detalle[$fecha.' '.$row['hora'].'_4_'.$row['cve']]['cargo'] = abs($row['monto']);
				$array_detalle[$fecha.' '.$row['hora'].'_4_'.$row['cve']]['fecha'] = $fecha.' '.$row['hora'];
				$array_detalle[$fecha.' '.$row['hora'].'_4_'.$row['cve']]['tipo'] = 0;
				$array_detalle[$fecha.' '.$row['hora'].'_4_'.$row['cve']]['concepto'] = $row['obs'].'&nbsp;';
				$array_detalle[$fecha.' '.$row['hora'].'_4_'.$row['cve']]['movimiento'] = 'Ajuste de Saldos # '.$row['cve'];
				/*$cargo+=abs($row['monto']);
				$saldo+=abs($row['monto']);
		...
		$res=mysql_db_query($base,"SELECT a.* FROM devolucion_certificado a INNER JOIN cobro_engomado b on b.plaza = a.plaza AND b.cve = a.ticket WHERE a.estatus!='C' AND a.fecha='$fecha' AND b.depositante='".$_POST['depositante']."' AND b.estatus!='C' AND b.tipo_pago IN (2,6)");
		while($row=mysql_fetch_array($res)){
			if($row['tipo_pago']==6){

				$array_detalle[$fecha.' '.$row['hora'].'_5_'.$row['cve']]['abono'] = abs($row['devolucion']);
				$array_detalle[$fecha.' '.$row['hora'].'_5_'.$row['cve']]['fecha'] = $fecha.' '.$row['hora'];
				$array_detalle[$fecha.' '.$row['hora'].'_5_'.$row['cve']]['tipo'] = 1;
				$array_detalle[$fecha.' '.$row['hora'].'_5_'.$row['cve']]['concepto'] = '&nbsp;';
				$array_detalle[$fecha.' '.$row['hora'].'_5_'.$row['cve']]['movimiento'] = 'Devolucion de dinero # '.$row['cve'].'&nbsp;Placa: '.$row['placa'];
...
if($_POST['cmd']==1){
	...
	function buscar_cargos(depositante)
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","edo_cuenta_depositante.php",true);
...
if($_POST['cmd']<1){
	/*** PAGINA PRINCIPAL **************************************************/
		...
		echo '<tr>
				...
				<td><a href="#" onclick="atcr(\'edo_cuenta_depositante.php\',\'_blank\',100,0);"><img src="images/b_print.png" border="0"></a>&nbsp;&nbsp;Imprimir</td>
				<!--<td><a href="#" onclick="atcr(\'edo_cuenta_depositante.php\',\'_blank\',102,0);"><img src="images/b_print.png" border="0"></a>&nbsp;&nbsp;Imprimir Detalle</td>-->
			 </tr>';
		echo '</table>';
		echo '<table>';
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","edo_cuenta_depositante.php",true);
			objeto.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
			objeto.send("ajax=1&tipo="+document.getElementById("tipo").value+"&nombre="+document.getElementById("nombre").value+"&fecha_ini="+document.getElementById("fecha_ini").value+"&fecha_fin="+document.getElementById("fecha_fin").value+"&numeroPagina="+document.getElementById("numeroPagina").value+"&plazausuario="+document.getElementById("plazausuario").value);
			objeto.onreadystatechange = function()
			{
				if (objeto.readyState==4)
