...
while($row=mysql_fetch_array($res)){
	...
	$array_personalclave[$row['cve']]=$row['clave'];
}
...
if($_POST['cmd']==102) {
	$filename = "Entregas.xls";
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			echo '<tr bgcolo="#E9F2F8"><th>&nbsp;</th><th>Folio</th><th>Fecha</th><th>Ticket</th><th>Tipo de Venta</th><th>Placa</th><th>Tipo de Combustible</th>
			<th>Tipo de Certificado</th><th>Tecnico 1</th><th>Tecnico 2</th><th>Holograma</th><th>Linea<!--Entregado--></th><th>Usuario</th>';
			echo '</tr>';
			$t=0;
			while($row=mysql_fetch_array($res)) {
				...
				echo '<td align="left">'.htmlentities(utf8_encode($array_personalclave[$row['tecnico']])).'</td>';
				echo '<td align="left">'.htmlentities(utf8_encode($array_personalclave[$row['tecnico2']])).'</td>';
				if($permite_editar==1 && $PlazaLocal != 1){
...
if($_POST['cmd']==101){
	...
	$pdf->Ln();
	$pdf->Cell(95,5,'Tecnico 1: '.$array_personalclave[$row['tecnico']],0,0,'L');
	$pdf->Ln();
	$pdf->Cell(95,5,'Tecnico 2: '.$array_personalclave[$row['tecnico2']],0,0,'L');
	$pdf->Ln();
...
if($_POST['cmd']==100){
	...
	class FPDF2 extends PDF_MC_Table {
		function Header(){
			...
			$this->Cell(25,4,'Tecnico 1',0,0,'C',0);
			$this->Cell(25,4,'Tecnico 2',0,0,'C',0);
	...
	$total=array();
	$i=0;
	$pdf->SetWidths(array(20,20,20,20,20,25,25,30,20));
	$pdf->SetAligns(array('C','C','C','C','C','L','L','C','C'));
	...
	while($row=mysql_fetch_array($res)){
		...
		$renglon[]=$array_personalclave[$row['tecnico']];
		$renglon[]=$array_personalclave[$row['tecnico2']];
...
if($_POST['ajax']==1) {
		...
		$permite_editar = $row['permite_editar'];
		//Listado de plazas
		$select= " SELECT a.*, b.tipo_venta, b.tipo_pago, d.nombre as nomdepositante, b.engomado as engomadoticket, b.tipo_combustible, b.factura FROM certificados a 
		INNER JOIN cobro_engomado b ON b.plaza = a.plaza AND b.cve = a.ticket
		...
		{
			echo '<table width="100%" border="0" cellpadding="4" cellspacing="1" class="">';
			echo '<tr bgcolor="#E9F2F8"><th>&nbsp;</th><th>Folio</th><th>Fecha</th><th>Ticket</th><th>Tipo de Venta</th><th>Tipo de Pago</th><th>Placa</th><!--<th>Tipo de Combustible</th>--><th>Problemas con OBDII</th>
			<th>Tipo de Certificado</th><th>Tecnico 1</th><th>Tecnico 2</th><th>Holograma</th><th>Linea<!--Entregado--></th><th>Depositante</th><th>Usuario</th>';
			echo '</tr>';
			$t=0;
			while($row=mysql_fetch_array($res)) {
				//$res1=mysql_query("SELECT engomado,factura,tipo_combustible FROM cobro_engomado WHERE plaza = '".$row['plaza']."' AND cve='".$row['ticket']."'");
	...
	if(mysql_num_rows($res)==0){
		...
		if($row=mysql_fetch_array($res)){
			...
			elseif($row['tipo_venta'] == 3){
				...
				$fecha = $row['fecha'];
				if($row['monto']==0){
					$res=mysql_query("SELECT cve,monto FROM cobro_engomado WHERE plaza='".$_POST['plazausuario']."' AND placa='".$row['placa']."' AND cve<'$folio' AND monto>0 ORDER BY cve DESC LIMIT 1");
					$row = mysql_fetch_array($res);
				}
				if($row['monto'] > 0)
				{
...
if($_POST['ajax']==9){
	...
	$res = mysql_query("SELECT cve FROM certificados_cancelados WHERE plaza='".$_POST['plazausuario']."' AND anio='".$_POST['anio']."' AND CAST(certificado AS UNSIGNED)='".intval($_POST['certificado'])."' AND estatus!='C'");
	if(mysql_num_rows($res)==0){
		$res = mysql_query("SELECT cve FROM certificados WHERE plaza='".$_POST['plazausuario']."' AND anio='".$_POST['anio']."' AND CAST(certificado AS UNSIGNED)='".intval($_POST['certificado'])."' AND estatus!='C'");
		if(mysql_num_rows($res)>0){
			$resultado['error'] = 1;
				...
				if($row=mysql_fetch_array($res)){
					...
					if(mysql_num_rows($res)>0 || $ValidarCertificados == 0){
						$insert = " INSERT certificados 
										...
										problema_obdii='".$_POST['problema_obdii']."',tecnico2='".$_POST['tecnico2']."'";
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		echo '<tr><th align="left">Problemas con OBDII</th><td><select name="problema_obdii" id="problema_obdii"><option value="all" selected>Seleccione</option>';
		foreach($array_nosi as $k=>$v){
			echo '<option value="'.$k.'">'.$v.'</option>';
		}
		echo '</select></td></tr>';
		echo '<tr><th align="left">Tecnico 1</th><td>
		...
		echo '<tr><th align="left">Tecnico 2</th><td>
		<input type="hidden" name="tecnico2" value="0" id="tecnico2">
		<input type="text" value="" id="clavetecnico2" class="textField" style="font-size:20px" size="5" onKeyUp="if(event.keyCode==13){ traeTecnico(\'2\');}">
		<input type="text" value="" id="nomtecnico2" class="readOnly" style="font-size:20px" size="50" readOnly>
		</td></tr>';
		...
		echo '<script>
				...
				function validar(){
					...
					if(document.forma.ticket.value==""){
						...
						$("#panel").hide();
						alert("Necesita seleccionar un tecnico 1");
					}
					else if(document.forma.tecnico2.value=="0"){
						$("#panel").hide();
						alert("Necesita seleccionar un tecnico 2");
					}
...
/*** PAGINA PRINCIPAL **************************************************/
	...
	if ($_POST['cmd']<1) {
		...
		echo '</select></td></tr>';
		echo '<tr><td>Problemas con OBDII</td><td><select name="problema_obdii" id="problema_obdii"><option value="all" selected>Todos</option>';
		foreach($array_nosi as $k=>$v){
			echo '<option value="'.$k.'">'.$v.'</option>';
		}
		echo '</select></td></tr>';
