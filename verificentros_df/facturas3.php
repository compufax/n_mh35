...
if($_POST['cmd']==110){
	...
	$Plaza=mysql_fetch_array($res);
	$filtro="";
	$select= " SELECT a.* FROM facturas as a WHERE a.plaza='".$_POST['plazausuario']."' AND a.tipo_serie = 2";
...
error_reporting(0);
	...
	WHERE a.plaza='".$_POST['plazausuario']."' AND a.tipo_serie = 2";
	if($_POST['folio'] != ""){
	...
	if(mysql_num_rows($rsabonos)>0) {
		...
		while ($Abono=mysql_fetch_array($rsabonos)){	
			...
			if($Abono['estatus']=='C'){
				...
				if(file_exists('../cfdi/comprobantes/facturac_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf')){
					...
					if($_POST['cveusuario']==1){
						echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'facturas3.php\',\'_blank\',\'103\',\'../cfdi/comprobantes/facturac_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf\');"><img src="images/basura.gif" border="0" title="Borrar PDF '.$Abono['folio'].'"></a>';
					}
				}
				else{
					echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'facturas3.php\',\'_blank\',\'101\',\''.$Abono['cve'].'\');"><img src="images/b_print.png" border="0" title="Imprimir '.$Abono['folio'].'"></a>';
				}
				echo '</td>';
				if($nivelUsuario>0){
			...
			elseif($Abono['respuesta1']==""){
				echo '<td align="center" width="40" nowrap>';
				if($bloqueada_sat!=1)
					echo '<input type="button" style=" font-size:25px;" value="TIMBRAR" onClick="if(confirm(\'Esta seguro que desea timbrar?\')){$(\'#panel\').show();atcr(\'facturas3.php\',\'\',\'5\',\''.$Abono['cve'].'\');}"><br>';
				//echo '<a href="#" onClick="if(confirm(\'Esta seguro que desea timbrar?\')){$(\'#panel\').show();atcr(\'facturas3.php\',\'\',\'5\',\''.$Abono['cve'].'\');}"><img src="images/validosi.gif" border="0" title="Timbrar '.$Abono['folio'].'"></a>';
				//if(nivelUsuario()>2){
				...
				if(file_exists('../cfdi/comprobantes/factura_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf')){
					...
					if($_POST['cveusuario']==1){
						echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'facturas3.php\',\'_blank\',\'103\',\'../cfdi/comprobantes/factura_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf\');"><img src="images/basura.gif" border="0" title="Borrar PDF '.$Abono['folio'].'"></a>';
					}
				}
				else{
					echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'facturas3.php\',\'_blank\',\'101\',\''.$Abono['cve'].'\');"><img src="images/b_print.png" border="0" title="Imprimir '.$Abono['folio'].'"></a>';
				...
				if($nivelUsuario>2 && $Abono['estatus'] != 'D' && $bloqueada_sat!=1){
					//if($_POST['cveusuario'] == 1)
						echo '&nbsp;&nbsp;<a href="#" onClick="if(confirm(\'Esta seguro de cancelar?\')){$(\'#panel\').show();atcr(\'facturas3.php\',\'\',\'3\',\''.$Abono['cve'].'\');}"><img src="images/validono.gif" border="0" title="Cancelar '.$Abono['folio'].'"></a>';
...
if($_POST['cmd']==2){
	...
	if($_POST['tipo_serie']==0)
		...
		$res = mysql_query("SELECT serie,folio_inicial FROM foliosiniciales WHERE plaza='".$_POST['plazausuario']."' AND tipo=0 AND tipodocumento=6");
	$row = mysql_fetch_array($res);
	...
	tipo_relacion='".$_POST['tipo_relacion']."',uuidsrelacionados='".$_POST['uuidsrelacionados']."',
	...
	foreach($_POST['cant'] as $k=>$v){
		if($v>0){
			...
			engomado='".$_POST['engomado_id'][$k]."',claveprodsat='77121503',claveunidadsat='E48'");
...
if($_POST['cmd']=='recargar'){
	...
	if($_POST['reg'] < 2 && $row[0]==1){
		echo '<script>atcr("facturas3.php","","recargar",'.($_POST['reg']+1).');</script>';
	}
	...
	if($_POST['cmd']==1){
		...
		if(nivelUsuario()>1 && $bloqueada_sat!=1){
			...
			else if(!validarConcepto())
			{
				...
				atcr(\'facturas3.php\',\'\',2,\'0\');
			}
			"><img src="images/guardar.gif" border="0">&nbsp;Guardar</a></td><td>&nbsp;</td>';
		}
		echo '<td><a href="#" onclick="$(\'#panel\').show();atcr(\'facturas3.php\',\'\',0,\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;&nbsp;Regresar</a></td>';
		echo '</tr></table>';
		echo '<br>';
		echo '<table>';
		...
		echo '</select></td></tr>';*/
		echo '<tr style="display:none;"><td>Tipo de Relacion</td><td><select name="tipo_relacion" id="tipo_relacion"><option value="">Ninguna</option>';
		foreach($array_tiporelacionsat as $k=>$v){
			echo '<option value="'.$k.'">'.$v.'</option>';
		}
		echo '</select></td></tr>';
		echo '<tr style="display:none;"><td>CFDIS Relacionados</td><td><input type="text" size="100" class="textField" name="uuidsrelacionados" id="uuidsrelacionados" value=""></td></tr>';
		echo '<tr';
		...
		$("#cliente").select2();
			...
			var ac_config = {
				source: "facturas3.php?ajax=2&plazausuario='.$_POST['plazausuario'].'&cveusuario='.$_POST['cveusuario'].'",
	...
	if ($_POST['cmd']<1) {
		...
		if($bloqueada_sat!=1)
			echo '<td><a href="#" onClick="atcr(\'facturas3.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0">&nbsp;Nuevo</a></td><td>&nbsp;</td>';
		if(nivelUsuario()>0){
			echo '<td><a href="#" onClick="mostrar_correo()"><img src="images/nuevo.gif" border="0">&nbsp;Reenviar Archivos</a></td><td>&nbsp;</td>';
			echo '<td><a href="#" onClick="atcr(\'facturas3.php\',\'_blank\',\'16\',\'0\');"><img src="../images/zip_grande.png" border="0" width="15px" height="15px" title="Descargar">&nbsp;Descargar Archivos</a></td><td>&nbsp;</td>';
			echo '<td><a href="#" onClick="atcr(\'facturas3.php\',\'_blank\',\'110\',\'0\');"><img src="images/b_print.png" border="0" width="15px" height="15px" title="Descargar">&nbsp;Descargar Excel</a></td><td>&nbsp;</td>';
		}
		echo '</tr>';
		echo '</table>';
		...
		echo '</select></td></tr>';
		echo '<tr><td align="left">RFC Emisor</td><td><input type="text" name="rfc_factura" id="rfc_factura"  size="20" class="textField" value=""></td></tr>';
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","facturas3.php",true);
			objeto.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
			objeto.send("ajax=1&rfc_factura="+document.getElementById("rfc_factura").value+"&tipo_pag="+document.getElementById("tipo_pag").value+"&nomcliente="+document.getElementById("nomcliente").value+"&tipofecha="+document.getElementById("tipofecha").value+"&mostrar="+document.getElementById("mostrar").value+"&folio="+document.getElementById("folio").value+"&estatus="+document.getElementById("estatus").value+"&cliente="+document.getElementById("cliente").value+"&fecha_ini="+document.getElementById("fecha_ini").value+"&fecha_fin="+document.getElementById("fecha_fin").value+"&usu="+document.getElementById("usu").value+"&numeroPagina="+document.getElementById("numeroPagina").value+"&plazausuario="+document.getElementById("plazausuario").value+"&cveusuario="+document.getElementById("cveusuario").value+"&cvemenu="+document.getElementById("cvemenu").value);
		...
		buttons: {
			"Aceptar": function(){ 
				if($("#conceptonotadialog").val()==""){
					...
					atcr("facturas3.php","","30",$("#facturanota").val());
				}
			},
			"Cerrar": function(){ 
				document.forma.obsnota.value="";
	...
	$("#dialog3").dialog({ 
		...
		buttons: {
			"Aceptar": function(){ 
				...
				atcr(\'facturas3.php\',\'\',\'3\',document.forma.cvecancelacion.value);
			},
			"Cerrar": function(){ 
				document.forma.cvecancelacion.value="";
				document.forma.obscancelacion.value="";
	...
	function cancelarsintimbrar(factura){
