...
if($_POST['ajax']==1) {
		//Listado de plazas
		$select= " SELECT * FROM depositantes WHERE solo_contado=1 ";
			...
			while($row=mysql_fetch_array($res)) {
				...
				if($nivelUsuario > 2){
					...
					if($row1[0] > 0){
						...
						elseif(($_POST['cveusuario']==1) or ($nivelUsuario > 2))
	...
	if($_POST['reg']) {
			...
			$insert = " INSERT INTO depositantes 
						...
						('".$_POST['nombre']."','".$_POST['plaza']."','1','".$_POST['tipo']."','".$_POST['agencia']."',
							...
							'".$_POST['bloquear_saldo_negativo']."','".$_POST['bloquear_vales_anterior']."',1,'".fechaLocal()."','".$_POST['cveusuario']."','".$_POST['pagos_cortesia']."')";
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		//Formulario 
		echo '<table>';
		echo '<tr><td class="tableEnc">Edicion Depositantes de Contado</td></tr>';
		echo '</table>';
		...
		echo '<tr style="display:none;"><th align="left">Tiene Estado de Cuenta</th><td><input type="checkbox" id="edo_cuenta" name="edo_cuenta" value="1" disabled></td></tr>';
		echo '<tr';
...
/*** PAGINA PRINCIPAL **************************************************/
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr>
				...
				<td><a href="#" onClick="atcr(\'depositantes_contado.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;Nuevo</td><td>&nbsp;</td>
			 </tr>';
		echo '</table>';
		echo '<table>';
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","depositantes_contado.php",true);
