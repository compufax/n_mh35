...
if($_POST['ajax']==1) {
		//Listado de plazas
		$select= " SELECT * FROM empresas WHERE plaza='".$_POST['plazausuario']."' ";
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			echo '<tr bgcolor="#E9F2F8"><th>Editar</th><th>Nombre</th><th>Numero de Lineas</th>';
...
if ($_POST['cmd']==2) {
	...
	if($_POST['reg']) {
			...
			$update = " UPDATE empresas 
						SET nombre='".$_POST['nombre']."'numero_lineas='".$_POST['numero_lineas']."'
						WHERE cve='".$_POST['reg']."' " ;
			$ejecutar = mysql_query($update);			
	} else {
			//Insertar el Registro
			$insert = " INSERT INTO empresas 
						(nombre,plaza,numero_lineas)
						VALUES 
						('".$_POST['nombre']."','".$_POST['plazausuario']."','".$_POST['numero_lineas']."')";
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		echo '<tr><th align="left">Numero Lineas</th><td><input type="text" name="numero_lineas" id="numero_lineas" class="textField" size="10" value="'.$row['numero_lineas'].'"></td></tr>';
		echo '</table>';
