...
if($_POST['ajax']==1) {
		//Listado de plazas
		$select= " SELECT * FROM depositantes WHERE tipo_depositante={$tipo_depositante} ";
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			echo '<tr><td bgcolor="#E9F2F8" colspan="9">'.mysql_num_rows($rsbenef).' Registro(s)</td></tr>';
			echo '<tr bgcolor="#E9F2F8"><th>Editar</th>';
			if($_POST['plazausuario']==0) echo '<th>Plaza</th>';
			echo '<th>Nombre</th><th>Email</th><th>Telefonos</th>
...
if ($_POST['cmd']==2) {
	...
	if($_POST['reg']) {
			...
			$insert = " INSERT INTO depositantes 
						(nombre,plaza,edo_cuenta,tipo,agencia,propietario,email,telefono,bloquear_saldo_negativo,bloquear_vales_anterior,solo_contado,fecha,usuario, tipo_depositante)
						VALUES 
						('".$_POST['nombre']."','".$_POST['plaza']."','1','".$_POST['tipo']."','".$_POST['agencia']."',
							'".$_POST['propietario']."','".$_POST['email']."','".$_POST['telefono']."',
							'".$_POST['bloquear_saldo_negativo']."','".$_POST['bloquear_vales_anterior']."',1,'".fechaLocal()."','".$_POST['cveusuario']."', {$tipo_depositante})";
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		echo '
			<tr>';
			if(nivelUsuario()>1)
				echo '<td><a href="#" onClick="$(\'#panel\').show();if(document.forma.plaza.value==\'0\'){$(\'#panel\').show(); alert(\'Necesita seleccionar la plaza\');}else{ atcr(\'depositantes_vales_credito.php\',\'\',\'2\',\''.$row['cve'].'\');}"><img src="images/guardar.gif" border="0">&nbsp;Guardar</a></td><td>&nbsp;</td>';
			echo '<td><a href="#" onClick="$(\'#panel\').show();atcr(\'depositantes_vales_credito.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver</a></td><td>&nbsp;</td>
			</tr>';
		echo '</table>';
		echo '<br>';
		...
		//Formulario 
		echo '<table>';
		echo '<tr><td class="tableEnc">Edicion Depositantes de Vale de Credito</td></tr>';
		echo '</table>';
...
/*** PAGINA PRINCIPAL **************************************************/
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr>
				...
				<td><a href="#" onClick="atcr(\'depositantes_vales_credito.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;Nuevo</td><td>&nbsp;</td>
			 </tr>';
		echo '</table>';
		echo '<table>';
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","depositantes_vales_credito.php",true);
