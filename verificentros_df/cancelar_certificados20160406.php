...
$res = mysql_query("SELECT * FROM anios_certificados  ORDER BY nombre DESC LIMIT 1");
while($row=mysql_fetch_array($res)){
...
//			if ($_POST['engomado']!="all") { $select.=" AND engomado='".$_POST['engomado']."' "; }
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			echo '<tr bgcolor="#E9F2F8"><th>&nbsp;</th><th>Folio</th><th>Fecha</th><th>Motivo</th><th>Tipo de Certificado</th><th>A&ntilde;o</th><th>Holograma</th><th>Placa</th><th>Observaciones</th><th>Usuario</th>';
			echo '</tr>';
			$t=0;
			while($row=mysql_fetch_array($res)) {
				rowb();
				...
				if($row['estatus']=='C'){
					...
					if(nivelUsuario()>2 && $PlazaLocal != 1 && ($row['fecha']==fechaLocal() || $_POST['cveusuario']==1))
...
if ($_POST['cmd']==2) {
	...
	if(mysql_num_rows($res)==0){
		...
		if(mysql_num_rows($res)==0){
			$res = mysql_query("SELECT b.cve FROM compra_certificados a INNER JOIN compra_certificados_detalle b ON a.plaza = b.plaza AND a.cve = b.cvecompra WHERE a.plaza='".$_POST['plazausuario']."' AND a.engomado = '".$_POST['engomado']."' AND a.anio='".$_POST['anio']."' AND b.folio='".intval($_POST['certificado'])."' AND b.estatus=0");
			if(mysql_num_rows($res)>0 || $ValidarCertificados == 0){
				$insert = " INSERT certificados_cancelados
									...
									placa='".$_POST['placa']."',obs='".$_POST['obs']."'";
				mysql_query($insert);
				mysql_query("UPDATE compra_certificados a INNER JOIN compra_certificados_detalle b ON a.plaza = b.plaza AND a.cve = b.cvecompra SET b.estatus=1 WHERE a.plaza='".$_POST['plazausuario']."' AND a.engomado = '".$_POST['engomado']."' AND a.anio='".$_POST['anio']."' AND b.folio='".intval($_POST['certificado'])."' AND b.estatus=0");
			}
			else{
