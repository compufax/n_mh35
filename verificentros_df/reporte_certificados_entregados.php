...
if($_POST['cmd']==1){
	...
	echo '<table>';
		...
		$select= " SELECT b.*,a.cve as certificado, a.certificado as holograma, a.fecha as fecha_entrega,a.engomado as engomado_entrega, IFNULL(c.cve, 0) as pago, b.depositante
			FROM certificados a 
			INNER JOIN cobro_engomado b ON b.plaza = a.plaza AND b.cve = a.cve 
			LEFT JOIN cobro_engomado c ON c.estatus != 'C' AND c.placa = b.placa AND c.tipo_venta != 1 AND CONCAT(c.fecha,' ',c.hora) < CONCAT(b.fecha,' ',b.hora) AND CONCAT(c.fecha,' ',c.hora) > DATE_ADD(CONCAT(b.fecha,' ',b.hora), INTERVAL -60 DAY) 
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			<th>Tiene Multa</th><th>Tipo de Certificado</th><th>A&ntilde;o Certificacion</th><th>Tipo de Pago</th><th>Depositante</th>
