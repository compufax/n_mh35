...
if($_POST['ajax']==1) {
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			if($_POST['cveusuario'] == 1) echo '<th>Diferencia</th>';
			echo '<th>Placa</th>
			...
			foreach($resultado as $i => $row){
				...
				if($row['estatus']=='C'){
					...
					if($row['fecha']==fechaLocal() || $_POST['cveusuario'] == 1)
						echo '<a href="#" onClick="atcr(\'cobro_engomado2.php\',\'_blank\',\'101\','.$row['cve'].')"><img src="images/b_print.png" border="0" title="Imprimir '.$row['cve'].'"></a>';
					if($_POST['cveusuario']==1 && $PlazaLocal != 1)
						echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'cobro_engomado2.php\',\'\',\'1\','.$row['cve'].');"><img src="images/modificar.gif" border="0" title="Editar '.$row['cve'].'"></a>';
					if(nivelUsuario()>2 && $PlazaLocal != 1 && $row['factura']==0 && $row['certificado']=="" && ($row['fecha']==fechaLocal() || $_POST['cveusuario']==1))
						echo '&nbsp;&nbsp;<a href="#" onClick="if(validarCancelacion('.$row['cve'].') == true && '.$_POST['cveusuario'].' != 1) alert(\'La placa tiene movimientos posteriores\'); else if(confirm(\'Esta seguro de cancelar el cobro\')) atcr(\'cobro_engomado2.php\',\'\',\'3\','.$row['cve'].')"><img src="images/validono.gif" border="0" title="Cancelar '.$row['cve'].'"></a>';
				}	
				echo '</td>';
			...
			echo '	
				<tr>
				<td colspan="'.(($_POST['cveusuario']==1)?9:1).'" bgcolor="#E9F2F8">';menunavegacion();echo '</td>
...
if($_POST['ajax']==6){
	$res = mysql_query("SELECT intentos, tipo FROM depositantes WHERE cve = '".$_POST['depositante']."'");
	if($row = mysql_fetch_assoc($res)){
		$intentos = $row['intentos'];
		if($row['tipo'] == 1)
			$intentos=$num_intentos_anticipados;
		else
			$intentos=$num_intentos_plaza;
...
if($_POST['ajax']==8){
	...
	if($intentoporcertificadodif == 1){
		$res = mysql_query("SELECT intentos, tipo FROM depositantes WHERE cve = '".$_POST['depositante']."'");
		$row = mysql_fetch_assoc($res);
		if($row['tipo'] == 1){
			$res = mysql_query("SELECT cve FROM cobro_engomado WHERE placa='".$_POST['placa']."' AND anio='".$_POST['anio']."' AND estatus!='C' AND tipo_venta in (0,2)  AND DATEDIFF(CURDATE(), fecha)<=65 ORDER BY cve DESC LIMIT 1");
			if($row=mysql_fetch_array($res)){
				$res1=mysql_query("SELECT a.* FROM cobro_engomado a LEFT JOIN certificados b ON a.plaza = b.plaza AND a.cve = b.ticket AND b.estatus!='C' WHERE a.placa='".$_POST['placa']."' AND a.anio='".$_POST['anio']."' AND a.estatus!='C' AND a.cve>='".$row['cve']."' AND IFNULL(b.engomado,0) != 19");
				if(mysql_num_rows($res1) < 2)
					echo '0';
				else 
					echo '1';
			}
			else{
				$res1=mysql_query("SELECT a.* FROM cobro_engomado a LEFT JOIN certificados b ON a.plaza = b.plaza AND a.cve = b.ticket AND b.estatus!='C' WHERE a.placa='".$_POST['placa']."' AND a.anio='".$_POST['anio']."' AND a.estatus!='C' AND IFNULL(b.engomado,0) != 19");
				if(mysql_num_rows($res1) < 2)
					echo '0';
				else 
					echo '1';
			...
			$res = mysql_query("SELECT a.cve, b.engomado, a.tipo_venta FROM cobro_engomado a LEFT JOIN certificados b ON a.plaza = b.plaza AND a.cve = b.ticket AND b.estatus!='C'
				WHERE a.placa='".$_POST['placa']."' AND a.estatus!='C' AND a.anio='".$_POST['anio']."' AND CONCAT(a.fecha,' ',a.hora)<'".$row['fecha']." ".$row['hora']."' ORDER BY a.fecha DESC, a.hora DESC LIMIT 2");
			$row = mysql_fetch_array($res);
			if($row['tipo_venta'] != 1){
				echo '0';
			}
		...
		$res = mysql_query("SELECT b.engomado FROM cobro_engomado a LEFT JOIN certificados b ON a.plaza = b.plaza AND a.cve = b.ticket AND b.estatus!='C'
			WHERE a.placa='".$_POST['placa']."' AND a.estatus!='C' AND a.anio='".$_POST['anio']."' AND CONCAT(a.fecha,' ',a.hora)<'".$row['fecha']." ".$row['hora']."' ORDER BY a.fecha DESC, a.hora DESC LIMIT 1");
		$row = mysql_fetch_array($res);
		if($row['engomado'] != 19)
			echo '1';
		else 
...
/*** EDICION  **************************************************/
	...
	if($_POST['cmd'] == 11){
		...
		echo '
			...
			echo '<td><a href="#" onClick="$(\'#panel\').show();atcr(\'cobro_engomado2.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver</a></td><td>&nbsp;</td>
			</tr>';
		echo '</table>';
		echo '<br>';
		...
		echo '<script>
				function validar(reg){
					if(document.forma.fecha_vigencia.value==""){
						...
						atcr("cobro_engomado2.php","",11.2,reg);
					}
				}
			</script>';
	}
	...
	if ($_POST['cmd']==1) {
		...
		if($_POST['reg']==0){
			...
			echo '<tr';
			if($array_tipo_venta[$row['tipo_venta']]['maneja_autoriza']!=1 || $_POST['reg']==0){
		...
		echo '<script>
				...
				function validar(reg){
					...
					else if(validarAutoriza() == false)
					{
						...
						atcr("cobro_engomado2.php","",2,reg);
					}
				}
				...
				function validarAutoriza(){
					if($("#tipo_venta").find("option:selected").attr("maneja_autoriza") == "1" && document.forma.plazausuario.value!=59){
						if($.trim(document.forma.autoriza.value)=="" && document.forma.tipo_pago.value!="6"){
							alert("Necesita ingresar la persona que autoriza la venta");
							return false;
						}
						regresar = false;
						$.ajax({
...
if ($_POST['cmd']<1) {
	...
	if($PlazaLocal!=1)
		echo '<td><a href="#" onClick="atcr(\'cobro_engomado2.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;Nuevo</td><td>&nbsp;</td>';
	if($nivelUsuario>1 && $VendeSeguros==1)
		echo '<td><a href="#" onClick="atcr(\'cobro_engomado2.php\',\'\',\'11\',\'0\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;Nuevo Seguro</td><td>&nbsp;</td>';
	echo '<td><a href="#" onClick="atcr(\'cobro_engomado2.php\',\'_blank\',\'102\',\'0\');"><img src="images/b_print.png" border="0"></a>&nbsp;Corte</td><td>&nbsp;</td>
			<td><a href="#" onClick="atcr(\'cobro_engomado2.php\',\'_blank\',\'102.1\',\'0\');"><img src="images/b_print.png" border="0"></a>&nbsp;Corte Detallado</td><td>&nbsp;</td>
			<td><a href="#" onClick="atcr(\'cobro_engomado2.php\',\'_blank\',\'103\',\'0\');"><img src="images/b_print.png" border="0"></a>&nbsp;Corte HTML</td><td>&nbsp;</td>';
	if($nivelUsuario>3){
...
<Script language="javascript">
	...
	function buscarRegistros(btn)
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","cobro_engomado2.php",true);
			objeto.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
			objeto.send("ajax=1&btn="+btn+"&folio="+document.getElementById("folio").value+"&tipo_venta="+document.getElementById("tipo_venta").value+"&anio="+document.getElementById("anio").value+"&multa="+document.getElementById("multa").value+"&tipo_pago="+document.getElementById("tipo_pago").value+"&tipo_combustible="+document.getElementById("tipo_combustible").value+"&estatus="+document.getElementById("estatus").value+"&mostrar="+document.getElementById("mostrar").value+"&fecha_ini="+document.getElementById("fecha_ini").value+"&fecha_fin="+document.getElementById("fecha_fin").value+"&placa="+document.getElementById("placa").value+"&engomado="+document.getElementById("engomado").value+"&usuario="+document.getElementById("usuario").value+"&cveusuario="+document.getElementById("cveusuario").value+"&cvemenu="+document.getElementById("cvemenu").value+"&plazausuario="+document.getElementById("plazausuario").value+"&numeroPagina="+document.getElementById("numeroPagina").value);
...
if($cvecobro>0){
		echo '<script>atcr(\'cobro_engomado2.php\',\'_blank\',\'101\','.$cvecobro.');</script>';
	}
?>
