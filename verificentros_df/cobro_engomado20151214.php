...
if($_POST['ajax']==1) {
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			<th>Tiene Multa</th><th>Tipo de Certificado</th><th>Tipo Venta</th><th>Monto</th><th>A&ntilde;o Certificacion</th><th>Tipo de Pago</th><th>Tipo Combustible</th><!--<th>Documento</th>--><th>Factura</th><th>Entrega Certificado</th><th>Holograma Entregado</th><th>Usuario</th><th>Motivo Cancelacion</th>';
			...
			echo '	
				...
				<td colspan="10" bgcolor="#E9F2F8">&nbsp;</td>
				</tr>
			</table>';
...
if($_POST['ajax']==5){
	$res = mysql_query("SELECT monto FROM cobro_engomado WHERE placa='".$_POST['placa']."' AND estatus!='C' AND tipo_venta=0  AND DATEDIFF(CURDATE(), fecha)<=60 ORDER BY cve DESC LIMIT 1");
	if($row=mysql_fetch_array($res)){
		echo '1';
	}
	else{
...
if($_POST['cmd']==103){
	...
	FROM cobro_engomado a WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND estatus!='C' ".$filtro." AND tipo_venta=3 GROUP BY engomado");
	while($row=mysql_fetch_array($res)){
		echo '<h2>REPOSICION '.$array_engomado[$row['engomado']].'</h2>';
		echo '<table><tr><th>Tipo Pago</th><th>Cantidad</th><th>Importe</th></tr>';
		$res1=mysql_query("SELECT tipo_pago,SUM(monto),COUNT(cve)  FROM cobro_engomado a WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND engomado='".$row['engomado']."' AND estatus!='C' ".$filtro." AND tipo_venta=3 GROUP BY tipo_pago");
		while($row1=mysql_fetch_array($res1)){
			echo '<tr><td>'.$array_tipo_pago[$row1['tipo_pago']].'</td><td>'.$row1[2].'</td><td>'.number_format($row1[1],2).'</td></tr>';
		}
...
if($_POST['cmd']==101){
	...
	if($row1=mysql_fetch_array($res1)){
		...
		$texto.='|';
		$texto.=chr(27).'!'.chr(8)."     PAGINA PARA FACTURAR:|    www.facturacionweb.net|";
	}
	if($row['monto']==0){
		...
		if($_POST['reg']>0){
			...
			if($row['placa'] != $_POST['placa'] || $row['engomado'] != $_POST['engomado'] || $_POST['anio'] != $row['anio']){
				...
				$insert = " INSERT cobro_engomado 
								...
								tipo_venta = '".$_POST['tipo_venta']."',monto_verificacion='".$_POST['monto_verificacion']."'";
				mysql_query($insert);
	...
	if ($_POST['cmd']==1) {
		...
		if($_POST['reg']==0){
			...
			foreach($array_tipo_venta as $k=>$v){
				echo '<option value="'.$k.'" costo="'.$v['costo'].'" maneja_motivo="'.$v['maneja_motivo'].'"';
		...
		echo '<script>
				...
				function validar(reg){
					...
					/*else if(document.forma.placa2.value==""){
						...
						$("#panel").hide();
						alert("La placa debe de tener al menos una venta 60 dias atras");	
					}
					else{
