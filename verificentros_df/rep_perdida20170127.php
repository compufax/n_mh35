...
//	if($_POST['fecha_fin'] != '') $filtro_fechas .= " AND a.fecha <= '{$_POST['fecha_fin']}'";
	...
	$entregas=array();
	$res=mysql_query("SELECT TRIM(b.placa),b.tipo_pago,b.depositante,SUM(b.monto) as ingresos".$detallengomado.",
		...
		GROUP BY TRIM(a.placa)".$filtro_movimientos." ORDER BY TRIM(placa)") or die(mysql_error());
	...
	$x=0;
	foreach($entregas as $placa=>$row){
		rowb();
		echo '<td align="center"><a href="#" onClick="atcr(\'rep_perdida.php\',\'\',1,\''.$placa.'\');">'.$placa.'</a></td>';
