...
$array_tipo=array("Tecnico","Administrador");
...
if($_POST['ajax']==1) {
		...
		if($_SESSION['CveUsuario']>1){
			$select= " SELECT * FROM mantenimiento_usuarios WHERE estatus!='I' AND cve>'1'";
		}
		else{
			$select= " SELECT * FROM mantenimiento_usuarios WHERE estatus!='I'";
		}
...
if($_POST['ajax']==2){
	$res=mysql_query("SELECT * FROM mantenimiento_usuarios WHERE usuario='".$_POST['usuario']."' AND estatus!='I' AND cve!='".$_POST['cveusu']."'");
...
if ($_POST['cmd']==3) {
	$delete= "UPDATE mantenimiento_usuarios SET estatus='I' WHERE cve='".$_POST['reg']."' ";
	$ejecutar=mysql_query($delete);
	mysql_query("INSERT mantenimiento_historial SET menu='".$_POST['cvemenu']."',cveaux='".$_POST['reg']."',fecha='".fechaLocal()." ".horaLocal()."',
...
if ($_POST['cmd']==2) {
	if($_POST['reg']) {
		$select=" SELECT * FROM mantenimiento_usuarios WHERE cve='".$_POST['reg']."' ";
		...
		if($Usuario['proveedor']!=intval($_POST['proveedor'])){
			mysql_query("INSERT mantenimiento_historial SET menu='".$_POST['cvemenu']."',cveaux='".$_POST['reg']."',fecha='".fechaLocal()." ".horaLocal()."',
			dato='Tipo',nuevo='".intval($_POST['proveedor'])."',anterior='".$Usuario['proveedor']."',arreglo='proveedores',usuario='".$_POST['cveusuario']."'");
		}
		...
		//Actualizar el Registro
			$update = " UPDATE mantenimiento_usuarios 
					SET nombre='".$_POST['nombre']."',password='".$_POST['password']."',tipo='".$_POST['tipo']."',proveedor='".$_POST['proveedor']."' 
					WHERE cve='".$_POST['reg']."' " ;
		$ejecutar = mysql_query($update);
		$cveusu=$_POST['reg'];
	} else {
		//Insertar el Registro
		$insert = " INSERT INTO mantenimiento_usuarios (nombre,usuario,password,tipo,proveedor)
					VALUES 
					( '".$_POST['nombre']."','".$_POST['usuario']."','".$_POST['password']."','".$_POST['tipo']."','".$_POST['proveedor']."')";
		$ejecutar = mysql_query($insert) or die(mysql_error());
		$cveusu=mysql_insert_id();
		mysql_query("INSERT mantenimiento_historial SET menu='".$_POST['cvemenu']."',cveaux='".$cveusu."',fecha='".fechaLocal()." ".horaLocal()."',
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		echo '
			<td><a href="#" onClick="$(\'#panel\').show();atcr(\'mantenimiento_usuarios.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver</a></td><td>&nbsp;</td>
			</tr>';
		echo '</table>';
		echo '<br>';
		...
		echo '</select></td></tr>';
		echo '<tr><th align="left">Proveedor</th><td><select name="proveedor" id="proveedor"><option value="0">Todos</option>';
		foreach($array_proveedores as $k=>$v)
		{
			echo '<option value="'.$k.'"';
			if($k==$Usuario['proveedor']) echo ' selected';
			echo '>'.$v.'</option>';
		}
		echo '</select></td></tr>';
		echo '</table>';
		...
		echo '<script>
				function validarUsuario(reg)
				{
					...
					if (objeto.readyState != 0) {
						...
						objeto.open("POST","mantenimiento_usuarios.php",true);
						...
						objeto.onreadystatechange = function()
						{
							if (objeto.readyState==4)
							{
								if(objeto.responseText=="1"){
									...
									atcr("mantenimiento_usuarios.php","",2,reg);
								}
							}
						}
					}
...
/*** PAGINA PRINCIPAL **************************************************/
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr>
				...
				<td><a href="#" onClick="atcr(\'mantenimiento_usuarios.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;Nuevo</td><td>&nbsp;</td>
			 </tr>';
		echo '</table>';
