...
if($_POST['ajax']==1){
	$sel="SELECT cve,fecha,hora,placa from cobro_engomado where estatus!='C' and fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' and plaza='".$_POST['plazausuario']."'";
	$rs=mysql_query($sel) or die(mysql_error());
...
//	echo '<h3>Total de Intentos de 250.00: '.number_format($row[0],2).'</h3>';
	...
	echo'<table width="100%" border="0">
	     <tr bgcolor="#E9F2F8">
		 <th>Folio</th><th>Fecha</th><th>Hora</th><th>Placa</th><th>Fecha</br>Entrega</th><th>Hora</br>Entrega</th><th>Diferencia</th>
		 </tr>';
//		 echo $sel;
	while($row=mysql_fetch_array($rs)){
	rowb();	
	$sel5="SELECT fecha,hora 
	from certificados 
	where estatus='A' and plaza='".$_POST['plazausuario']."' and ticket='".$row['cve']."'";
	$rs5=mysql_query($sel5);
	$row1=mysql_fetch_array($rs5);
	...
	echo'<td align="center">'.$row1['hora'].'</td>';
	$fechaUno=new DateTime($row['hora']);
	$fechaDos=new DateTime($row1['hora']);
	$dateInterval = $fechaUno->diff($fechaDos);
//	echo $dateInterval->format('Total: %H horas %i minutos %s segundos').PHP_EOL;
	echo'<td align="center">'.$dateInterval->format('%H:%i:%s').PHP_EOL.'</td>';
	$c++;
...
//	echo'<td align="right">'.number_format($t_ingreso,2).'</td>';
	...
	echo'<tr bgcolor="#E9F2F8">
	     <td align="left" colspan="7">Registro(s)'.mysql_num_rows($rs).'</td>
	     </tr>';
    echo'</table>';	
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","tiempo_entrega.php",true);
