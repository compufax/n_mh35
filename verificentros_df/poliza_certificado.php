...
if($_POST['cmd']==101){
	...
	header("Expires: 0");
	echo '<h2>Poliza '.$Plaza['numero'].' '.$Plaza['nombre'].' del dia '.$_POST['fecha'].'</h2>';
	echo '<table><tr>
	...
	<td valign="top"><h2>Tipos</h2>
		...
		<tr><td align="center">R</td><td>Reposición</td></tr>
	...
	foreach($array_engomado as $k=>$v){
		...
		if(b.tipo_pago=6 OR b.fecha!=a.fecha,0,b.monto) as monto, a.certificado as certificado, IF(a.fecha!=b.fecha,1,0) as diffechas, b.motivo_intento, b.engomado as engomadoventa, b.anio,
		b.tipo_venta 
		FROM certificados a 
		...
		while($row = mysql_fetch_array($res)){
			...
			$res1 = mysql_query("SELECT fecha,cve,monto FROM cobro_engomado WHERE plaza = '".$_POST['plazausuario']."' AND cve<'".$row['ticket']."' AND placa = '".$row['placa']."' AND anio='".$row['anio']."' AND estatus!='C' ORDER BY cve DESC LIMIT 1");
			$row1 = mysql_fetch_array($res1);
	...
	$c2=count($array_engomado)-1;
	foreach($array_engomado as $k=>$v){
		$select = "SELECT a.cve,a.fecha,a.placa, a.monto, IF(a.tipo_pago=6,0,a.monto) as monto,a.tipo_pago,a.tipo_venta FROM cobro_engomado a  
		...
		while($row = mysql_fetch_array($res)){
			...
			echo '<td align="center">'.$tipo.'</td>';
			echo '<td align="center">'.$c.'</td>';
			if($row['tipo_pago']==7) echo '<td align="center">TD</td>';
			elseif($row['tipo_pago']==5) echo '<td align="center">TC</td>';
			...
			elseif($row['tipo_venta'] == 4)
			{
				echo '<td align="center">PI</td>';
				$row['monto'] = 0;
			}
			else
				echo '<td align="center">Sin Verificar</td>';
			for($i=0;$i<$c2;$i++) echo '<td>&nbsp;</td>';
	...
	$tb=0;
	$res = mysql_query("SELECT * FROM bonos WHERE plaza='".$_POST['plazausuario']."' AND estatus!='C' AND fecha='".$_POST['fecha']."' ORDER BY cve");
	while($row = mysql_fetch_array($res)){
		rowb();
		$tipo='B';
		echo '<td align="center">'.$tipo.'</td>';
	...
	echo '<tr>';
	echo '<td align="center">R</td><td align="right">'.number_format($tr,2).'</td></tr>';
	echo '<tr>';
	...
	echo '<tr>';
	echo '<td align="center">TD</td><td align="right">'.number_format($ttd,2).'</td></tr>';
	echo '<tr>';
	...
	echo '<tr>';
	echo '<td align="center">B</td><td align="right">'.number_format($tb,2).'</td></tr>';
	echo '<tr>';
	echo '<td align="center">Total</td><td align="right">'.number_format($tnv+$tr+$tp+$tpa-$tdv+$tre+$ttc+$ttd-$tb,2).'</td></tr>';
	echo '</table></span>';
...
if($_POST['cmd']==100){
	...
	<td valign="top"><h2>Tipos</h2>
		...
		<tr><td align="center">TC</td><td>Tarjeta Credito</td></tr>
		<tr><td align="center">TD</td><td>Tarjeta Debito</td></tr>
		<tr><td align="center">R</td><td>Repoci&oacute;n</td></tr>
		<tr><td align="center">NV</td><td>No Verificado</td></tr>
		<tr><td align="center">PA</td><td>Pago Anticipado</td></tr>
		<tr><td align="center">RV</td><td>Reverificacion</td></tr>
		<tr><td align="center">CT</td><td>Cortesia</td></tr>
