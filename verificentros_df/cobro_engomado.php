...
$array_tipo_cortesia = array(1=>'Autorizada', 2=>'Pago Anticipado', 3=>'Pago Acumulado');
...
$res=mysql_query("SELECT local,vende_seguros,intentoporcertificadodif,num_intentos,num_intentosanticipados,bloquear_impresion,maneja_medio_pago, monto_medio_pago FROM plazas WHERE cve='".$_POST['plazausuario']."'");
...
if($_POST['cveusuario']==1)
	$res = mysql_query("SELECT * FROM anios_certificados WHERE fecha_ini <= CURDATE() ORDER BY nombre DESC");
elseif($_POST['cmd']==1)
...
$depositantes_vale_credito='<option value="0">Seleccione</option>';
$depositantes_vale_preventa='<option value="0">Seleccione</option>';
if($_POST['cmd']==1)
...
while($row=mysql_fetch_array($res)){
	...
	if($row['tipo_depositante']==6){
		$depositantes_vale_preventa .= '<option value="'.$row['cve'].'">'.htmlentities(utf8_encode($row['nombre'])).'</option>';
	}
	elseif($row['tipo_depositante']==5){
		$depositantes_vale_credito .= '<option value="'.$row['cve'].'">'.htmlentities(utf8_encode($row['nombre'])).'</option>';
	}
	elseif($row['solo_contado']==1){
		$depositantes_contado .= '<option value="'.$row['cve'].'">'.htmlentities(utf8_encode($row['nombre'])).'</option>';
	}
	else{
...
while($row=mysql_fetch_array($res)){
	//if($row['cve'] == 3 && $_POST['plazausuario']==1) $row['costo'] = 208;
...
$array_estatus = array('A'=>'Activo','C'=>'Cancelado', 'B' => 'Bloqueado', 'D' => 'Devuelto');
/*** CONSULTA AJAX  **************************************************/
if($_POST['cmd']==-103){
	...
	$res=mysql_query("SELECT engomado,
		...
		SUM(IF(estatus='A', copias, 0)) as copias
...
if($_POST['ajax']==1) {
		if($_POST['cveusuario']!=3 && $_POST['cveusuario']!=12 && $_POST['cveusuario']!=1){
...
/////////////////////////////////////////////////////////////////////////////////////////////////
	...
	$res=mysql_query("SELECT engomado,
		SUM(IF(tipo_pago!=6 AND tipo_pago!=10,monto,0)),
		COUNT(cve),
		SUM(IF(tipo_pago IN (2),monto,0)),
		SUM(IF(tipo_pago IN (2),1,0)),
		SUM(IF(tipo_pago IN (5),monto,0)),
...
/////////////////////////////////////////////////////////////////////////////////////////////////
		...
		elseif ($_POST['placa']!=""){ 
			...
			if ($_POST['descuento50']!="all") { $select.=" AND a.descuento50='".$_POST['descuento50']."' "; }
...
/*sin*/				elseif($_POST['mostrar']==2) $select.=" AND IFNULL(b.cve,0)=0 AND a.estatus='A'";
			...
			/*if($nivelUsuario < 3) {
				$select .= " AND (DATEDIFF(CURDATE(), a.fecha)<2 OR IFNULL(b.cve,0)>0)";
			}*/
		}
		...
		$permite_cancelar_dias_atras = false;
		if($_POST['cveusuario']==11 && $_POST['plazausuario']==1);
		$permite_cancelar_dias_atras=true;
		if(mysql_num_rows($res)>0) 
		{
//			echo''.$select.'';
			echo '<table width="100%" border="0" cellpadding="4" cellspacing="1" class="">';
			echo '<tr bgcolor="#E9F2F8"><th>&nbsp;</th><!--<th>Asignar Factura<br>
			<input type="checkbox" onClick="if(this.checked) $(\'.chksfacturar\').attr(\'checked\',\'checked\'); else $(\'.chksfacturar\').removeAttr(\'checked\');"></th>-->';

			echo '<th>Ticket</th><th>Fecha</th><!--<th>Referencia Maquina Registradora--></th>';
			if($_SESSION['TipoUsuario'] == 1) echo '<th>Fecha Entrega</th><th>Diferencia</th>';
			echo '<th>Placa</th><!--<th>Marca</th><th>Modelo</th>-->
			<!--<th>Tiene Multa</th><th>Tipo de Certificado</th>--><th>Tipo Venta</th><th>Tipo de Verificacion</th><th>Monto</th><th>Copias</th><th>Total</th>
			...
			foreach($resultado as $i => $row){
				...
				elseif($row['estatus']=='D'){
					...
					if($nivelUsuario>2 && $PlazaLocal != 1 && ($row['factura']==0 || $row['notacredito']>0) && $row['certificado']=="" && ($row['fecha']==fechaLocal() || $_POST['cveusuario']==1 || $permite_cancelar_dias_atras))
				...
				echo '</td>';
				if(($row['tipo_pago']==2 || $row['tipo_pago']==6 || $row['tipo_pago']==10 || $row['tipo_pago']==5 || $row['tipo_pago']==7) && $_POST['tipo_pago']=="all") $row['monto']=0;
				/*echo '<td align="center" width="40" nowrap>&nbsp;';
				...
				elseif($permite_editar==1){
					echo '<td align="center"><input type="text" id="placa_'.$row['cve'].'" class="textField" size="23" value="'.$row['placa'].'" onKeyUp="if(event.keyCode==13){ traeRegistro();}else{this.value = this.value.toUpperCase();}"><br>
				...
				echo '</td>';
				echo '<td>'.$array_engomado[$row['engomado']].'</td>';
				...
				if($permite_editar==1 && $row['estatus']!='C' && $row['vale_pago_anticipado'] > 0){
					echo '<select id="tipo_vale_'.$row['cve'].'"><option value="1">Externo</option>
				...
				elseif($row['vale_pago_anticipado'] > 0){
					if($row['tipo_vale'] == 2) echo 'Interno';
					else echo 'Externo';
				}
			...
			$colspan = 6;
			if($_SESSION['TipoUsuario']==1) $colspan+=2;
			echo '	
				<tr>
				<td colspan="'.$colspan.'" bgcolor="#E9F2F8">';menunavegacion();echo '</td>
...
if($_POST['ajax']==21){
	$res = mysql_query("SELECT SUM(IF(tipo_venta=0,1,0)), SUM(IF(tipo_venta=2 AND codigo_cortesia='' AND tipo_cortesia=2,1,0)) FROM cobro_engomado WHERE plaza='".$_POST['plazausuario']."' AND depositante='".$_POST['depositante']."' AND estatus!='C' AND estatus!='D' AND descuento50!=1");
	$row = mysql_fetch_array($res);
	$cortesias_ganadas = intval($row[0]/10);
	$cortesias_usadas = $row[1];
	$cortesias_disponibles = $cortesias_ganadas - $cortesias_usadas;
	$compras_para_cortesia = $row[0]-($cortesias_ganadas*10);
	echo 'Cortesias Disponibles: '.$cortesias_disponibles.', '.$compras_para_cortesia.' de 10 para la siguiente cortesia';
	exit();
}
...
if($_POST['ajax']==5){
	...
	$res = mysql_query("SELECT a.tipo_venta, b.engomado FROM cobro_engomado a LEFT JOIN certificados b ON a.plaza = b.plaza AND a.cve = b.ticket WHERE a.placa='".$_POST['placa']."' AND a.estatus!='C' AND a.estatus!='D' AND a.anio='".$_POST['anio']."' ORDER BY a.cve DESC");
	$movimientos = mysql_num_rows($res);
	$row=mysql_fetch_array($res);
	/*if($movimientos<=3){
		$row1=mysql_fetch_array($res);
		if(($row['tipo_venta'] == 0 && $row['engomado']==19) && ($row1['tipo_venta']==0 && $row1['engomado']==19)){
			echo '1';
		}
		else{
			echo '0';
	...
	else*/if(($row['tipo_venta'] == 0 && $row['engomado']==19)){
		echo '1';
	}
	else{
		echo '0';
...
if($_POST['ajax']==7){
	if($_POST['tipo_cortesia'] == 2){
		...
		$res = mysql_query("SELECT SUM(IF(tipo_venta=0,1,0)), SUM(IF(tipo_venta=2 AND tipo_cortesia=3,1,0)) FROM cobro_engomado WHERE plaza='".$_POST['plazausuario']."' AND depositante='".$_POST['depositante']."' AND estatus!='C' AND descuento50!=1 AND estatus!='D'");
		$row = mysql_fetch_array($res);
		$cortesias_ganadas = intval($row[0]/10);
		$cortesias_usadas = $row[1];
		$cortesias_disponibles = $cortesias_ganadas - $cortesias_usadas;
...
if($_POST['ajax']==71){
	if($_POST['tipo_vale'] == 1){
		...
		$tipo = ($_POST['tipo_pago'] == 6) ? 0 : 1;
		$res = mysql_query("SELECT cve FROM cobro_engomado WHERE plaza='".$_POST['plazausuario']."' AND estatus!='C' AND estatus!='D' AND tipo_vale='".$_POST['tipo_vale']."' AND vale_pago_anticipado='".$_POST['vale_pago_anticipado']."'");
		if($row = mysql_fetch_assoc($res)){
			echo '2';
			exit();
		}
		$res = mysql_query("SELECT a.depositante FROM pagos_caja a INNER JOIN vales_pago_anticipado b ON a.plaza = b.plaza AND a.cve = b.pago WHERE a.plaza='".$_POST['plazausuario']."' AND a.estatus!='C' AND b.estatus!='C' AND b.cve= '".$_POST['vale_pago_anticipado']."' AND b.tipo='$tipo'");
		if($row = mysql_fetch_array($res)){
...
if ($_POST['ajax']==73){
	...
	if($_POST['tipo_cortesia'] == 3){
		$res = mysql_query("SELECT cve FROM cobro_engomado WHERE plaza='{$_POST['plazausuario']}' AND tipo_venta=2 AND tipo_cortesia=3 AND codigo_cortesia='{$_POST['codigo_cortesia']}' AND fecha>='2023-01-09' AND estatus!='C'");
		if($row = mysql_fetch_array($res)){
			echo 'El vale ya fue utilizado en otro ticket';
		}
		else{
			$res = mysql_query("SELECT depositante FROM vale_cortesia_acumulado WHERE plaza='{$_POST['plazausuario']}' AND folio='{$_POST['codigo_cortesia']}' AND estatus!='C'");
			if($row=mysql_fetch_assoc($res)){
				if($row['depositante'] != $_POST['depositante']){
					echo 'El vale no pertenece al depositante';
				}
				else{
					echo '1';
				}
				...
				echo 'No se encontro el vale de cortesia';
			}
		}
	}
	else{
		$res = mysql_query("SELECT cve FROM cobro_engomado WHERE plaza='{$_POST['plazausuario']}' AND tipo_venta=2 AND tipo_pago=6 AND tipo_cortesia=2 AND codigo_cortesia='{$_POST['codigo_cortesia']}' AND estatus!='C'");
		if($row = mysql_fetch_array($res)){
			echo 'El vale ya fue utilizado en otro ticket';
		}
		else{
			$res = mysql_query("SELECT depositante FROM vales_pago_anticipado WHERE plaza='{$_POST['plazausuario']}' AND cve='{$_POST['codigo_cortesia']}' AND tipo=1 AND estatus!='C'");
			if($row=mysql_fetch_assoc($res)){
				if($row['depositante'] != $_POST['depositante']){
					echo 'El vale no pertenece al depositante';
				}
...
if($_POST['ajax']==80){
	...
	$select = "SELECT cve, nombre FROM depositantes WHERE plaza='{$_POST['plazausuario']}'";
	if($_POST['tipo_pago']==1 || $_POST['tipo_pago']==5 || $_POST['tipo_pago']==7){
		$select .= " AND solo_contado = 1 AND tipo_depositante=0";
	}
	elseif($_POST['tipo_pago']==6){
		$select .= " AND solo_contado != 1 AND tipo_depositante=0";	
	}
	elseif($_POST['tipo_pago']==2){
		$select .= " AND tipo_depositante=5";	
	}
...
if($_POST['cmd']==110) {
	$filename = "Venta de Certificados.xls";
...
//			echo''.$select.'';
			echo '<table width="100%" border="0" cellpadding="4" cellspacing="1" class="">';
			echo '<tr bgcoor="#E9F2F8"><th>&nbsp;</th><!--<th>Asignar Factura<br>
			<input type="checkbox" onClick="if(this.checked) $(\'.chksfacturar\').attr(\'checked\',\'checked\'); else $(\'.chksfacturar\').removeAttr(\'checked\');"></th>-->';
			...
			foreach($resultado as $i => $row){
				rowb();
				echo '<tr><td align="center" width="40" nowrap>';
				if($row['estatus']=='C'){
					echo 'Cancelado<br>'.$array_usuario[$row['usucan']].'<br>'.$row['fechacan'];
					$row['monto']=0;
					if($row['fecha']>='2023-03-01'){
...
//}
			...
			echo '	
				<tr>
				<td colspan="'.$colspan.'" bcolor="#E9F2F8">';menunavegacion();echo '</td>
				<td align="right" bcolor="#E9F2F8">'.number_format($t,2).'</td>
				<td align="right" bgclor="#E9F2F8">'.number_format($t2,2).'</td>
				<td align="right" bgclor="#E9F2F8">'.number_format($t+$t2,2).'</td>
				<td colspan="13" bgclor="#E9F2F8">&nbsp;</td>
				</tr>
			</table>';
...
if($_POST['cmd']==103){
	...
	if ($_POST['usuario']!=""){ 
		$filtro.=" AND a.usuario='".$_POST['usuario']."' "; 
		$filtro2.=" AND a.usuario_aplica='".$_POST['usuario']."' "; 
		echo '<br>USUARIO: '.$array_usuario[$_POST['usuario']];
	}
	else{
	...
	$texto.='<br><br>';
	$res = mysql_query("SELECT COUNT(cve), SUM(IF(estatus='C',1,0)), SUM(IF(estatus='A' OR fecha<'2023-03-01',copias,0)) FROM cobro_engomado a WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' ".$filtro."");
	$row = mysql_fetch_array($res);
	echo '<h2>NUMERO DE REGISTROS: '.$row[0].'<br>CANCELADOS: '.$row[1].'<br></h2>';
	$t1=$t2=$t3=$t4=$t5=$t6=$t7=$t8=0;
	$copias=0;
	$adepositar = 0;
	...
	echo '<table><tr><th>Tipo Pago</th><th>Cantidad</th><th>Importe</th></tr>';
	//$array_tipo_pago = array();
	$array_tipo_pago[-4] = 'CANCELADOS';
	$array_tipo_pago[-1] = 'CORTESIAS';
	$array_tipo_pago[-2] = 'INTENTO';
	$array_tipo_pago[-3] = 'REPOSICION';
	$array_orden = array(-4, 1, 5, 7, 6, -2, 10, 11, -1, -3);
	$res1=mysql_query("SELECT IF(estatus='C',-4,IF(tipo_venta=0,tipo_pago,IF(tipo_venta=1,-2,IF(tipo_venta=2,-1,-3)))) as tipo_pago, SUM(IF(estatus!='C',monto,0)), COUNT(cve), SUM(IF(estatus='A' OR fecha<'2023-03-01',copias,0))  FROM cobro_engomado a WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."'  ".$filtro." GROUP BY IF(estatus='C',-4,IF(tipo_venta=0,tipo_pago,IF(tipo_venta=1,-2,IF(tipo_venta=2,-1,-3))))") or die(mysql_error());
	$cantidad = 0;
	$total = 0;
	$copias=0;
	$resultado = array();
	while($row1=mysql_fetch_array($res1)){
		$resultado[$row1['tipo_pago']] = array($row1[1], $row1[2]);
	...
	foreach($array_orden as $tipo_pago){
		if(isset($resultado[$tipo_pago])){
			echo '<tr><td>'.$array_tipo_pago[$tipo_pago].'</td><td align="right">'.$resultado[$tipo_pago][1].'</td><td align="right">'.number_format($resultado[$tipo_pago][0],2).'</td></tr>';
		}
	}
	echo '<tr><td>TOTAL:</td><td align="right">'.$cantidad.'</td><td align="right">'.number_format($total,2).'</td></tr><tr><td></td></tr></table>';

	echo '<br>COPIAS CANT: '.$copias.'';
	$res3=mysql_query("SELECT SUM(total),SUM(cantidad)  FROM copiasxplaca a WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND estatus='A' $filtro");
	$row3=mysql_fetch_array($res3);
	echo '<br>VENTA DE COPIAS CANT: '.($row3[1]+$copias).', IMP: '.number_format($row3[0]+$copias,2);

	if($maneja_medio_pago==1){
		echo '<table><tr><td colspan="3"><h2>DETALLE DE COBRO DE MEDIOS PAGOS</td></tr>';
		$res1=mysql_query("SELECT tipo_pago, SUM(monto), COUNT(cve), SUM(IF(estatus='A' OR fecha<'2023-03-01',copias,0))  FROM cobro_engomado a WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND tipo_venta=0 AND descuento50=1 AND estatus!='C' ".$filtro." GROUP BY tipo_pago");
		$cantidad = 0;
		$total = 0;
		while($row1=mysql_fetch_array($res1)){
			echo '<tr><td>'.$array_tipo_pago[$row1['tipo_pago']].'</td><td align="right">'.$row1[2].'</td><td align="right">'.number_format($row1[1],2).'</td></tr>';
			if($row1['tipo_pago']!=6){
				$cantidad+=$row1[2];
				$total+=$row1[1];
				if($row1['tipo_pago'] == 1){
	...
	$res1=mysql_query("SELECT COUNT(cve) FROM cobro_engomado a WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND engomado=24 AND estatus!='C' {$filtro}");
	$row1 = mysql_fetch_array($res1);
	echo '<table><tr><td></td></tr><tr><td colspan="3"><h2>DETALLE DE EXCENTO</h2></td></tr><tr><td>CANT EXCENTO:</td><td align="right">'.$row1[0].'</td></tr><tr><td></td></tr>';

	echo '<tr><td colspan="3"><h2>DETALLE DE COBRO DE REPOSICIONES</h2></td></tr>';
	$cantidad = 0;
	$total = 0;
	foreach($array_engomado as $k=>$v){
		echo '<tr><td colspan="3"><h3>'.$v.'</h3></td></tr>';
		$res1=mysql_query("SELECT tipo_pago, SUM(monto), COUNT(cve), SUM(IF(estatus='A' OR fecha<'2023-03-01',copias,0))  FROM cobro_engomado a WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND engomado='".$k."' AND tipo_venta=3 AND estatus!='C' ".$filtro." GROUP BY tipo_pago");
		$cantidade = 0;
		$totale = 0;
		while($row1=mysql_fetch_array($res1)){
			echo '<tr><td>'.$array_tipo_pago[$row1['tipo_pago']].'</td><td align="right">'.$row1[2].'</td><td align="right">'.number_format($row1[1],2).'</td></tr>';
		...
		echo '<tr><td>TOTAL '.$v.':</td><td align="right">'.$cantidade.'</td><td align="right">'.number_format($totale,2).'</td></tr><tr><td></td></tr>';
	}
	echo '<tr><td>TOTAL:</td><td align="right">'.$cantidad.'</td><td align="right">'.number_format($total,2).'</td></tr><tr><td></td></tr></table>';

	
	$efectivo+=$row3[0]+$copias;
	
	echo '<h2>DEVOLUCION EFECTIVO</h2>';
	$res1=mysql_query("SELECT SUM(a.devolucion),COUNT(a.cve)  FROM devolucion_certificado a LEFT JOIN cobro_engomado b ON b.plaza = a.plaza AND b.cve = a.ticket WHERE a.plaza='".$_POST['plazausuario']."' AND a.fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND IFNULL(b.tipo_pago,0) NOT IN (2,6) AND a.estatus!='C' $filtro2");
	$row1=mysql_fetch_array($res1);
	echo ' CANT: '.$row1[1].', IMP: '.number_format($row1[0],2).'';
	$adepositar-=$row1[0];
	...
	while($row3=mysql_fetch_array($res3)){
		...
		$t31+=$row3[1];
		$t32+=$row3[2];
		if($row3['forma_pago'] == 1) $adepositar+=$row3[1];
	}
	echo '<tr><td>TOTAL</td><td>'.$t32.'</td><td>'.number_format($t31,2).'</td></tr></table>';

	echo '<h2>GASTOS</h2>';
	$res3 = mysql_query("SELECT COUNT(cve), SUM(monto) FROM recibos_salida WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND estatus!='C' $filtro");
	$row3 = mysql_fetch_array($res3);
	echo '<table><tr><th>Motivo</th><th>Cantidad</th><th>Importe</th></tr><tr><td>RECIBOS DE SALIDA</td><td>'.$row3[0].'</td><td>'.number_format($row3[1],2).'</td></tr></table>';
	$adepositar-=$row3[1];

	
	echo '<br><br>GRAN TOTAL IMP: '.number_format($t2+$row2[0]-$row1[0]+$t31-$row4[0]-$row5[0],2).'';
	echo '<br>TOTAL A DEPOSITAR IMP: '.number_format($adepositar,2).'';
	exit();
}
...
if($_POST['cmd']==102){
	...
	$res = mysql_query("SELECT COUNT(cve), SUM(IF(estatus='C',1,0)), SUM(IF(estatus='A' OR fecha<'2023-03-01',copias,0)) FROM cobro_engomado a WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' ".$filtro."");
	$row = mysql_fetch_array($res);
	$texto.=' NUMERO DE REGISTROS: '.$row[0].'| CANCELADOS: '.$row[1].'|COPIAS: '.$row[2].'||';
	$copias=$row[2];

	$efectivo = 0;
	$total=0;
	$texto .= 'VERIFICACION|';
	$res=mysql_query("SELECT tipo_pago,COUNT(cve),sum(monto) FROM cobro_engomado a WHERE plaza='".$_POST['plazausuario']."' AND tipo_venta!=3 AND descuento50!=1 AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND estatus!='C' ".$filtro." GROUP BY tipo_pago ORDER BY tipo_pago");
	while($row=mysql_fetch_array($res)){
		if($row[0] == 6) $row[2] = 0;
		if($row[0] == 1){
	...
	$res1=mysql_query("SELECT COUNT(cve) FROM cobro_engomado a WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND engomado=24 AND estatus!='C' {$filtro}");
	$row1 = mysql_fetch_array($res1);
	echo '|EXCENTO|CANT. EXCENTO: '.$row1[0].'||';

	$texto .= 'REPOSICION|';
	$res=mysql_query("SELECT tipo_pago,COUNT(cve),sum(monto) FROM cobro_engomado a WHERE plaza='".$_POST['plazausuario']."' AND tipo_venta=3 AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND estatus!='C' ".$filtro." GROUP BY tipo_pago ORDER BY tipo_pago");
	while($row=mysql_fetch_array($res)){
		if($row[0] == 1){
			$texto.=" EFECTIVO CANT: ".$row[1]." IMP: ".number_format($row[2],2).'|';
			$efectivo += $row[2];
	...
	$res3=mysql_query("SELECT SUM(total),SUM(cantidad)  FROM copiasxplaca a WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND estatus='A' $filtro");
	$row3=mysql_fetch_array($res3);
	$texto.=' VENTA DE COPIAS CANT: '.($row3[1]+$copias).' IMP: '.number_format($row3[0]+$copias,2).'||';
	$efectivo+=$row3[0];
...
if($_POST['cmd']==101){
	...
	$res=mysql_query("SELECT a.* FROM cobro_engomado a WHERE a.plaza='".$_POST['plazausuario']."' AND a.cve='".$_POST['reg']."'");
	$row=mysql_fetch_array($res);
	//if($row['descuento50']==1) $row['monto']=0;
	$texto=chr(27)."@";
	$texto.='|';
	if($row['tipo_venta']==1){
	...
	if($row['tipo_pago'] != 2 && $row['tipo_pago'] != 6 && $rowPlaza['bloqueada_sat'] != 1 && $_GET['reimpresion'] != 1){
		...
		if($row1=mysql_fetch_array($res1)){
			...
			//if($row['plaza']!=59 && $row['plaza']!=1 && $row['plaza']!=15) 
				$texto.=chr(27).'!'.chr(8)." PAGINA PARA FACTURAR:|    http://servicio35.com/facturacion/|";
		}
	}
	...
	$texto.='|';
	$texto.=chr(27).'!'.chr(8)." TOTAL: ".($row['copias']+$row['monto']);
	$texto.='|';
	$texto.=chr(27).'!'.chr(8)." ".numlet(($row['copias']+$row['monto']));
	$texto.='|';
	...
	if($row['tipo_venta'] == 0){
		//$texto.='|AL PAGAR EL SERVICIO SE INFORMA QUE NO EXISTE DEVOLUCION POR CAUSAS NO IMPUTABLES AL CENTRO';
		//$texto.='|||______________________|FIRMA DEL PROPIETARIO|PLACA '.$row['placa'];
	}
	...
	if($row['tipo_venta']==0 && $row['depositante'] > 0 && ($row['tipo_pago']==1 || $row['tipo_pago']==5 || $row['tipo_pago']==7)){
		$res1 = mysql_query("SELECT cve, folio FROM vale_cortesia_acumulado WHERE plaza='{$_POST['plazausuario']}' AND ticket='{$_POST['reg']}' AND estatus!='C'");
		$primero=true;
		$foliosv = '';
		while($row1 = mysql_fetch_array($res1)){
			if($primero){
		...
		$fechav = $row['fecha'];
		$depositantev = $array_depositantes[$row['depositante']];
	}
	$impresion='<iframe src="http://localhost/impresiongenerallogo.php?textoimp='.$texto.'&logo='.str_replace(' ','',$array_plaza[$row['plaza']]).'&foliosv='.$foliosv.'&depositantev='.$depositantev.'&fechav='.$fechav.'&plazav='.$plazav.'&nplazav='.$nplazav.'&barcode=1'.sprintf("%011s",(intval($row['cve']))).'&copia=1" width=200 height=200></iframe>';
	echo '<html><body>'.$impresion.'</body></html>';
	echo '<script>setTimeout("window.close()",10000);</script>';
	exit();
}
...
if($_POST['cmd']==3){
	...
	if($row['tipo_venta']==2 && $row['tipo_cortesia']==3){
		mysql_query("UPDATE vale_cortesia_acumulado SET usado=0 WHERE plaza={$_POST['plazausuario']} AND folio='{$row['codigo_cortesia']}'");
	}
	$_POST['cmd']='0';
}
...
if ($_POST['cmd']==2) {
	...
	if($_POST['tipo_vale']==1 && $_POST['tipo_pago'] == 6) $_POST['monto'] = 0;
	if($_POST['engomado']>0){
		if($_POST['reg']>0){
			...
			$row = mysql_fetch_array($res);
			if($row['placa'] != $_POST['placa'] || $row['engomado'] != $_POST['engomado'] || $_POST['anio'] != $row['anio'] || $_POST['tipo_venta'] != $row['tipo_venta']){
				...
				$insert = " INSERT cobro_engomado 
								...
								tipo_vale='".$_POST['tipo_vale']."',ticketpagointento='$ticketpagointento',copias='".$_POST['copias']."',descuento50='".$_POST['descuento50']."',
				...
				if($_POST['tipo_venta']==2 && $_POST['tipo_cortesia']==3){
					mysql_query("UPDATE vale_cortesia_acumulado SET usado=1 WHERE plaza={$_POST['plazausuario']} AND folio='{$_POST['codigo_cortesia']}'");
				}

				if($_POST['tipo_venta']==0 && $_POST['engomado']==1 && ($_POST['tipo_pago']==1 || $_POST['tipo_pago']==5 || $_POST['tipo_pago']==7) && $_POST['depositante'] > 0){
					if(date('d')<='05'){
						$fecha_ini = date( "Y-m-d" , strtotime ( "-1 month" , strtotime(date("Y-m").'-01') ) );
					}
					else{
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		if($_POST['reg']==0){
			...
			echo '<tr style="display:none;"><th align="left" style="font-size:18px">Tiene Multa</th><td><input type="checkbox" name="multa" id="multa" value="1" style="font-size:18px"></td></tr>';
			echo '<tr style="display:none;"><th align="left">Año Certificado</th><td><select name="anio" id="anio" style="font-size:20px">';
			...
			echo '</select></td></tr>';
			echo '<tr';
			if($row['tipo_venta']!=0 || $maneja_medio_pago != 1){
				echo ' style="display:none;"';
			}
			echo '><th align="left">Medio Pago</th><td><input type="checkbox" name="descuento50" id="descuento50" value="1" onClick="descontar()"></td></tr>';
			echo '<tr';
			...
			/*if(this.value==\'2\' || this.value==\'6\' || this.value==\'1\'){ 
			...
			else if(this.value==\'10\' && document.forma.tipo_venta.value==\'0\'){ 
				$(\'#vale_pago_anticipado\').parents(\'tr:first\').show();
			}
			else if(this.value==\'11\' && document.forma.tipo_venta.value==\'0\'){ 
				$(\'#vale_pago_anticipado\').parents(\'tr:first\').show();
			} "><!--<option value="0">Seleccione</option>-->';
			foreach($array_tipo_pago as $k=>$v){
				echo '<option value="'.$k.'"';
			...
			echo '><th align="left">Vale Externo</th><td><input type="checkbox" id="tipo_vale" name="tipo_vale" value="1"';
			...
			echo '<tr';
			/*if($row['tipo_pago']!=2 && $row['tipo_pago']!=6 && $row['tipo_pago']!=0){
				echo ' style="display:none;"';
			}*/
			echo '><th align="left">Depositante</th><td><select name="depositante" id="depositante" style="font-size:20px" onChange="traer_numero_cortesias_contado()">';
			/*foreach($array_depositantes as $k=>$v){
			...
			echo '</select><span id="capanumerocortesias" style="display:none"></span></td></tr>';
			...
			echo '<tr style="display:none;"><th align="left">Tiene Multa</th><td><input type="hidden" name="multa" id="multa" value="'.$row['multa'].'"><input type="checkbox"';
		...
		echo '<script>
				...
				function traer_numero_cortesias_contado(){
					$("#capanumerocortesias").html("");
					if(document.forma.tipo_pago.value==1 || document.forma.tipo_pago.value==5 || document.forma.tipo_pago.value==7){
						$.ajax({
				...
				function traer_depositantes(){
					...
					else if(document.forma.tipo_pago.value==\'10\'){ 
						$(\'#depositante\').html(\''.$depositantes_vale_credito.'\');
					}
					else if(document.forma.tipo_pago.value==\'11\'){ 
						$(\'#depositante\').html(\''.$depositantes_vale_preventa.'\');
					}
					else{
				...
				function muestra_precio(){
					...
					else if(document.forma.tipo_venta.value=="0" && (document.forma.tipo_pago.value=="10" || document.forma.tipo_pago.value=="11")){
						$(\'#vale_pago_anticipado\').parents(\'tr:first\').show();
					}
					else{
						$(\'#vale_pago_anticipado\').parents(\'tr:first\').hide();
					...
					if(document.forma.tipo_venta.value==0 && "'.$maneja_medio_pago.'" == "1"){
						$("#descuento50").parents("tr:first").show();
					}
					else{
						$("#descuento50").parents("tr:first").hide();
					}
				...
				function descontar(){
					monto_verificacion = $("#engomado").find("option:selected").attr("costo");
					if(document.forma.descuento50.checked)
						monto = '.$monto_medio_pago.';
					else
						monto = monto_verificacion/1;
					document.forma.monto.value = monto.toFixed(0);
					sumarmontos();
				}
				...
				function validar(reg){
					...
					else if(document.forma.engomado.value!="1" && document.forma.descuento50.checked){
						$("#panel").hide();
						alert("El descuento no se puede aplicar a Concesionado y Plataformas");
					}
					else if(document.forma.engomado.value!="1" && (document.forma.tipo_pago.value=="6" || document.forma.tipo_pago.value=="2")){
						$("#panel").hide();
						alert("No se puede pagar con pago anticipado ni credito los tipos de verificacion para plataformas y concesionados");
					}
					else if((document.forma.tipo_pago.value=="2" || document.forma.tipo_pago.value=="6" || document.forma.tipo_pago.value=="10" || document.forma.tipo_pago.value=="11") && document.forma.depositante.value=="0"){
						$("#panel").hide();
					...
					/*else if(validarPlaca() == false){
						...
						$("#panel").hide();
						alert("La placa debe de tener 1 pago previo con rechazo");	
					}
					...
					else if(document.forma.tipo_venta.value=="2" && (document.forma.tipo_cortesia.value=="2" || document.forma.tipo_cortesia.value=="3") && $.trim(document.forma.codigo_cortesia.value)==""){
						$("#panel").hide();
					...
					else if(document.forma.tipo_venta.value=="2" && document.forma.tipo_cortesia.value=="2" && document.forma.tipo_pago.value!="6"){
						$("#panel").hide();
						alert("El tipo de cortesia es para pago anticipado");
					}
					else if(document.forma.tipo_venta.value=="2" && document.forma.tipo_cortesia.value=="3" && document.forma.depositante.value=="0"){
						$("#panel").hide();
						alert("Necesita seleccionar el depositante");
					}
					else if(document.forma.tipo_venta.value=="2" && document.forma.tipo_cortesia.value=="3" && document.forma.tipo_pago.value!="1" && document.forma.tipo_pago.value!="5" && document.forma.tipo_pago.value!="7"){
						$("#panel").hide();
						alert("La cortesia pago acumulado por pagos acumulados de contado");
					}
					else if(validarValePagoAnticipado() == false)
					{
						$("#panel").hide();
					}
					else if(validarValeCortesia() == false)
					{
						$("#panel").hide();
					}
					else if(validarCortesiaAcumulado() == false)
					{
						$("#panel").hide();	
					}
					else if(validarValeCredito() == false)
					{
						$("#panel").hide();
					}
					else if(validarValePreventa() == false)
					{
						$("#panel").hide();
					}
				...
				function validarValeCortesia(){
					if(document.forma.tipo_venta.value == "2" && document.forma.tipo_pago.value == "6" && document.forma.tipo_cortesia.value == "2"){
						regresar = false;
						$.ajax({
						  url: "cobro_engomado.php",
						  type: "POST",
						  ...
						  data: {
						  	depositante: document.getElementById("depositante").value,
							codigo_cortesia: document.getElementById("codigo_cortesia").value,
							plazausuario: document.forma.plazausuario.value,
							ajax: 73
						  },
							success: function(data) {
				...
				function validarValePagoAnticipado(){
					if(document.forma.tipo_venta.value == "0" && document.forma.tipo_pago.value == "6"){
						...
						$.ajax({
						  ...
						  data: {
							depositante: document.getElementById("depositante").value,
							tipo_pago: document.forma.tipo_pago.value, 
				...
				function validarValeCredito(){
					if(document.forma.tipo_venta.value == "0" && document.forma.tipo_pago.value == "10"){
						if((document.forma.vale_pago_anticipado.value/1)<=0){
							alert("Necesita ingresar el vale");
							return false;
						}
				...
				function validarValePreventa(){
					if(document.forma.tipo_venta.value == "0" && document.forma.tipo_pago.value == "11"){
						if((document.forma.vale_pago_anticipado.value/1)<=0){
							alert("Necesita ingresar el vale");
							return false;
						}
				...
				function campos_cortesia(){
					...
					if(document.forma.tipo_cortesia.value=="2" || document.forma.tipo_cortesia.value=="3"){
				...
				function validarCortesiaAcumulado(){
					if(document.forma.tipo_venta.value == "2" && document.forma.tipo_cortesia.value == "3"){
						regresar = false;
						$.ajax({
						  url: "cobro_engomado.php",
						  type: "POST",
				...
				function validarAutoriza(){
					...
					if(document.forma.tipo_venta.value == "2" && document.forma.tipo_cortesia.value != "1" && document.forma.cveusuario.value != "1"){
						...
						$.ajax({
						  ...
						  data: {
							depositante: document.getElementById("depositante").value,
							tipo_cortesia: document.getElementById("tipo_cortesia").value,
							placa: document.getElementById("placa").value,
							anio: document.getElementById("anio").value,
							plazausuario: document.forma.plazausuario.value,
							ajax: 7
							...
							success: function(data) {
								if(data == 2){
									...
									regresar = false;
									alert("El depositante no tiene cortesias");
								}
								else if(data == 1){
									regresar = true;
								}
...
if ($_POST['cmd']<1) {
	...
	if($saldocaja>=6000){
		echo '<h2><font color="RED">En caja tiene mas de 6000 pesos necesita generar un desglose de dinero</font></h2>';
	}
	//Busqueda
		echo '<div id="mnj"></div>';
	echo '<table>';
	...
	echo '<td><a href="#" onClick="atcr(\'cobro_engomado.php\',\'_blank\',\'102\',\'0\');"><img src="images/b_print.png" border="0"></a>&nbsp;Corte</td><td>&nbsp;</td>
			...
			<td><a href="#" onClick="atcr(\'cobro_engomado.php\',\'_blank\',\'191\',\'0\');"><img src="images/b_print.png" border="0"></a>&nbsp;Corte HTML Intento con Importe</td><td>&nbsp;</td>-->
						<td><a href="#" onClick="atcr(\'cobro_engomado.php\',\'_blank\',\'110\',\'0\');"><img src="images/b_print.png" border="0"></a>&nbsp;Excel</td><td>&nbsp;</td>';
	if($nivelUsuario>0){
	...
	echo '<table>';
	echo '<tr';
	if($_POST['cveusuario'] != 1 && $_POST['cveusuario']!=3 && $_POST['cveusuario']!=12) echo ' style="display:none;"';
	echo '><td>Fecha Inicial</td><td><input type="text" name="fecha_ini" id="fecha_ini" class="readOnly" size="12" value="'.fechaLocal().'" readOnly>&nbsp;<a href="#" onClick="displayCalendar(document.forms[0].fecha_ini,\'yyyy-mm-dd\',this,true)"><img src="images/calendario.gif" border="0"></a></td></tr>';
	echo '<tr';
	if($_POST['cveusuario'] != 1 && $_POST['cveusuario']!=3 && $_POST['cveusuario']!=12) echo ' style="display:none;"';
	echo '><td>Fecha Final</td><td><input type="text" name="fecha_fin" id="fecha_fin" class="readOnly" size="12" value="'.fechaLocal().'" readOnly>&nbsp;<a href="#" onClick="displayCalendar(document.forms[0].fecha_fin,\'yyyy-mm-dd\',this,true)"><img src="images/calendario.gif" border="0"></a></td></tr>';
	echo '<tr';
	if($_POST['cveusuario'] != 1) echo ' style="display:none;"';
	echo '><td>Ticket</td><td><input type="text" name="folio" id="folio" size="10" class="textField" value=""></td></tr>';
	...
	echo '</select></td></tr>';
	echo '<tr><td>Tipo de Pago</td><td><select name="tipo_pago" id="tipo_pago" onChange="traerDepositantes()"><option value="all" selected>Todos</option>';
	foreach($array_tipo_pago as $k=>$v){
			echo '<option value="'.$k.'"';
			echo '>'.$v.'</option>';
	}
	...
	echo '</select></td></tr>';
	echo '<tr';
	if($maneja_medio_pago!=1) echo ' style="display:none;"';
	echo '><td>Medio Pago</td><td><select name="descuento50" id="descuento50"><option value="all" selected>Todos</option><option value="0">No</option><option value="1">Si</option></select></td></tr>';
	...
	echo '</select></td></tr>';
	echo '<tr style="display:none;"><td>Tiene Multa</td><td><select name="multa" id="multa"><option value="all" selected>Todos</option>';
