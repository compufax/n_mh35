...
if($_POST['cveusuario']==1)
	$res = mysql_query("SELECT * FROM anios_certificados WHERE cve in (1,2,3,4,5,6) ORDER BY nombre DESC");
elseif($_POST['cmd']==1)
...
if($_POST['ajax']==1) {
		...
		WHERE a.plaza='".$_POST['plazausuario']."' AND a.fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND IFNULL(b.cve,0)=0  and a.estatus!='C'";
...
/*sin*/				elseif($_POST['mostrar']==2) $select.=" AND IFNULL(b.cve,0)=0";
			}
		}
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			echo '<th>Placa</th>
			<th>Tiene Multa</th><!--<th>Tipo de Certificado</th>--><th>Tipo Venta</th><th>Monto</th>
			...
			foreach($resultado as $i => $row){
				...
				echo '</td>';
				echo '<td align="center">'.htmlentities($row['vale_pago_anticipado']).'<br>';
			...
			echo '	
				<tr>
				<td colspan="'.(($_SESSION['TipoUsuario']==1)?9:7).'" bgcolor="#E9F2F8">';menunavegacion();echo '</td>
		...
		echo'|*|';
		if($dinero>=5000){
			echo'<h2 style="color:#FE0000">**En caja tiene mas de 5000 pesos necesita generar un desglose de dinero**</h2>';
...
if($_POST['ajax']==5){
	$res = mysql_query("SELECT monto FROM cobro_engomado WHERE placa='".$_POST['placa']."' AND estatus!='C' AND tipo_venta IN (0,2)  AND DATEDIFF(CURDATE(),fecha)<150 ORDER BY cve DESC LIMIT 1");
	if($row=mysql_fetch_array($res)){
		echo '1';
	}
	else{
...
if($_POST['ajax']==6){
	...
	$res = mysql_query("SELECT cve FROM cobro_engomado WHERE plaza='".$_POST['plazausuario']."' AND placa='".$_POST['placa']."' AND estatus!='C' AND tipo_venta IN (0,2)  AND DATEDIFF(CURDATE(), fecha)<=150 ORDER BY cve DESC LIMIT 1");
	if($row=mysql_fetch_array($res)){
...
if($_POST['ajax']==7){
	...
	$res = mysql_query("SELECT SUM(verificaciones) FROM pagos_caja WHERE plaza='{$_POST['plazausuario']}' AND depositante='{$_POST['depositante']}' AND estatus!='C' AND YEAR(fecha)='".date('Y-m')."'");
	$row = mysql_fetch_array($res);
	$precios = array_values($array_engomadoprecio);
	$cortesias = intval($row[0]/10);

	$res = mysql_query("SELECT COUNT(cve) FROM cobro_engomado WHERE plaza='{$_POST['plazausuario']}' AND depositante='{$_POST['depositante']}' AND estatus!='C' AND tipo_venta='2' AND tipo_cortesia!='1' AND YEAR(fecha)='".date('Y-m')."'");
	$row = mysql_fetch_array($res);
...
if($_POST['ajax']==71){
	if($_POST['tipo_vale'] == 1){
		...
		if($row[1]==1){
			...
			$saldo = $row[0];
			$res = mysql_query("SELECT COUNT(cve) FROM cobro_engomado WHERE plaza='".$_POST['plazausuario']."' AND estatus!='C' AND tipo_vale='".$_POST['tipo_vale']."'");
		...
		$res = mysql_query("SELECT cve FROM cobro_engomado WHERE plaza='".$_POST['plazausuario']."' AND estatus!='C' AND tipo_vale='".$_POST['tipo_vale']."' AND vale_pago_anticipado='".$_POST['vale_pago_anticipado']."'");
		if($row = mysql_fetch_assoc($res)){
			echo '2';
			exit();
		}
...
if($_POST['ajax']==8){
	...
	if($_POST['cveusuario']==1){
		...
		WHERE a.plaza = '".$_POST['plazausuario']."' AND a.placa='".$_POST['placa']."' AND a.tipo_venta!=3 AND a.estatus!='C' AND DATEDIFF(CURDATE(), a.fecha) <= 150 AND CONCAT(a.fecha,' ',a.hora)<'".$row['fecha']." ".$row['hora']."' ORDER BY a.fecha DESC, a.hora DESC LIMIT 2");
		$row = mysql_fetch_array($res);
		if($row['tipo_venta'] == 1){
			$row = mysql_fetch_array($res);
	...
	elseif($intentoporcertificadodif == 1 && $_POST['depositante'] > 0){
		...
		if($row1['agencia'] == 9){
			...
			WHERE a.plaza = '".$_POST['plazausuario']."' AND a.placa='".$_POST['placa']."' AND a.tipo_venta!=3 AND a.estatus!='C' AND DATEDIFF(CURDATE(), a.fecha) <= 150 AND CONCAT(a.fecha,' ',a.hora)<'".$row['fecha']." ".$row['hora']."' ORDER BY a.fecha DESC, a.hora DESC LIMIT 2");
			$row = mysql_fetch_array($res);
			if($row['engomado'] != 19 || ($row['usuario']==1 && $row['tipo_venta']==1))
				echo '1';
			else{
					echo '0';	
			}
...
if($_POST['ajax']==14){
	$res = mysql_query("SELECT depositante FROM cobro_engomado WHERE plaza='".$_POST['plazausuario']."' AND estatus!='C' AND TRIM(placa)='".trim($_POST['placa'])."' AND DATEDIFF(CURDATE(),fecha)<150 AND tipo_venta IN (0,2) ORDER BY cve DESC");
	$row = mysql_fetch_array($res);
	if($row['depositante'] != $_POST['depositante'])
		echo '1';
	exit();
...
if($_POST['cmd']==102){
	...
	$res=mysql_query("SELECT engomado,SUM(IF(tipo_pago!=6,monto,0)),COUNT(cve),
		SUM(IF(tipo_pago IN (2),monto,0)),SUM(IF(tipo_pago IN (2),1,0)),SUM(IF(tipo_pago IN (5),monto,0)),
		SUM(IF(tipo_pago IN (5),1,0)),SUM(IF(tipo_pago IN (7),monto,0)),
		SUM(IF(tipo_pago IN (7),1,0)) FROM cobro_engomado a WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND estatus!='C' ".$filtro." GROUP BY engomado");
	while($row=mysql_fetch_array($res)){
		/*$texto.=" ".$array_engomado[$row['engomado']].'|';
		$res1=mysql_query("SELECT tipo_pago,SUM(monto),COUNT(cve)  FROM cobro_engomado WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND engomado='".$row['engomado']."' AND estatus!='C' $filtro GROUP BY tipo_pago");
		while($row1=mysql_fetch_array($res1)){
	...
	$texto.=' VENTA  CANT: '.($t1-$t3-$t5).', IMP: '.number_format($t2-$t4-$t6-$t8,2).'|';
	//$texto.=' GRAN TOTAL VENTA CRED CANT: '.$t3.', IMP: '.number_format($t4,2).'|';
	//$texto.=' GRAN TOTAL VENTA      CANT: '.$t1.', IMP: '.number_format($t2,2).'||';
	...
	$texto.=' TOTAL A DEPOSITAR IMP: '.number_format($t2-$t4-$t6-$t8+$row2[0]-$row1[0]+$t33-$row4[0]-$row5[0],2).'||';
...
if($_POST['cmd']==102.1){
	...
	$res=mysql_query("SELECT engomado,SUM(IF(tipo_pago!=6,monto,0)),COUNT(cve),
		...
		FROM cobro_engomado a WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND estatus!='C' ".$filtro." GROUP BY engomado");
	while($row=mysql_fetch_array($res)){
		$texto.=" ".$array_engomado[$row['engomado']].'|';
...
if($_POST['cmd']==101){
	...
	$texto.='|';
	if($row['tipo_pago'] != 2 && $_GET['reimpresion']!=1){
		...
		if($_POST['reg']>0){
			...
			if($row['placa'] != $_POST['placa'] || $row['engomado'] != $_POST['engomado'] || $_POST['anio'] != $row['anio']){
				...
				if($_POST['tipo_venta']!=1){
					...
					$res1 = mysql_query("SELECT cve FROM cobro_engomado WHERE plaza='".$_POST['plazausuario']."' AND placa = '".$_POST['placa']."' AND estatus!='C' AND tipo_venta IN (0,2) ORDER BY cve DESC LIMIT 1");
					$row1 = mysql_fetch_array($res1);
					$ticketpago = $row1[0];
				}
				$_POST['anio']=6;
				...
				if($_POST['monto']>0 && $_POST['tipo_pago'] != 2){
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		if($_POST['reg']==0){
			...
			echo '><th align="left">Vale Anterior</th><td><input type="checkbox" id="tipo_vale" name="tipo_vale" value="1"></td></tr>';
			echo '<tr';
			if($row['tipo_pago']!=6){
				echo ' style="display:none;"';
			}
		...
		echo '<script>
				...
				function validar(reg){
					...
					else if(validarNumIntento() == false){
				...
				function validarAutoriza(){
					if(document.forma.tipo_venta.value == "2" && document.forma.tipo_cortesia.value != "1"){
						regresar = false;
						$.ajax({
						  url: "cobro_engomado.php",
						  type: "POST",
...
if ($_POST['cmd']<1) {
	...
	$row = mysql_fetch_array($res);
	$res1 = mysql_query("SELECT SUM(monto) FROM pagos_caja WHERE plaza='".$_POST['plazausuario']."' AND estatus!='C' AND tipo_pago IN (1,2,6) AND fecha>='2017-01-16'");
	$row1 = mysql_fetch_array($res1);
	$row[0]+=$row1[0];
