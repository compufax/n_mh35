...
$pdf->AliasNbPages();
  ...
  $array_plazas=array();
	$res=mysql_query("SELECT a.* FROM plazas a INNER JOIN datosempresas b ON a.cve = b.plaza WHERE a.estatus!='I' AND a.cve IN (".$_POST['plaza'].") AND b.rfc LIKE '%".$_POST['rfc']."%' ORDER BY a.numero, a.nombre");
	while($row=mysql_fetch_array($res)){
...
if($_POST['cmd']==100){
	...
	$array_plazas=array();
	$res=mysql_query("SELECT a.* FROM plazas a INNER JOIN datosempresas b ON a.cve = b.plaza WHERE a.estatus!='I' AND a.cve IN (".$_POST['plaza'].") AND b.rfc LIKE '%".$_POST['rfc']."%' ORDER BY a.nombre");
	while($row=mysql_fetch_array($res)){
		$array_plazas[$row['cve']]=$row['numero'].' '.$row['nombre'];
	}
	...
	echo '<th>No de Facturas</th><th>Subtotal Facturas</th>
	<th>IVA Facturas</th><th>Total Facturas</th><th>No Canceladas Facturas</th>
	<th>Facturas sin Timbrar</th>
	<th>Facturas sin Timbrar Canceladas</th>
	<th>No de Notas de Credito</th><th>Subtotal Notas de Credito</th>
	<th>IVA Notas de Credito</th><th>Total Notas de Credito</th><th>No Canceladas Notas de Credito</th>
	<th>Notas de Credito sin Timbrar</th>
	<th>Notas de Credito sin Timbrar Canceladas</th>
	<th>Timbres Ocupados</th></tr>'; 
	$sumacargo=array(0,0,0,0,0,0);
	$x=0;
	foreach($array_plazas as $k=>$v){
		//if($_POST['rfc']=='' || $_POST['rfc']==$array_plazasrfc[$k]){
		//	if($_POST['plaza']=='all' || $k==$_POST['plaza']){
				...
				SUM(IF(a.respuesta1!='' AND DATE(a.fechatimbre)>='".$_POST['fecha_ini']."' AND DATE(a.fechatimbre)<='".$_POST['fecha_fin']."',1,0)),
				SUM(IF(a.respuesta2!='' AND DATE(a.fechacan)>='".$_POST['fecha_ini']."' AND DATE(a.fechacan)<='".$_POST['fecha_fin']."',1,0))
				FROM notascredito as a WHERE a.plaza='".$k."' AND ((DATE(a.fechatimbre)>='".$_POST['fecha_ini']."' AND DATE(a.fechatimbre)<='".$_POST['fecha_fin']."') OR (DATE(a.fechacan)>='".$_POST['fecha_ini']."' AND DATE(a.fechacan)<='".$_POST['fecha_fin']."')) ";
				...
				echo '<td align="right">'.number_format($row1[2],2).'</td>';
				echo '<td align="right">'.number_format($row1[3],2).'</td>';
				echo '<td align="right">'.number_format($row1[4],0).'</td>';
				echo '<td align="right">'.number_format($row1[5],0).'</td>';
				echo '<td align="right">'.number_format($row1[6],0).'</td>';
				echo '<td align="right">'.number_format($row[7]+$row[8]+$row1[7]+$row1[8],0).'</td>';
				echo '</tr>';
				$x++;
				$sumacargo[0]+=$row[0];
				$sumacargo[1]+=$row[1];
				...
				$sumacargo[12]+=$row1[5];
				$sumacargo[13]+=$row1[6];
				$sumacargo[14]+=$row[7]+$row[8]+$row1[7]+$row1[8];
		//	}
		//}
	}
	$c=4;
...
if($_POST['ajax']==1){
	...
	<th>IVA Notas de Credito</th><th>Total Notas de Credito</th><th>No Canceladas Notas de Credito</th>
	<th>Notas de Credito sin Timbrar</th>
	<th>Notas de Credito sin Timbrar Canceladas</th><th>Timbres Ocupados</th></tr>'; 
	$sumacargo=array(0,0,0,0,0,0);
	$x=0;
	foreach($array_plazas as $k=>$v){
		//if($_POST['rfc']=='' || $_POST['rfc']==$array_plazasrfc[$k]){
