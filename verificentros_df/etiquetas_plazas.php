...
if($_POST['ajax']==1){
	...
	$select = "SELECT * FROM etiquetas_plazas WHERE DATE(fechahora) BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' ORDER BY cve DESC";
	$res=mysql_query($select);
	$totalRegistros = mysql_num_rows($res);
	if(mysql_num_rows($res)>0) 
	{
		echo '<table width="100%" border="0" cellpadding="4" cellspacing="1" class="">';
		echo '<tr bgcolor="#E9F2F8"><th>&nbsp;</th><th>Consecutivo</th><th>Fecha</th><th>Mensaje</th><th>Plazas</th><th>Usuario</th></tr>';
		while($row = mysql_fetch_array($res)){
			rowb();
			echo '<td align="center">';
			if($row['estatus']!='C'){
				echo '<a href="#" onClick="if(confirm(\'Esta seguro de terminar el mensaje\')) atcr(\'etiquetas_plazas.php\',\'\',\'3\','.$row['cve'].')"><img src="images/validono.gif" border="0" title="Cancelar '.$row['cve'].'"></a>';
			}
			else{
				echo 'TERMINADO';
			}
			...
			echo '<td align="left">'.utf8_encode($row['mensaje']).'</td>';
...
if($_POST['cmd']==3){
	mysql_query("UPDATE etiquetas_plazas SET estatus='C',fechaquita=NOW() WHERE cve='".$_POST['reg']."'");
	$_POST['cmd']=0;
}
...
top($_SESSION);	
	if(($_POST['cveusuario']!=1 && $_POST['cmd'] == 0) || $_POST['cmd']==1){
		...
		$select=" SELECT * FROM etiquetas_plazas ORDER BY cve DESC LIMIT 1 ";
		$res=mysql_query($select);
		...
		echo '
			<tr>';
		echo '<td><a href="#" onClick="$(\'#panel\').show();atcr(\'etiquetas_plazas.php\',\'\',\'2\',\'0\');"><img src="images/guardar.gif" border="0">&nbsp;Guardar</a></td><td>&nbsp;</td>';
		if($_POST['cmd']==1)
			echo '<td><a href="#" onClick="$(\'#panel\').show();atcr(\'etiquetas_plazas.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Guardar</a></td><td>&nbsp;</td>';
		echo '
			</tr>';
		echo '</table>';
		echo '<br>';
		...
		//Formulario 
		echo '<table>';
		echo '<tr><td class="tableEnc">Edicion Etiqueta</td></tr>';
		echo '</table>';
	...
	if($_POST['cveusuario']==1 && $_POST['cmd'] == 0){
		...
		echo '<td><a href="#" onClick="atcr(\'etiquetas_plazas.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;Nuevo</td><td>&nbsp;</td>';
		echo '
			 </tr>';
		echo '</table>';
		echo '<table>';
	...
	<Script language="javascript">
		...
		function buscarRegistros()
		{
			...
			if (objeto.readyState != 0) {
				...
				objeto.open("POST","etiquetas_plazas.php",true);
