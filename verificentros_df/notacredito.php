...
if($_POST['cmd']==16){
	...
	if($zip->open("../cfdi/zipcfdis".$fecha.".zip",ZipArchive::CREATE)){
	    ...
	    if(file_exists("../cfdi/zipcfdis".$fecha.".zip")){ 
	        ...
	        readfile("../cfdi/zipcfdisnc".$fecha.".zip"); 
	         
	        unlink("../cfdi/zipcfdisnc".$fecha.".zip"); 
	        foreach($_POST['checksf'] as $cvefact){
				$res = mysql_query("SELECT * FROM notascredito WHERE plaza='".$_POST['plazausuario']."' AND cve='".$cvefact."'");
				$row = mysql_fetch_array($res);
				if($row['estatus']=='C')
					@unlink("../cfdi/comprobantes/ncc_".$_POST['plazausuario']."_".$cvefact.".pdf");
				else
	...
	if(mysql_num_rows($rsabonos)>0) {
		...
		while ($Abono=mysql_fetch_array($rsabonos)){	
			...
			elseif($Abono['respuesta1']==""){
				...
				if(nivelUsuario()>2 && $bloqueada_sat!=1){
					echo '&nbsp;&nbsp;<a href="#" onClick="if(confirm(\'Esta seguro de cancelar?\')){$(\'#panel\').show();atcr(\'notacredito.php\',\'\',\'3\',\''.$Abono['cve'].'\');}"><img src="images/validono.gif" border="0" title="Cancelar '.$Abono['folio'].'"></a>';
				}
				//if($_POST['cveusuario']==1)
...
if($_POST['cmd']==5){
	...
	//echo '</pre>';
	//print_r($documento);
	$documento = genera_arreglo_facturacion($_POST['plazausuario'], $cvefact, 'E');
		...
		if($row['respuesta1']!=""){
			...
			if($resultadotimbres['seguir']){
				...
				if($err!=""){
					...
					$oSoapClient->timeout = 300;
					$oSoapClient->response_timeout = 300;
					$respuesta = $oSoapClient->call("CancelarCFDI", array ('id' => $rowempresa['idplaza'],'rfcemisor' =>$rowempresa['rfc'],'idcertificado' => $rowempresa['idcertificado'],'uuid' => $row['respuesta1'], 'usuario' => $rowempresa['usuario'],'password' => $rowempresa['pass']));
					if ($oSoapClient->fault) {
						echo '<p><b>Fault: ';
						print_r($respuesta);
						echo '</b></p>';
...
if($_POST['cmd']==2){
	...
	factura='".$_POST['factura']."',engomado='".$_POST['engomado']."',banco='".$ClienteCuenta['banco']."',cuenta_cheque='".$ClienteCuenta['cuenta']."',tiene_descuento='".$_POST['tiene_descuento']."'".$datossucursal;
	...
	mysql_query("UPDATE cobro_engomado SET notacredito = '$cvefact' WHERE plaza='".$_POST['plazausuario']."' AND factura='".$_POST['factura']."' AND factura>0");
	mysql_query("UPDATE facturas SET estatus = 'D' WHERE plaza='".$_POST['plazausuario']."' AND cve='".$_POST['factura']."'");
	mysql_query("UPDATE venta_engomado_factura SET notacredito = '$cvefact' WHERE plaza='".$_POST['plazausuario']."' AND factura='".$_POST['factura']."'");
...
if($_POST['cmd']=='recargar'){
	...
	if($_POST['cmd']==1){
		...
		echo '<tr><td>Nota de Credito de Engomado</td><td><input type="checkbox" name="engomado" id="engomado" value="1" onClick="facturar_engomado();" checked></td></tr>';
		echo '<tr id="id_descuento"><td>Tiene 10% Descuento</td><td><input type="checkbox" name="tiene_descuento" id="tiene_descuento" value="1" onClick="aplicar_descuento();sumarproductos();"></td></tr>';
		...
		if($i==0){
			...
			echo '<td align="center"><input type="text" class="readOnly" size="10" name="precio['.$i.']" id="precio'.$i.'" value=""  onKeyUp="sumarproductos()" readOnly>
	...
	if ($_POST['cmd']<1) {
		...
		//if($bloqueada_sat!=1)
		//	echo '<td><a href="#" onClick="atcr(\'notacredito.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0">&nbsp;Nuevo</a></td><td>&nbsp;</td>';
		if(nivelUsuario()>0){
