...
if($_POST['cmd']==101){
	...
	$pdf->Cell(190,10,'Verificentro - '.$array_plaza[$_POST['plazausuario']],0,0,'C');
	$pdf->Ln();
	$pdf->Ln();
	$pdf->SetFont('Arial','B',18.4);
	$pdf->Cell(190,10,'Fecha: '.$_POST['anio'].'-'.$_POST['mes'],0,0,'l');
	$select= " SELECT a.*, b.nombre as engomado, IFNULL(c.cve,0) as cveentrega, IFNULL(d.cve,0) as cvecancelacion FROM certificados_ecologia a LEFT JOIN engomados b ON b.cve = a.tipo 
	LEFT JOIN certificados c ON c.plaza = a.plaza AND c.estatus!='C' AND c.certificado = a.certificado
	LEFT JOIN certificados_cancelados d ON d.plaza = a.plaza AND d.estatus!='C' AND d.certificado = a.certificado 
	WHERE a.plaza='".$_POST['plazausuario']."' AND a.fecha = '".$_POST['anio']."-".$_POST['mes']."'";
	...
	if($_POST['mostrar']==1) $select .= " AND IFNULL(c.cve,0)=0 AND IFNULL(d.cve,0)=0";
	elseif($_POST['mostrar']==2) $select .= " AND (IFNULL(c.cve,0)>0 OR IFNULL(d.cve,0)>0)";
			
	$select.=" GROUP BY a.cve ORDER BY a.fecha DESC, a.cve DESC";

	$pdf->Ln();
	$pdf->SetFont('Arial','B',9);
	$fecha=$_POST['fecha_ini'];
	$pdf->GetX();
	$pdf->Cell(38,4,'Mes',1,0,'C');
	$pdf->Cell(38,4,'Certificado',1,0,'C');
	$pdf->Cell(38,4,'Tipo.',1,0,'C');
	$pdf->Cell(38,4,'Estado.',1,0,'C');
	$pdf->Cell(38,4,'Placa',1,0,'C');
	$pdf->Ln();
	$res = mysql_query($select);
	$x = 0;
	while($row = mysql_fetch_array($res)){
		$pdf->Cell(38,4,$row['fecha'],1,0,'C');
		$pdf->Cell(38,4,$row['certificado'],1,0,'C');
		$pdf->Cell(38,4,$row['engomado'],1,0,'C');
		$pdf->Cell(38,4,$row['estado'],1,0,'C');
		$pdf->Cell(38,4,$row['placa'],1,0,'C');
		$x++;
		$pdf->Ln();
	}
	$pdf->Cell(190,4,$x.'Registros',1,0,'C');
...
if($_POST['ajax']==1){
	...
	echo '<th>Mes</th><th>Certificado</th><th>Tipo</th><th>Estado</th><th>Placa</th></tr>';
	$res = mysql_query($select);
	$x = 0;
	while($row = mysql_fetch_array($res)){
		rowb();
		echo '<td align="center">'.$row['fecha'].'</td>';
		if($row['cveentrega'] == 0 && $row['cvecancelacion'] == 0)
...
if($_POST['cmd']==1){
	...
	echo '
		<tr>';
		if(nivelUsuario()>1)
			echo '<td><a href="#" onClick="$(\'#panel\').show();atcr(\'certificados_ecologia.php\',\'\',\'2\',\'0\');"><img src="images/guardar.gif" border="0">&nbsp;Guardar</a></td><td>&nbsp;</td>';
		echo '<td><a href="#" onClick="$(\'#panel\').show();atcr(\'certificados_ecologia.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver</a></td><td>&nbsp;</td>
		</tr>';
	echo '</table>';
	echo '<br>';
	echo '<table>';
...
if($_POST['cmd'] == 0){
	echo '<table>';
	echo '<tr>
			<td><a href="#" onclick="buscarRegistros();"><img src="images/buscar.gif" border="0"></a>&nbsp;&nbsp;Buscar</td><td>&nbsp;
				<a href="#" onClick="atcr(\'\',\'_blank\',\'101\',\'0\');"><img src="images/b_print.png" border="0" title="Imprimir"></a>Imprimir</td>';

	echo '<td><a href="#" onClick="atcr(\'certificados_ecologia.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;Subir Archivo</td><td>&nbsp;</td>
		 </tr>';
	echo '</table>';
	echo '<table>';
	...
	echo '<tr><td>Certificado</td><td><input type="text" name="certificado" id="certificado" size="15" class="textField" value=""></td></tr>';
	echo '<tr><td>Estado</td><td><select name="estado" id="estado"><option value="">Todos</option>';
		foreach($array_estado as $k=>$v){
			echo '<option value="'.$k.'">'.$v.'</option>';
		}
		echo '</select></td></tr>';
	echo '<tr><td>Mostrar</td><td><select name="mostrar" id="mostrar"><option value="">Todos</option>';
	echo '<option value="1">Sin Movimientos</option><option value="2">Con Movimientos</option>';
		echo '</select></td></tr>';
	echo '</table>';
	...
	function buscarRegistros(btn)
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","certificados_ecologia.php",true);
			objeto.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
			objeto.send("ajax=1&anio="+document.getElementById("anio").value+"&mes="+document.getElementById("mes").value+"&placa="+document.getElementById("placa").value+"&cveusuario="+document.getElementById("cveusuario").value+"&cvemenu="+document.getElementById("cvemenu").value+"&plazausuario="+document.getElementById("plazausuario").value+"&numeroPagina="+document.getElementById("numeroPagina").value+"&certificado="+document.getElementById("certificado").value+"&estado="+document.getElementById("estado").value+"&mostrar="+document.getElementById("mostrar").value);
