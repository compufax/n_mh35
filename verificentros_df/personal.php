...
if ($_POST['cmd']==2) {
	...
	if($_POST['reg']) {
			...
			if($Conductor['estatus']!=$_POST['estatus']){
				...
				if($_POST['estatus']==1){
					mysql_query("insert into asistencia (plaza, fecha, personal, estatus) select a.plaza, CURDATE(), a.cve, IF(IFNULL(b.motivo,0) > 0, 2, 0) from personal a left join dias_justificados b on a.cve = b.personal and b.fecha = CURDATE() where a.cve='".$_POST['reg']."'");
				}
...
//		echo''.$select.'';
		if(mysql_num_rows($rspersonal)>0) 
		{
			...
			if($_POST['cveusuario']==1) echo '<th>Usuario Creo</th><th>Fecha Creo</th>';
...
//				echo '<td align="right">'.number_format($Personal['salario_integrado']+$Personal['compensacion'],2).'</td>';
				...
				if($_POST['cveusaurio']==1){
					$res = mysql_query("SELECT b.usuario, a.fecha_reg FROM cambios_datos_personal a INNER JOIN usuarios b ON b.cve = a.usuario WHERE a.cve_personal = '".$Usuario['cve']."' ORDER BY a.cve LIMIT 1");
					$row = mysql_fetch_array($res);
					echo '<td align="center">'.$row['usuario'].'</td>';
					echo '<td align="center">'.$row['fecha_reg'].'</td>';
				}
				echo '</tr>';
			}
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		if($_POST['reg'] == 0) $Personal['fecha_ini'] = date('Y-m-d');
		//Menu
		echo '<table>';
		echo '
			<tr>';
			...
			if(nivelUsuario()>1){
				echo '<td><a href="#" onClick="validar_rfc(\''.$_POST['reg'].'\');"><img src="images/guardar.gif" border="0">&nbsp;Guardar</a></td><td>&nbsp;</td>';
		...
		//if($Personal['plaza']==0 || $_POST['cveusuario']==1){
		if($Personal['plaza']==0 || nivelUsuario()>2 || $Personal['estatus'] == 2){
		...
		while($Plaza=mysql_fetch_array($rsPlaza)){
			if($_POST['plazausuario']==0 || $_POST['plazausuario']==$Plaza['cve'] || $Personal['estatus'] == 2 || nivelUsuario()>2){
	...
	//	echo '<input type="hidden" name="regunidad" id="unidad" value="">';
		...
		echo '<script language="javascript">
				...
				function validar_rfc(reg){
					...
					else if($.trim(document.getElementById("rfc").value)=="")
						alert("Necesita seleccionar la plaza del personal");
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr>
				<td><a href="#" onclick="buscarRegistros();"><img src="images/buscar.gif" border="0"></a>&nbsp;&nbsp;Buscar&nbsp;&nbsp;
					...
					<!--<a href="#" onClick="atcr(\'personal.php\',\'_blank\',\'50\',\'0\');"><img src="images/b_print.png" border="0"></a>&nbsp;Exce--></td><td>&nbsp;</td>
			 </trx|>';
		echo '</table>';
		echo '<table>';
		...
		echo '</select></td><td></td><td>&nbsp;</td></tr>';
		echo '<tr style="display:none"><td>Fecha Inicial</td><td><input type="text" name="fecha_ini" id="fecha_ini" class="readOnly" size="12" readonly>&nbsp;<a href="#" onClick="displayCalendar(document.forms[0].fecha_ini,\'yyyy-mm-dd\',this,true)"><img src="images/calendario.gif" border="0"></a></td></tr>';
		echo '<tr style="display:none"><td>Fecha Final</td><td><input type="text" name="fecha_fin" id="fecha_fin" class="readOnly" size="12" readonly>&nbsp;<a href="#" onClick="displayCalendar(document.forms[0].fecha_fin,\'yyyy-mm-dd\',this,true)"><img src="images/calendario.gif" border="0"></a></td></tr>';
		echo '<tr style="display:none"><td>Credencia</td><td><input type="text" name="credencial" id="credencial" class="textField"></td></tr>'; 
		echo '<tr><td>Nombre</td><td><input type="text" name="nombre" id="nombre" class="textField"></td></tr>'; 
		echo '<tr style="display:none"><td>Clave Ecologica</td><td><input type="text" name="num" id="num" class="textField"></td></tr>'; 
		echo '<tr><td>Puesto</td><td><select name="puesto" id="puesto" class="textField"><option value="all">---Todos---</option>';
		foreach($array_puestos as $k=>$v){
			echo '<option value="'.$k.'">'.$v.'</option>';
		}
		echo '</select></td></tr>';
		echo '<tr style="display:none"><td>Asegurado</td><td><select name="asegurado" id="asegurado" class="textField"><option value="all">---Todos---</option>';
