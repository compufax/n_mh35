...
if($_POST['cmd']==101){
		...
		echo '<br>';
		echo '<h2>Intentos del tipo '.$row['nombre'].' del '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'</h2>';
...
if($_POST['cmd']==100){
	...
	$html='<html><body><h1 align="center">Plaza: '.$array_plazas[$_POST['plazausuario']].'</h2><h3 align="right">Fecha: '.fechaLocal().'-'.horaLocal().'</h3></br><h2>Inventario del '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'</h2>';
	$html.= '<table width="100%" border="1" cellpadding="4" cellspacing="1" class="" style="font-size:13px">';
	$html.= '<tr bgcolor="#E9F2F8"><th>Tipo de Verificacion</th><th>Inicial</th><th>Compra</th><th>Consumidos</th><th>Cancelados</th>
	<th>Cortesia</th><th>Intentos</th><th>F/Ingreso</th><th>Final</th><!--<th>Total Compra</th><th>Total Venta</th>--></tr>';
	$totales = array();
...
//		$html.= '<td align="right">'.number_format($ventas,2).'</td>';
	...
	$html.= '<tr bgcolor="#E9F2F8"><th>Tipo de Verificacion</th><th>Consumidos</th><th>Total de Venta</th>';
	$totales = array();
...
if($_POST['ajax']==1){
	...
	<th>Cortesia</th><th>Intentos</th><th>S/Ingreso</th><th>Final</th><!--<th>Total Compra</th><th>Total Venta</th>--></tr>';
	$totales = array();
	$res = mysql_query("SELECT * FROM engomados WHERE entrega=1 ORDER BY nombre");
	while($row = mysql_fetch_array($res)){
		rowb();
	...
	if($k<7){
		...
		else echo '<th align="right">'.number_format($v,0).'</th>';
		}
	}
	echo '<th align="right">'.number_format($tfi,0).'</th>';
	echo '</tr>';
	$res=mysql_query("SELECT SUM(monto) FROM pagos_caja a WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND estatus!='C' $filtro1");
	$row=mysql_fetch_array($res);
	rowb();
...
top($_SESSION);
	if($_POST['cmd']==2){
		...
		echo '<input type="hidden" name="fecha_ini" value="'.$_POST['fecha_ini'].'">';
		echo '<input type="hidden" name="fecha_fin" value="'.$_POST['fecha_fin'].'">';
		echo '<h2>Intentos del tipo '.$row['nombre'].' del '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].' de la placa '.$_POST['reg'].'</h2>';
		...
		$res=mysql_query("SELECT b.*,a.cve as certificado, a.certificado as holograma, a.fecha as fecha_entrega,a.engomado as engomado_entrega, b.depositante
			...
			IF(a.fecha<c.fecha_ini,c.fecha_ini,a.fecha) BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND TRIM(b.placa) = '".$_POST['reg']."'");
	...
	if($_POST['cmd']==1){
		...
		$res1=mysql_query("SELECT TRIM(b.placa),COUNT(b.cve)
			...
			INNER JOIN cobro_engomado b ON b.plaza = a.plaza AND b.cve = a.ticket
			INNER JOIN anios_certificados c ON c.cve = a.anio
			WHERE a.plaza='".$_POST['plazausuario']."' AND a.engomado='".$row['cve']."' AND a.estatus!='C' AND b.tipo_venta=1 AND IF(a.fecha<c.fecha_ini,c.fecha_ini,a.fecha) BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' GROUP BY TRIM(b.placa)");
		while($row1 = mysql_fetch_array($res1)){
			rowb();
			echo '<td><a href="#" onClick="atcr(\'concentrado.php\',\'\',2,\''.$row1[0].'\')">'.$row1[0].'</a></td>';
			echo '<td align="center">'.$row1[1].'</td>';
