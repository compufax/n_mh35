...
if($_POST['ajax']==2) {
		//Listado de plazas
		$select= " SELECT a.*,CONCAT(c.numero,' ',c.nombre) as nomplaza FROM certificados a INNER JOIN plazas c ON c.cve = a.plaza WHERE a.placa='".$_POST['placa']."' "; 
...
//				echo '<td align="center">'.htmlentities($array_tipo_combustible[$row1['tipo_combustible']]).'</td>';
		...
		//Listado de plazas
		$select= " SELECT a.*,CONCAT(c.numero,' ',c.nombre) as nomplaza FROM certificados_cancelados a INNER JOIN plazas c ON c.cve = a.plaza WHERE a.placa='".$_POST['placa']."' "; 
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			while($row=mysql_fetch_array($res)) {
				...
				echo '<td align="center">'.htmlentities($row['certificado']).'</td>';
				echo '<td align="center">'.htmlentities($row['placa']).'</td>';
				echo '<td align="center">'.htmlentities(utf8_encode($array_personal[$row['plaza']][$row['tecnico']])).'</td>';
...
if ($_POST['cmd']<1) {
	...
	echo '<tr style="display:none;"><td>Mostrar</td><td><select name="tipo" id="tipo"><option value="1">Ventas</option><option value="2">Entregas</option></select></td></tr>';
	echo '<tr style="display:none;"><td>A&ntilde;o</td><td><select name="anio" id="anio">';
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","consulta_placa.php",true);
			objeto.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
			objeto.send("ajax="+document.getElementById("tipo").value+"&anio="+document.getElementById("anio").value+"&placa="+document.getElementById("placa").value+"&cveusuario="+document.getElementById("cveusuario").value+"&cvemenu="+document.getElementById("cvemenu").value+"&plazausuario="+document.getElementById("plazausuario").value+"&numeroPagina="+document.getElementById("numeroPagina").value);
