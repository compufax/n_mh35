	...
	$Plaza=mysql_fetch_array($res);
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			while($row=mysql_fetch_array($res)) {
				...
				/*else{
					//echo '<a href="#" onClick="atcr(\'certificados_extra.php\',\'_blank\',\'101\','.$row['cve'].')"><img src="images/b_print.png" border="0" title="Imprimir '.$row['cve'].'"></a>';
					if(nivelUsuario()>2 && $row['factura']==0 && $PlazaLocal != 1 && (fechaLocal() == $row['fecha'] || $_POST['cveusuario']==1 || $permite_editar==1)){
						echo '<a href="#" onClick="if(confirm(\'Esta seguro de cancelar el registro\')) atcr(\'certificados_extra.php\',\'\',\'3\','.$row['cve'].')"><img src="images/validono.gif" border="0" title="Cancelar '.$row['cve'].'"></a>';
					}
					if(nivelUsuario()>1 && $row['factura']==0 && $PlazaLocal != 1 && (fechaLocal() == $row['fecha'] || $_POST['cveusuario']==1)){
						if(nivelUsuario()>2){
							if($row['entregado']==0){
								echo '&nbsp;&nbsp;<a href="#" onClick="if(confirm(\'Esta seguro de que fue entregado el certificado\')) atcr(\'certificados_extra.php\',\'\',\'4\','.$row['cve'].')"><img src="images/validosi.gif" border="0" title="Entregado '.$row['cve'].'"></a>';
							}
							else{
								echo '&nbsp;&nbsp;<a href="#" onClick="if(confirm(\'Esta seguro de que no fue entregado el certificado\')) atcr(\'certificados_extra.php\',\'\',\'5\','.$row['cve'].')"><img src="images/cerrar.gif" border="0" title="No Entregado '.$row['cve'].'"></a>';
							}
						}
					}
				}*/	
...
if($_POST['ajax']==1) {
		...
		$permite_editar = $row['permite_editar'];
		//Listado de plazas
		$select= " SELECT a.*, b.tipo_venta, b.tipo_pago, d.nombre as nomdepositante, b.engomado as engomadoticket, b.tipo_combustible, b.factura, f.cve as referencia, b.cambios_documento FROM certificados a 
		INNER JOIN cobro_engomado b ON b.plaza = a.plaza AND b.cve = a.ticket
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			echo '<th>Folio de Nota</th><th>Tipo de Venta</th><th>Tipo de Pago</th><th>Placa</th><th>Marca</th><th style="display:none;">Problemas con OBDII</th>
			...
			while($row=mysql_fetch_array($res)) {
				...
				if($row['estatus']=='C'){
					...
					//echo '<a href="#" onClick="atcr(\'certificados_extra.php\',\'_blank\',\'101\','.$row['cve'].')"><img src="images/b_print.png" border="0" title="Imprimir '.$row['cve'].'"></a>';
					if(nivelUsuario()>2 && $row['factura']==0 && $PlazaLocal != 1 && (fechaLocal() == $row['fecha'] || $_POST['cveusuario']==1 || $permite_editar==1)){
						echo '<a href="#" onClick="if(confirm(\'Esta seguro de cancelar el registro\')){ obscan=prompt(\'Motivo Cancelacion\'); atcr(\'certificados_extra.php?obscan=\'+obscan,\'\',\'3\','.$row['cve'].');}"><img src="images/validono.gif" border="0" title="Cancelar '.$row['cve'].'"></a>';
					}
					if(nivelUsuario()>1 && $row['factura']==0 && $PlazaLocal != 1 && (fechaLocal() == $row['fecha'] || $_POST['cveusuario']==1)){
						if(nivelUsuario()>2){
							if($row['entregado']==0){
...
if($_POST['ajax']==2){
	...
	$res = mysql_query("SELECT a.* FROM cobro_engomado a INNER JOIN cobro_engomado_referencia b ON a.plaza = b.plaza AND a.cve = b.ticket WHERE a.plaza='".$_POST['plazausuario']."' AND a.placa='".$_POST['placa']."' AND a.fecha='{$_POST['fecha_nota']}' AND b.cve='".$folio."' ORDER BY a.cve DESC LIMIT 1");
	if($row=mysql_fetch_array($res)){
		if($row['estatus']=='C'){
			echo '-2|';
		}
				...
				if($row=mysql_fetch_array($res)){
					...
					if(mysql_num_rows($res)>0 || $ValidarCertificados == 0){
						$insert = " INSERT certificados 
										SET 
										plaza = '".$_POST['plazausuario']."',fecha_captura='".fechaLocal()."',fecha='{$_POST['fecha']}',hora='".horaLocal()."',monto='".$array_engomadoprecio[$_POST['engomado']]."',
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		echo '
			...
			echo '<td><a href="#" onClick="$(\'#panel\').show();atcr(\'certificados_extra.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver</a></td><td>&nbsp;</td>
			</tr>';
		echo '</table>';
		echo '<br>';
		...
		echo '<table>';
		echo '<tr><td>Fecha</td><td><input type="text" name="fecha" id="fecha" class="textField" size="12" value="'.fechaLocal().'" readOnly>&nbsp;<a href="#" onClick="displayCalendar(document.forms[0].fecha,\'yyyy-mm-dd\',this,true)"><img src="images/calendario.gif" border="0"></a></td></tr>';
		...
		echo '<tr><td>Fecha Nota</td><td><input type="text" name="fecha_nota" id="fecha_nota" class="textField" size="12" value="'.fechaLocal().'" readOnly>&nbsp;<a href="#" onClick="displayCalendar(document.forms[0].fecha_nota,\'yyyy-mm-dd\',this,true)"><img src="images/calendario.gif" border="0"></a></td></tr>';
		echo '<tr><th align="left">Nota</th><td><input type="text" name="nota" id="nota" class="textField enteros" style="font-size:20px" size="10" value="" onKeyUp="if(event.keyCode==13){ traeTicket();} else{ this.value = this.value.toUpperCase();}">&nbsp;&nbsp;<font color="RED">Dar enter para traer el engomado del ticket</font></td></tr>';
		...
		echo '<script>
				...
				function validar(){
					...
					if(document.forma.ticket.value==""){
						...
						validarCertificado();
						//atcr("certificados_extra.php","",2,0);
					}
				}
				...
				function validarCertificado(){
					...
					if (objeto.readyState != 0) {
						...
						objeto.open("POST","certificados_extra.php",true);
				...
				function traeTicket(){
					if(document.forma.nota.value==""){
						...
						if (objeto.readyState != 0) {
							...
							objeto.open("POST","certificados_extra.php",true);
							objeto.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
							objeto.send("ajax=2&placa="+document.getElementById("placa").value+"&nota="+document.getElementById("nota").value+"&fecha_nota="+document.getElementById("fecha_nota").value+"&plazausuario="+document.getElementById("plazausuario").value);
							objeto.onreadystatechange = function()
							{
								if (objeto.readyState==4)
								{
...
/*** PAGINA PRINCIPAL **************************************************/
	...
	if ($_POST['cmd']<1) {
		...
		if($PlazaLocal!=1)
			echo '<td><a href="#" onClick="atcr(\'certificados_extra.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;Nuevo</td><td>&nbsp;</td>';
		echo '<td><a href="#" onClick="atcr(\'certificados_extra.php\',\'_blank\',\'100\',\'0\');"><img src="images/b_print.png" border="0"></a>&nbsp;Imprimir</td><td>&nbsp;</td>';
		if($_POST['cveusuario']==1)
			echo '<td><a href="#" onClick="atcr(\'certificados_extra.php\',\'_blank\',\'200\',\'0\');"><img src="images/b_print.png" border="0"></a>&nbsp;Desglose</td><td>&nbsp;</td>';
		echo '<td><a href="#" onClick="atcr(\'certificados_extra.php\',\'_blank\',\'102\',\'0\');"><img src="images/b_print.png" border="0"></a>&nbsp;Excell</td><td>&nbsp;</td>';
		echo '
			 </tr>';
		echo '</table>';
