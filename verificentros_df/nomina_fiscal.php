...
if($_POST['cmd'] == 51){
	...
	$Plaza=mysql_fetch_array(mysql_query("SELECT * FROM plazas WHERE cve='".$_POST['plazausuario']."'"));
	$Empresa=mysql_fetch_array(mysql_query("SELECT * FROM datosempresas WHERE plaza='".$_POST['plazausuario']."'"));
	$Periodo=mysql_fetch_array(mysql_query("SELECT * FROM periodos_nomina WHERE cve='".$_POST['periodo']."'"));
	$pdf->AddPage();
	$pdf->SetFont("Arial","B",20);
	$pdf->Cell(190,5,$Plaza['nombre'],0,0,'C');
	$pdf->SetFont("Arial","",10);
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Cell(190,4,fecha_letra(fechaLocal()));
	$pdf->Ln();
	$pdf->Cell(190,4,'RFC: '.$Empresa['rfc']);
	$pdf->Ln();
	$pdf->Cell(190,4,'Registro patronal: '.$Empresa['regimen']);
	$pdf->Ln();
	$pdf->Ln();
	$pdf->SetFont("Arial","B",20);
	$pdf->Cell(190,5,'Reporte de la nómina',0,0,'C');
	$pdf->SetFont("Arial","",10);
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Cell(190,4,'Periodo de pago: Del '.fecha_letra($Periodo['fecha_ini']).' al '.fecha_letra($Periodo['fecha_fin']));
	$pdf->Ln();
	$pdf->Cell(190,4,'Periocidad: Quincenal ');
	$pdf->Ln();
	$pdf->Cell(190,4,'Forma de pagos: Todas',0,0,'R');
	$pdf->Ln();
	$pdf->SetFont("Arial","B",20);
	$pdf->Cell(190,5,'PERCEPCIONES',0,0,'C');
	$pdf->SetFont("Arial","",10);
	$pdf->Ln();
	$totalgravable=0;
	$totalpercepciones=0;
	$totaldeducciones=0;
	$res=mysql_query("SELECT SUM(b.total) FROM personal_nomina a INNER JOIN personal_nomina_percepcion b ON a.cve = b.nomina WHERE a.periodo = '".$_POST['periodo']."' AND a.plaza_empleado = '".$_POST['plazausuario']."' AND percepcion = 1");
	$row=mysql_fetch_array($res);
	if($row[0] > 0){
		$res1=mysql_query("SELECT SUM(b.total) FROM personal_nomina a INNER JOIN personal_nomina_deduccion b ON a.cve = b.nomina WHERE a.periodo = '".$_POST['periodo']."' AND a.plaza_empleado = '".$_POST['plazausuario']."' AND deduccion = 1");
		$row1=mysql_fetch_array($res1);
		$row[0]-=$row1[0];
		$pdf->Ln();
		$pdf->Cell(30,4,'P001');
		$pdf->Cell(60,4,'SUELDO');
		$pdf->Cell(30,4,number_format($row[0],2),0,0,'R');
		$totalgravable+=$row[0];
		$totalpercepciones+=$row[0];
	}
	$res=mysql_query("SELECT SUM(b.total) FROM personal_nomina a INNER JOIN personal_nomina_percepcion b ON a.cve = b.nomina WHERE a.periodo = '".$_POST['periodo']."' AND a.plaza_empleado = '".$_POST['plazausuario']."' AND percepcion = 32");
	$row=mysql_fetch_array($res);
	if($row[0] > 0){
		$pdf->Ln();
		$pdf->Cell(30,4,'P002');
		$pdf->Cell(60,4,'AGUINADO');
		$pdf->Cell(30,4,number_format($row[0],2),0,0,'R');
		$totalgravable+=$row[0];
		$totalpercepciones+=$row[0];
	}
	$res=mysql_query("SELECT SUM(IF(b.dias>9,9,b.dias)*b.montoxdia*2) FROM personal_nomina a INNER JOIN personal_nomina_percepcion b ON a.cve = b.nomina WHERE a.periodo = '".$_POST['periodo']."' AND a.plaza_empleado = '".$_POST['plazausuario']."' AND percepcion = 4 AND dias>0");
	$row=mysql_fetch_array($res);
	if($row[0] > 0){
		$pdf->Ln();
		$pdf->Cell(30,4,'P003');
		$pdf->Cell(60,4,'HORAS EXTRAS DOBLES');
		$pdf->Cell(30,4,number_format($row[0],2),0,0,'R');
		$totalgravable+=$row[0];
		$totalpercepciones+=$row[0];
	}
	$res=mysql_query("SELECT SUM((b.dias-9)*b.montoxdia*3) FROM personal_nomina a INNER JOIN personal_nomina_percepcion b ON a.cve = b.nomina WHERE a.periodo = '".$_POST['periodo']."' AND a.plaza_empleado = '".$_POST['plazausuario']."' AND percepcion = 4 AND dias>9");
	$row=mysql_fetch_array($res);
	if($row[0] > 0){
		$pdf->Ln();
		$pdf->Cell(30,4,'P005');
		$pdf->Cell(60,4,'HORAS EXTRAS TIPLES');
		$pdf->Cell(30,4,number_format($row[0],2),0,0,'R');
		$totalgravable+=$row[0];
		$totalpercepciones+=$row[0];
	}
	$res=mysql_query("SELECT SUM(b.total) FROM personal_nomina a INNER JOIN personal_nomina_percepcion b ON a.cve = b.nomina WHERE a.periodo = '".$_POST['periodo']."' AND a.plaza_empleado = '".$_POST['plazausuario']."' AND percepcion = 2");
	$row=mysql_fetch_array($res);
	if($row[0] > 0){
		$pdf->Ln();
		$pdf->Cell(30,4,'P021');
		$pdf->Cell(60,4,'PRIMA DOMINICAL');
		$pdf->Cell(30,4,number_format($row[0],2),0,0,'R');
		$totalgravable+=$row[0];
		$totalpercepciones+=$row[0];
	}
	$res=mysql_query("SELECT SUM(b.total) FROM personal_nomina a INNER JOIN personal_nomina_percepcion b ON a.cve = b.nomina WHERE a.periodo = '".$_POST['periodo']."' AND a.plaza_empleado = '".$_POST['plazausuario']."' AND percepcion = 7");
	$row=mysql_fetch_array($res);
	if($row[0] > 0){
		$pdf->Ln();
		$pdf->Cell(30,4,'P021');
		$pdf->Cell(60,4,'PRIMA VACACIONAL');
		$pdf->Cell(30,4,number_format($row[0],2),0,0,'R');
		$totalpercepciones+=$row[0];
	}
	$res=mysql_query("SELECT SUM(b.total) FROM personal_nomina a INNER JOIN personal_nomina_percepcion b ON a.cve = b.nomina WHERE a.periodo = '".$_POST['periodo']."' AND a.plaza_empleado = '".$_POST['plazausuario']."' AND percepcion = 29");
	$row=mysql_fetch_array($res);
	if($row[0] > 0){
		$pdf->Ln();
		$pdf->Cell(30,4,'P038');
		$pdf->Cell(60,4,'COMPENSACIONES');
		$pdf->Cell(30,4,number_format($row[0],2),0,0,'R');
		$totalpercepciones+=$row[0];
	}
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Cell(60,4,'');
	$pdf->Cell(60,4,'Total de percepciones');
	$pdf->Cell(30,4,number_format($totalpercepciones,2),0,0,'R');
	$pdf->Ln();
	$pdf->Ln();
	$pdf->SetFont("Arial","B",20);
	$pdf->Cell(190,5,'DEDUCCIONES',0,0,'C');
	$pdf->SetFont("Arial","",10);
	$pdf->Ln();
	$subsidio=0;
	$res=mysql_query("SELECT SUM(b.total) FROM personal_nomina a INNER JOIN personal_nomina_deduccion b ON a.cve = b.nomina WHERE a.periodo = '".$_POST['periodo']."' AND a.plaza_empleado = '".$_POST['plazausuario']."' AND deduccion = 3");
	$row=mysql_fetch_array($res);
	if($row[0] > 0){
		$pdf->Ln();
		$pdf->Cell(30,4,'D001');
		$pdf->Cell(60,4,'ISR');
		$pdf->Cell(30,4,number_format($row[0],2),0,0,'R');
		$totaldeducciones+=$row[0];
	}
	$res=mysql_query("SELECT SUM(b.total) FROM personal_nomina a INNER JOIN personal_nomina_deduccion b ON a.cve = b.nomina WHERE a.periodo = '".$_POST['periodo']."' AND a.plaza_empleado = '".$_POST['plazausuario']."' AND deduccion = 2");
	$row=mysql_fetch_array($res);
	if($row[0] > 0){
		$pdf->Ln();
		$pdf->Cell(30,4,'D002');
	...
	$res=mysql_query("SELECT SUM(b.total) FROM personal_nomina a INNER JOIN personal_nomina_deduccion b ON a.cve = b.nomina WHERE a.periodo = '".$_POST['periodo']."' AND a.plaza_empleado = '".$_POST['plazausuario']."' AND deduccion = 5");
	$row=mysql_fetch_array($res);
	if($row[0] > 0){
		$pdf->Ln();
		$pdf->Cell(30,4,'D004');
		$pdf->Cell(60,4,'PRESTAMO PERSONAL');
		$pdf->Cell(30,4,number_format($row[0],2),0,0,'R');
		$totaldeducciones+=$row[0];
	}
	$res=mysql_query("SELECT SUM(b.total) FROM personal_nomina a INNER JOIN personal_nomina_deduccion b ON a.cve = b.nomina WHERE a.periodo = '".$_POST['periodo']."' AND a.plaza_empleado = '".$_POST['plazausuario']."' AND deduccion = 4");
	$row=mysql_fetch_array($res);
	if($row[0] > 0){
		$pdf->Ln();
		$pdf->Cell(30,4,'D094');
		$pdf->Cell(60,4,'CREDITO INFONAVIT');
		$pdf->Cell(30,4,number_format($row[0],2),0,0,'R');
		$totaldeducciones+=$row[0];
	}
	$res=mysql_query("SELECT SUM(b.total) FROM personal_nomina a INNER JOIN personal_nomina_percepcion b ON a.cve = b.nomina WHERE a.periodo = '".$_POST['periodo']."' AND a.plaza_empleado = '".$_POST['plazausuario']."' AND percepcion = 3");
	$row=mysql_fetch_array($res);
	if($row[0] > 0){
		$pdf->Ln();
		$pdf->Cell(30,4,'D100');
		$pdf->Cell(60,4,'SUBSIDIO AL EMPLEO');
		$pdf->Cell(30,4,'-'.number_format($row[0],2),0,0,'R');
		$totaldeducciones-=$row[0];
		$subsidio=$row[0]/-1;
	}
	...
	$pdf->Ln();
	$pdf->Cell(60,4,'');
	$pdf->Cell(60,4,'Total de deducciones');
	$pdf->Cell(30,4,number_format($totaldeducciones),0,0,'R');

	$pdf->Ln();
	$pdf->Ln();
	$pdf->Ln();
	$pdf->SetFont("Arial","B",20);
	$pdf->Cell(190,5,'TOTALES',0,0,'C');
	$pdf->SetFont("Arial","",10);
	$pdf->Ln();
	$pdf->Ln();
	$pdf->SetX(50);
	$pdf->Cell(60,4,'TOTAL EN EFECTIVO');
	$pdf->Cell(30,4,number_format($totalpercepciones-$totaldeducciones,2),0,0,'R');
	$pdf->Ln();
	$pdf->SetX(50);
	$pdf->Cell(60,4,'NETO PAGADO');
	$pdf->Cell(30,4,number_format($totalpercepciones-$totaldeducciones,2),0,0,'R');
	$pdf->Ln();
	$pdf->SetX(50);
	$pdf->Cell(60,4,'Total gravable');
	$pdf->Cell(30,4,number_format($totalgravable,2),0,0,'R');
	$pdf->Ln();
	$pdf->SetX(50);
	$pdf->Cell(60,4,'Subs. Empleo');
	$pdf->Cell(30,4,number_format($subsidio,2),0,0,'R');
	$pdf->Ln();
	$pdf->SetX(50);
	$pdf->Cell(60,4,'Total en especie');
	$pdf->Cell(30,4,number_format(0,2),0,0,'R');

	$pdf->Ln();
	$pdf->Ln();
	$pdf->Ln();
	$pdf->MultiCell(190,4,'Las percepciones marcadas con * se pagan en especie.
		EL NETO PAGADO incluye el total en efectivo y el total en especie.');

	$pdf->Ln();
	$pdf->Ln();
	$pdf->Cell(90,4,'Usuario: '.$array_usuario[$_POST['cveusuario']]);
	$pdf->Cell(90,4,'Fecha y hora '.fecha_letra(date('Y-m-d')).''.date('H:i:s'),0,0,'R');
	$pdf->Output();
	exit();
}
...
if($_POST['cmd']==50){
	...
	$select= " SELECT a.cuenta,a.nombre,IF(b.plazatimbro!=0,b.plazatimbro,a.plaza) as plaza,a.nombre,a.puesto,a.rfc,a.imss,b.cve,b.totalpercepciones,b.totaldeducciones,a.salario_integrado as salario_base_personal, b.sal_diario,
	...
	$worksheet1->write_string(3,$c,'Cuenta');$c++;
	...
	while($Personal=mysql_fetch_array($rspersonal)) {
		...
		$worksheet1->write_string($l,$c,$Personal['cuenta']);$c++;
		...
		$array_totales[$c]+=$total_deducciones;
		$worksheet1->write_string($l,$c,$total_deducciones);$c++;
		$array_totales[$c]+=$total_percepciones-$total_deducciones;
		$worksheet1->write_string($l,$c,$total_percepciones-$total_deducciones);$c++;
		$l++;
	}
	foreach($array_totales as $c=>$monto){
		$worksheet1->write_string($l,$c,$monto);$c++;
	}
	$workbook->close();
	exit();
}
...
if($_POST['cmd']==102){
	...
	$zip = new ZipArchive();
	$error = $zip->open("cfdi/nominas_".$_POST['cveempresa']."_".$cveperiodo.".zip",ZipArchive::CREATE);
...
'.$Plaza['calle'].' '.$Plaza['numexterior'].', '.$Plaza['colonia'],0,'J');   
			...
			$pdf->Output("cfdi/nomina_".$_POST['cveempresa']."_".$personal.".pdf","F");
			if(file_exists("cfdi/comprobantes/barcoden_".$Personal['empresa'].'_'.$Personal['cve'].".png")) unlink("cfdi/comprobantes/barcoden_".$Personal['empresa'].'_'.$Personal['cve'].".png");
			$zip->addFile("cfdi/nomina_".$_POST['cveempresa']."_".$personal.".pdf","recibo_nomina_".$_POST['cveempresa']."_".$personal.".pdf");
			//unlink("cfdi/nomina_".$_POST['cveempresa']."_".$personal.".pdf");
		}
		$zip->addFile("cfdi/comprobantes/cfdin_".$_POST['cveempresa']."_".$personal.".xml","recibo_nomina_".$_POST['cveempresa']."_".$personal.".xml");
	}
	$zip->close();
	if(file_exists("cfdi/nominas_".$_POST['cveempresa']."_".$cveperiodo.".zip")){ 
        header('Content-type: "application/zip"'); 
        header('Content-Disposition: attachment; filename="nominas_'.$_POST['cveempresa'].'_'.$cveperiodo.'.zip"'); 
        readfile("cfdi/nominas_".$_POST['cveempresa']."_".$cveperiodo.".zip"); 
...
'.$Plaza['calle'].' '.$Plaza['numexterior'].', '.$Plaza['colonia'],0,'J');     
			...
			if($email != ''){
				...
				$mail->AddAttachment("cfdi/comprobantes/cfdin_".$_POST['cveempresa']."_".$personal.".xml", "Recibo de Nomina ".$cvefact.".xml");
				$mail->Send();
	...
	foreach($_POST['cvepersonal'] as $personal){
		...
		while($row=mysql_fetch_array($res)){
			...
			if($monto>0){
				...
				$nomina['metodopago']=$array_tipo_pagosat[$Personal['metodo_pago']];
						...
						if ($oSoapClient->fault) {
							...
							if ($err){
								...
								foreach($nominas as $k=>$v){
									...
									if($respuesta['resultado']){
										$Personal = mysql_fetch_array(mysql_query("SELECT plaza FROM personal WHERE cve='".$v['empleado']['numero']."'"));
										...
										plazatimbro='".$plazas_nominas[$k]."',rfc='".$v['empleado']['rfc']."',rfc_empresa='".$Empresa['rfc']."',
										nombre='".$v['empleado']['nombre']."',imss='".$v['empleado']['nss']."',puesto='".$v['empleado']['puesto']."',
										departamento='".$v['empleado']['departamento']."',metodo_pago='".$v['metodopago']."',plaza_empleado='".$Personal['plaza']."',
										nombre_empresa='".addslashes($Empresa['nombre'])."'
										WHERE cve=".$k);
...
if($_POST['ajax']==1){
	...
	if($_POST['plazausuario']>0){
		$res=mysql_query("SELECT * FROM periodos_nomina_plaza WHERE periodo = '".$Periodo['cve']."' AND plaza = '".$_POST['plazausuario']."'");
		$PeriodoPlaza = mysql_fetch_array($res);
		$Periodo['generada_fiscal']=$PeriodoPlaza['generada_fiscal'];
	}
	else{
		$Periodo['generada_fiscal']=1;
	}
	...
	if($Periodo['generada_fiscal']==1){
		...
		if($Periodo['aguinaldo']==1){
			$select= " SELECT a.cuenta,a.nombre,a.plaza,a.nombre,a.puesto,a.rfc,a.imss,b.cve,b.totalpercepciones,b.totaldeducciones,a.salario_integrado as salario_base_personal, b.sal_diario,
			a.sdi as salario_integrado_personal, b.dias_tra, a.cobro_prestamo as monto_prestamo, a.cve as cvepersonal, b.uuid, b.baja, 0 as honorarios,a.fecha_imss,
			IF(a.infonavit='SI',a.monto_infonavit,0) as monto_infonavit, b.cancelacion_sat, b.cadenaoriginal, IF(a.fecha_ini>'".$Periodo['fecha_ini']."',ROUND(((DATEDIFF('".$Periodo['fecha_fin']."',a.fecha_ini)+1)*".$Periodo['dias_trabajados']."/(DATEDIFF('".$Periodo['fecha_fin']."','".$Periodo['fecha_ini']."')+1)),2),'".$Periodo['dias_trabajados']."') as dias_lab
			FROM personal as a 
	...
	if(mysql_num_rows($rspersonal)>0) 
	{
		...
		if($Periodo['generada_fiscal']==1){
			echo '<th rowspan="2"><input type="checkbox" name="selall" value="1" onClick="if(this.checked) $(\'.chkpersonal\').attr(\'checked\',\'checked\'); else $(\'.chkpersonal\').removeAttr(\'checked\');"';
			if($_POST['plazausuario']==0) echo ' disabled';
			echo '></th>';
			echo '<th rowspan="2">Estatus Timbre</th><th rowspan="2">Fecha Timbre</th>';
		}
		if(count($array_plaza)>1) echo '<th rowspan="2">Plaza</th>';
		echo '
		<th rowspan="2">Registro Patronal</th><th rowspan="2"><a href="#" onclick="SortTable(2,\'S\',\'tabla1\');">Nombre</a></th><th rowspan="2">Puesto</th><th rowspan="2">R.F.C.</th><th rowspan="2">N.S.S.</th><th rowspan="2">Cuenta</th>';
		if($Periodo['aguinaldo']==1){
		...
		while($row1=mysql_fetch_array($res1)){
			...
			elseif($row1['cve']==32){
				echo '<th>Sueldo Base</th><th>Dias Aguinaldo</th><th>Importe</th>';
			}
			else{
		...
		while($Personal=mysql_fetch_array($rspersonal)) {
			...
			echo '<td align="center">'.$Personal['cuenta'].'</td>';
			...
			while($row2=mysql_fetch_array($res2)){
				if($row2['cve']==1){
					echo '<td align="right">'.number_format($row2['dias'],0).'</td>';
					$array_totales[$indice_total]=-1;$indice_total++;
				}
...
if($_POST['cmd']==4){
	mysql_query("UPDATE periodos_nomina_plaza a SET generada_fiscal=0 WHERE periodo='".$_POST['cveperiodo']."' AND plaza='".$_POST['plazausuario']."'");
	$_POST['cmd']=0;
}

if($_POST['cmd']==3){
	$res = mysql_query("SELECT cve FROM periodos_nomina_plaza WHERE periodo='".$_POST['cveperiodo']."' AND plaza='".$_POST['plazausuario']."'");
	if($row=mysql_fetch_array($res)){
		mysql_query("UPDATE periodos_nomina_plaza SET generada_fiscal=1 WHERE cve='".$row['cve']."'");
	}
	else{
		mysql_query("INSERT periodos_nomina_plaza SET generada_fiscal=1,periodo='".$_POST['cveperiodo']."',plaza='".$_POST['plazausuario']."'");
	}
	mysql_query("UPDATE personal_nomina a INNER JOIN personal b ON b.cve=a.personal SET a.plazatimbro='".$_POST['plazausuario']."' WHERE a.periodo='".$_POST['cveperiodo']."' AND a.plazatimbro=0 AND b.plaza='".$_POST['plazausuario']."'");
...
if($_POST['cmd']==1){
	...
	echo '</table>';
	echo '<br>';
	echo '<input type="hidden" name="periodo_nomina" id="periodo_nomina" value="'.$Nomina['periodo'].'">';
	...
	while($rowp=mysql_fetch_array($resp)){
		...
		elseif($rowp['cve']==32){
			if($Periodo['generada_nofiscal']!=1){
				$resDias=mysql_query("SELECT IF('".$Personal['fecha_ini']."'>'".$Periodo['fecha_ini']."',ROUND(((DATEDIFF('".$Periodo['fecha_fin']."','".$Personal['fecha_ini']."')+1)*".$Periodo['dias_trabajados']."/(DATEDIFF('".$Periodo['fecha_fin']."','".$Periodo['fecha_ini']."')+1)),2),'".$Periodo['dias_trabajados']."')");
...
if($_POST['cmd']==0){
	...
	echo '<tr>
			...
			<td><a href="#" onclick="if(!$(\'.chkpersonal\').is(\':checked\')) alert(\'Necesita seleccionar al menos un personal\'); else atcr(\'nomina_fiscal.php\',\'_blank\',\'102\',\'\');"><img src="images/zip_grande.png" border="0" width="15px" height="15px" title="Descargar"></a>&nbsp;&nbsp;Descargar Archivos&nbsp;&nbsp;</td>
			<td><a href="#" onclick="$(\'#confirm\').dialog(\'open\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;&nbsp;Enviar Email&nbsp;&nbsp;</td>';
	if($_POST['plazausuario']>0){
		echo '<td><a href="#" onclick="atcr(\'nomina_fiscal.php\',\'_blank\',\'51\',\'\');"><img src="images/b_print.png" border="0"></a>&nbsp;&nbsp;Reporte de Nomina&nbsp;&nbsp;</td>';
		if(nivelUsuario()>1){
	...
	while($row=mysql_fetch_array($res)){
		$seleccionado = '';
		if($_POST['periodo_nomina']==$row['cve']) $seleccionado = ' selected';
		if($_POST['plazausuario']>0){
			$res1=mysql_query("SELECT generada_fiscal FROM periodos_nomina_plaza WHERE plaza='".$_POST['plazausuario']."' AND periodo='".$row['cve']."'");
			$row1=mysql_fetch_array($res1);
			$row['generada_fiscal']=$row1['generada_fiscal'];
		}
		//if(mysql_num_rows($res1)>0)
		if($row['generada_fiscal']==1){
			echo '<option value="'.$row['cve'].'"'.$seleccionado;
			if($_POST['plazausuario']>0)
				$res1=mysql_query("SELECT * FROM personal_nomina a INNER JOIN personal b ON b.cve = a.personal WHERE a.periodo=".$row['cve']." AND b.plaza='".$_POST['plazausuario']."' AND a.tipo=1 AND a.uuid='' AND a.eliminada!=1");
			else
			...
			echo '<option value="'.$row['cve'].'"'.$seleccionado.'>'.$row['nombre'].' del '.$row['fecha_ini'].' al '.$row['fecha_fin'].'(Sin generar)</option>';
	}
	echo '</select></td></tr>';
	if($_POST['plazausuario']>0){
