...
if($_POST['ajax']==1){
	...
	if($_POST['anio'] >= 4){
		...
		$res = mysql_query("SELECT SUM(foliofin+1-folioini),MAX(fecha_compra),MIN(fecha_compra),WEEK(fecha_compra) FROM 
			compra_certificados WHERE plaza='".$_POST['plazausuario']."' AND engomado=2 AND anio='".$_POST['anio']."' AND estatus!='C' 
		...
		$res = mysql_query("SELECT COUNT(cve),MAX(fecha),MIN(fecha),WEEK(fecha) FROM 
			certificados WHERE plaza='".$_POST['plazausuario']."' AND engomado=2 AND anio='".$_POST['anio']."' AND estatus!='C' 
			GROUP BY WEEK(fecha)");
		$semanamax2=-1;
		$semanamin2=-1;
		$maximo2=-1;
		...
		while($row = mysql_fetch_array($res)){
			...
			$res = mysql_query("SELECT a.engomado,COUNT(b.cve) as compras,a.cve,WEEK(a.fecha_compra) FROM compra_certificados a INNER JOIN compra_certificados_detalle b ON a.plaza = b.plaza AND a.cve = b.cvecompra AND b.tipo!=1 WHERE a.plaza='".$_POST['plazausuario']."' AND a.anio='".$_POST['anio']."' AND a.estatus!='C' GROUP BY a.cve") or die(mysql_error());
			$array_compras = array();
			while($row = mysql_fetch_array($res)){
			...
			$res = mysql_query("SELECT engomado,COUNT(cve),WEEK(fecha) FROM certificados WHERE plaza='".$_POST['plazausuario']."' AND anio='".$_POST['anio']."' AND estatus!='C' GROUP BY engomado,WEEK(fecha)") or die(mysql_error());
			while($row = mysql_fetch_array($res)){
