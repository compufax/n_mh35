...
if($_POST['cmd']==100){
	...
	echo '<th>Aforo</th><th>Rechazos</th><th>Pagos Efectivo</th><th>Pagos Documento</th><th>Total Pagos</th><th>Recuperacion</th><th>Venta</th><th>Total Venta</th><th>Devoluciones</th><th>Devolucion Ajuste</th><th>Total</th><th>Cancelados</th></tr>'; 
	...
	foreach($array_plazas as $k=>$v){
		...
		//	if($_POST['plaza']=='all' || $k==$_POST['plaza']){
				...
				echo '<td align="right">'.number_format($row3[0],2).'</td>';
				echo '<td align="right">'.number_format($row3[1],2).'</td>';
				echo '<td align="right">'.number_format($row3[0]+$row3[1],2).'</td>';
				...
				echo '<td align="right">'.number_format($row4[0],2).'</td>';
				...
				echo '<td align="right">'.number_format($row5[0],2).'</td>';
				echo '<td align="right">'.number_format($total_venta-$row1[1]-$row5[0],2).'</td>';
				echo '<td align="right">'.number_format($row['cancelados'],0).'</td>';
				echo '</tr>';
				$x++;
				$sumacargo[0]+=$row2[0];
				$sumacargo[1]+=$row2[1];
				$sumacargo[2]+=$row3[0];
				$sumacargo[3]+=$row3[1];
				$sumacargo[4]+=$row3[0]+$row3[1];
				$sumacargo[5]+=$row4[0];
				$sumacargo[6]+=$row[1];
				$sumacargo[7]+=$total_venta;
				$sumacargo[8]+=$row1[1];
				$sumacargo[9]+=$row5[0];
				$sumacargo[10]+=$total_venta-$row1[1]-$row5[0];
				$sumacargo[11]+=$row['cancelados'];
		//	}
		//}
	}
	$c=4;
