...
if($_POST['ajax']==2){
	...
	if($_POST['proveedor'] > 0){
		$res = mysql_query("SELECT cve, nombre FROM usuarios WHERE proveedor_mantenimiento = '".$_POST['proveedor']."' AND estatus!='I' ORDER BY nombre");
		while($row = mysql_fetch_array($res))
...
if ($_POST['cmd']==2) {
	...
	if($_POST['reg']) {
		...
		mysql_query("INSERT reporte_lineas SET estatus='A',fecha=CURDATE(), hora=CURTIME(), plaza='".$_POST['plazausuario']."',linea='".$_POST['linea']."',
		...
		usuario='".$_POST['cveusuario']."',proveedor='".$_POST['proveedor']."',nomenclatura='".$_POST['nomenclatura']."'");
			...
			$insert = " INSERT INTO mantenimiento_reportes 
						...
						VALUES 
						('".$idd."','".$_POST['plazausuario']."','".$_POST['proveedor']."','".$_POST['no_reporte']."','".$_POST['tecnico']."','".$_POST['operaciones']."','".$_POST['obs']."',
...
if($_POST['cmd']==1){
	...
	$res = mysql_query("SELECT a.* FROM cat_lineas a WHERE a.plaza='".$_POST['plazausuario']."' ORDER BY a.nombre");
	while($row=mysql_fetch_array($res)){
		echo '<option value="'.$row['cve'].'">'.$row['nombre'].'</option>';
	}
	echo '</select></td></tr>';
	echo '<tr><th align="left">Tipo</th><td align="center"><select name="tipo_'.$row['cve'].'" id="tipo_'.$row['cve'].'" onChange="cambiatipo('.$row['cve'].')"><option value="0">Seleccione</option>';
	...
	foreach ($array_materiales as $k=>$v) { 
		echo '<tr><th align="left">'.$v.'</th><td><input type="text" name="cve_'.$k.'" id="cve_'.$k.'" class="textField" size="" placeholder="Cantidad" value=""></td></tr>';
	}
	...
	echo '<script>
		...
		function guardarLinea(linea){
			...
			else if($("#tipo").val() == "2" && $("#fechaprogramacion").val() == ""){
				alert("Necesita seleccionar la fecha de programacion");
			}
			else if($("#proveedor_"+linea).val() == "0"){
				alert("Necesita seleccionar el proveedor");
			}
			else if($("#nomenclatura_"+linea).val() == "0"){
				alert("Necesita seleccionar el motivo");
			}
			else{
				atcr("reporte_lineas.php","",2,linea);
			}
		}
