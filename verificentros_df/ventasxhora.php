...
header("Expires: 0");
	...
	$html= '<h2 align="center" >Informe de Ventas </h2><h2 style="font-size:15px">Periodo: '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'</h2></br>
	...
	$html.= '<tr bgcolo="#E9F2F8">
			...
			while($fecha<=$_POST['fecha_fin']){
				$html.='<th colspan="13">'.$fecha.'</th>';

				$fecha=date( "Y-m-d" , strtotime ( "+1 day" , strtotime($fecha) ) );
			}
			$html.='<th rowspan="2">Total</th></tr><tr bgolor="#E9F2F8">';
			 $fecha=$_POST['fecha_ini'];
			while($fecha<=$_POST['fecha_fin']){
			//	$html.='<th colspan="13">'.$fecha.'</th>';
			  //  $html.='</tr>';
...
//		rowb();
		$html.='<tr>';
		$totl=0;
	$html.= '<td align="left">'.$v.'</td>';

		$fecha=$_POST['fecha_ini'];
			while($fecha<=$_POST['fecha_fin']){
			
			    foreach ($array_horas as $kk => $vv) {
			    	if($kk<14){
			$select5= "SELECT count(a.cve) as totales,b.cve as certificado, b.certificado as holograma,b.engomado as engomado_entrega, CONCAT(b.fecha,' ',b.hora) as fechaentrega, TIMEDIFF(IFNULL(CONCAT(b.fecha,' ',b.hora),NOW()),CONCAT(a.fecha,' ',a.hora)) as diferencia FROM cobro_engomado a LEFT JOIN certificados b ON a.plaza=b.plaza AND a.cve=b.ticket AND b.estatus!='C' LEFT JOIN depositantes c ON c.cve = a.depositante AND c.plaza = a.plaza WHERE a.plaza='".$k."' AND a.fecha ='".$fecha."' AND a.estatus='A' and a.hora between '".$vv."' and '".$array_horas[($kk+1)]."' ORDER BY a.fecha DESC, a.cve DESC";
				$res5=mysql_query($select5) or die(mysql_error());
				$roww5=mysql_fetch_array($res5);
				$html.= '<td align="center">'.$roww5['totales'].'</td>';
				$totl+=$roww5['totales'];
				//$array_total[$x]+=$roww5['totales'];
...
//	$html.= '<td bgcolor="#E9F2F8" align="right" colspan="">&nbsp;Total</td>';
			foreach ($array_total as $k => $v) {
				$html.='<th bgcolo="#E9F2F8" align="center">'.$v.'</th>';
			}
...
if($_POST['ajax']==1){ 
	...
	$html.= '<tr bgcolor="#E9F2F8">
			 <th rowspan="" width="20">Horario</th>';

			 $fecha=$_POST['fecha_ini'];
			while($fecha<=$_POST['fecha_fin']){
				$html.='<th colspan="">'.$fecha.'</th>';
	...
	foreach ($array_horas as $k => $v) {
		   	if($k<14){

		   		rowb();
		   		$rest = substr($v, -8, 5);
		   		$html.= '<td align="center">'.$rest.'</td>';
		   		$fecha=$_POST['fecha_ini'];
			while($fecha<=$_POST['fecha_fin']){
				$select5= "SELECT count(a.cve) as totales,b.cve as certificado, b.certificado as holograma,b.engomado as engomado_entrega, CONCAT(b.fecha,' ',b.hora) as fechaentrega, TIMEDIFF(IFNULL(CONCAT(b.fecha,' ',b.hora),NOW()),CONCAT(a.fecha,' ',a.hora)) as diferencia FROM cobro_engomado a LEFT JOIN certificados b ON a.plaza=b.plaza AND a.cve=b.ticket AND b.estatus!='C' LEFT JOIN depositantes c ON c.cve = a.depositante AND c.plaza = a.plaza WHERE a.plaza='".$_POST['plazausuario']."' AND a.fecha ='".$fecha."' AND a.estatus='A' and a.hora between '".$v."' and '".$array_horas[($k+1)]."' ORDER BY a.fecha DESC, a.cve DESC";
				$res5=mysql_query($select5) or die(mysql_error());
				$roww5=mysql_fetch_array($res5);
				$html.= '<td align="center">'.$roww5['totales'].'</td>';
				$totl+=$roww5['totales'];
...
//	$html.= '<td align="right">Totale</td><td align="center">'.$totl.'</td>';
	$html.= '</tr>';
		$t1=$t1+$roww1['t_vendidos'];
		$t2=$t2+$roww2['t_importe'];
		$t3=$t3+$roww3['t_intento'];
...
//	$html.= '<td bgcolor="#E9F2F8" align="right" colspan="">&nbsp;Total</td>';
			foreach ($array_total as $k => $v) {
				$html.='<th bgcolor="#E9F2F8" align="center">'.$v.'</th>';
			}
...
top($_SESSION);
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr>
				...
				<!--<a href="#" onClick="atcr(\'\',\'_blank\',\'103\',\'0\');"><img src="images/b_print.png" border="0" title="Imprimir"></a>&nbsp;Excell--></td>
				<!--<td><a href="#" onclick="atcr(\'#ventasxhora.php\',\'_blank\',100,0);"><img src="images/b_print.png" border="0">&nbsp;&nbsp;Imprimir</a>&nbsp;&nbsp;</td>
				<td><a href="#" onclick="atcr(\'#ventasxhora.php\',\'_blank\',101,0);"><img src="images/b_print.png" border="0">&nbsp;&nbsp;Imprimir</a>&nbsp;&nbsp;</td>
				<td><a href="#" onclick="atcr(\'#ventasxhora.php\',\'_blank\',102,0);"><img src="images/b_print.png" border="0">&nbsp;&nbsp;Imprimir</a>&nbsp;&nbsp;</td>-->';
		echo '</tr>';
		echo '</table>';
		echo '<table>';
...
<Script language="javascript">
	function buscarRegistros(){
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","#ventasxhora.php",true);
