...
$html.= '<table width="100%" border="0" cellpadding="4" cellspacing="1"  id="tabla1" >';
	$html.= '<tr style="font-size:22px"><td align="center">'.$array_plaza[$_POST['plazausuario']].' - '.$PlazaLocal_nombre.'</td></tr><tr>
			<td align="center" style="font-size:16px">Desglose de Ventas Periodo: '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'</td>
		 </tr>
		 ';
	$html.= '</table>';
	$html.= '<br>';
...
$select= " SELECT sum(if(a.tipo_venta=0 AND a.tipo_pago=1 AND a.estatus='A' AND a.engomado!=23,1,0)) as efectivos,
				  ...
				  sum(if(a.tipo_venta=0 AND a.tipo_pago=1 AND a.estatus='A' AND a.engomado!=23,a.monto,0)) as efectivos_monto,
				  ...
				  sum(if(a.tipo_pago not in (5,7,2,6,12) and (a.estatus in('A','B','C')),a.copias,0)) as efectivos_copiasssss,
				  sum(if(a.tipo_pago in (5,7),a.copias,0)) as bancos_copias,
		...
		$_select1= " SELECT  sum(if(a.forma_pago=1,1,0)) as pagos_efectivos,
							sum(if(a.forma_pago=1,a.monto,0)) as pagos_efectivos_monto,
							sum(if(a.forma_pago in (2,3,4,5),1,0)) as pagos_bancos,
							sum(if(a.forma_pago in (2,3,4,5),a.monto,0)) as pagos_bancos_monto
		FROM pagos_caja a
...
//		$html.= $select;
			...
			$row=mysql_fetch_array($res);
				//rowb();
				$html.= '<tr>
						 <td>Exentos Verifcados</td>
						 <td align="center">'.htmlentities($row['exentos']).'</td>
						 <td align="right">'.number_format($row['exentos_monto'],2).'</td>
						 </tr>';
				$html.= '<tr>
						 <td>Vales de pago anticipado Utilizados o canje de pagos anticipados</td>
						 <td align="center">'.htmlentities($row['vales_usados']).'</td>
						 <td align="right">'.number_format(0,2).'</td>
						 </tr>';		 
				$html.= '<tr>
				...
				$cant=$row['exentos']+$row['vales_usados']+$row['intentos']+$row['cortesias'];
				...
				$html.='<tr>';
				$html.= '<td>Creditos Verificados</td>
						 <td align="center">'.htmlentities($row['credito']).'</td>
						 <td align="right">'.number_format($row['credito_monto'],2).'</td>
						 </tr>';
				$html.='<tr>';
				$html.= '<td>Efectivo</td>
						 <td align="center">'.htmlentities($row['efectivos']).'</td>
		...
		//				</tr>';
				...
				$row1=mysql_fetch_array($res1);
				$html.='<tr><td>Pagos Anteriores</td><td align="center">'.htmlentities($row['pagos_anteriores']).'</td><td align="right"></td></tr>';
				$html.='<tr><td>Pagados No verificados</td><td align="center">'.htmlentities($row['no_verificado']).'</td><td align="right"></td></tr>';
			...
			//		$html.= $select1;
				...
				$html.= '<tr>
						 <td align="left" colspan="3">DESGLOSE DE CERTIFICADOS DE ENTREGA (utilizados)</td>
						 </tr>';
						 $cant=0;
				foreach($array_engomado as $k=>$v){
...
//				$html.='<td align="right">&nbsp;'.$re.'</td></tr>';
				...
				$html.= '<tr>
						<th align="left" colspan="3">DESGLOSE DE CERTIFICADOS CANCELADOS</th>
						</tr>';
						$cant=0;
				foreach($array_engomado as $k=>$v){
				$select2= " SELECT count(cve) as cancelados FROM certificados_cancelados WHERE plaza='".$_POST['plazausuario']."' AND engomado IN (".$k.")";
...
header("Content-Disposition: attachment; filename=Amarre de Venta por Periodo.xls");
...
$html.= '<table width="100%" border="0" cellpadding="4" cellspacing="1"  id="tabla1" >';
	$html.= '<tr style="font-size:22px"><td align="center">'.$array_plaza[$_POST['plazausuario']].' - '.$PlazaLocal_nombre.'</td></tr><tr>
			<td align="center" style="font-size:16px">Amarre de Venta por Periodo: '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'</td>
		 </tr>
		 ';
	$html.= '</table>';
	$html.= '<br>';
...
//		$html.= $select;
			...
			$html.= '<table width="100%" border="0" cellpadding="4" cellspacing="1" class="" style="font-size:8.7px">';
			$html.= '<tr gcolor="#E9F2F8"><th width="100">Tipo</th><th>Cantidad</th><th>Importe</th>';
			$html.= '</tr>';
			$t=0;
			$total1;
			$row=mysql_fetch_array($res);
...
// //								WHERE plaza='".$_POST['plazausuario']."' and fecha_compra BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."'";
			...
			// $html.= '	
				// <tr>
				// <td colspan="2" bgcolor="#E9F2F8">';;$html.= '</td>
...
//		echo $select;
	...
	// //			echo '<td>Exentos</td><td align="center">'.htmlentities($row['exentos']).'</td><td align="right">'.number_format($row['exentos_monto'],2).'</td></tr>';
				...
				echo '<td>Creditos Verificados</td><td align="center">'.htmlentities($row['credito']).'</td><td align="right">'.number_format($row['credito_monto'],2).'</td></tr>';
...
//				echo '<tr><td>Recuperacion del Mes</td><td align="center">'.number_format($row7['recuperados'],0).'</td><td align="right">'.number_format($row7['recuperados_monto'],2).'</td></tr>';
			...
			//	echo '<tr><th align="left" colspan="3">DESGLOSE DE CERTIFICADOS DE ENTREGA (utilizados)</th></tr>';
				// $cant=0;
				// foreach($array_engomado as $k=>$v){
...
// //								WHERE plaza='".$_POST['plazausuario']."' and fecha_compra BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."'";
			...
			// while($row1 = mysql_fetch_assoc($res1)){
				...
				// $row2 = mysql_fetch_assoc($res2);
				// $existencia -= $row2['registros'];
				// echo '<tr><td>'.$row1['nombre'].'</td><td align="center">'.number_format($existencia,0).'</td></tr>';
				// $cant += $existencia;
			// }
			// echo '<tr><td style="border-top-color:#000099; border-top-style:solid; border-top-width:1px;" align="right">TOTAL</td>
				          // <td style="border-top-color:#000099; border-top-style:solid; border-top-width:1px;" align="center">'.number_format($cant,0).'</td>
...
//	echo '<span style="cursor:pointer" onClick="atcr(\'amarre_ventaxperiodo.php\',\'\',300,0)"><font size="20px" color="BLUE">Descargar Archivo de Impresion</font></span>';
...
<Script language="javascript">
	...
	function buscarRegistros(btn)
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","amarre_ventaxperiodo.php",true);
...
if($cvecobro>0){
		echo '<script>atcr(\'amarre_ventaxperiodo.php?nuevo=1\',\'_blank\',\'101\','.$cvecobro.');</script>';
	}
?>
