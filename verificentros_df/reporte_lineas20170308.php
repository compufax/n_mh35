...
$array_tipo_cargo=Array(1=>"Con Cargo",2=>"Garantia",3=>"Poliza");
	echo '<table><input type="hidden" name="camposi[linea]" id="camposi[linea]" class="textField" size="" value="'.$_POST['linea'].'">
				 <input type="hidden" name="camposi[plaza]" id="camposi[plaza]" class="textField" size="" value="'.$_POST['plaza'].'">';
		echo '<tr><th align="left">Empresa</th><td><select name="camposi[empresa]" id="camposi[empresa]" class="textField"><option value="">---Seleccione---</option>';
			foreach ($array_empresas as $k=>$v) { 
	    echo '<option value="'.$k.'"';if($row['empresa']==$k){echo'selected';}echo'>'.$v.'</option>';
		}
		echo '</select></td></tr>';
		echo '<tr><th align="left">Reporte</th><td><input type="text" name="camposi[no_reporte]" id="camposi[no_reporte]" class="textField" size="" value="'.$row['no_reporte'].'"></td></tr>';
		echo '<tr><th align="left">Tecnico</th><td><select name="camposi[tecnico]" id="camposi[tecnico]" class="textField"><option value="">---Seleccione---</option>';
			foreach ($array_tecnico as $k=>$v) { 
	    echo '<option value="'.$k.'"';if($row['tecnico']==$k){echo'selected';}echo'>'.$v.'</option>';
		}
		echo '</select></td></tr>';
		echo '<tr><th align="left">Motivo</th><td><select name="camposi[nomenclatura]" id="camposi[nomenclatura]"><option value="">Todos</option>';
	foreach($array_nomenclaturas as $k=>$v){
		echo '<option value="'.$k.'">'.utf8_encode($v).'</option>';
	}
	echo '</select></td></tr>';
		echo '<tr><th align="left">Tipo Cargo</th><td><select name="camposi[tipo_cargo]" id="camposi[tipo_cargo]" onChange="cargo()"><option value="">---Seleccione---</option>';
			foreach ($array_tipo_cargo as $k=>$v) { 
	    echo '<option value="'.$k.'"';if($row['tipo_cargo']==$k){echo'selected';}echo'>'.$v.'</option>';
		}
		echo '</select></td></tr>';
		echo '<tr class="monto_" style="display:none;"><th align="left">Monto</th><td><input type="text" name="camposi[monto]" id="camposi[monto]" class="textField" size="" value="'.number_format($row['monto'],2).'"></td></tr>';
		...
		foreach ($array_materiales as $k=>$v) { 
			...
			echo '<tr><th align="left">'.$v.'</th><td><input type="text" name="camposi[cve_'.$k.']" id="camposi[cve_'.$k.']" class="textField" size="" placeholder="Cantidad" value="'.$row1['cantidad'].'"></td></tr>';
			}

		
		
		echo '<tr><th align="left">Operaciones</th><td><textarea cols="50" rows="5" name="camposi[operaciones]" id="camposi[operaciones]">'.$row['operaciones'].'</textarea></td></tr>';
		echo '<tr><th align="left">Observaciones</th><td><textarea cols="50" rows="5" name="camposi[obs]" id="camposi[obs]">'.$row['obs'].'</textarea></td></tr>';
...
if ($_POST['ajax']==4) {
	...
	if($_POST['reg']) {
		...
		mysql_query("INSERT reporte_lineas SET estatus='A',fecha=CURDATE(), hora=CURTIME(), plaza='".$array_campo['plaza']."',linea='".$array_campo['linea']."',
		tipo='3',observacion='".$_POST['obs_'.$_POST['reg']]."',fechaprogramacion='".$_POST['fechaprogramacion_'.$_POST['reg']]."',
		usuario='".$_POST['cveusuario']."',proveedor='".$_POST['proveedor_'.$_POST['reg']]."',nomenclatura='".$array_campo['nomenclatura']."'");
			
			$idd=mysql_insert_id();
			//Insertar el Registro
			$insert = " INSERT INTO mantenimiento_reportes 
						(cve_aux,plaza,empresa,no_reporte,tecnico,operaciones,obs,tipo_cargo,monto,usuario,fecha,hora)
						VALUES 
						('".$idd."','".$array_campo['plaza']."','".$array_campo['empresa']."','".$array_campo['no_reporte']."','".$array_campo['tecnico']."','".$array_campo['operaciones']."','".$array_campo['obs']."',
						 '".$array_campo['tipo_cargo']."','".$array_campo['monto']."','".$_POST['cveusuario']."','".fechaLocal()."','".horaLocal()."')";
			$ejecutar = mysql_query($insert);
			$id=mysql_insert_id();
			foreach ($array_materiales as $k=>$v) { 
			
			if($array_campo['cve_'.$k.'']>0){
				$insert = " INSERT INTO mantenimiento_reportes_detalle_material 
						(cve_aux,cve_mat,cantidad)
						VALUES 
						('".$id."','".$k."','".$array_campo['cve_'.$k.'']."')";
			$ejecutar = mysql_query($insert);
			}
			}
			$arreglo["cve"] = "ok";
...
if($_POST['cmd']==1){
	...
	echo '<table><tr><th>Linea</th><th>Tipo</th><th>Observacion</th><th>Fecha de Programación</th><th>Proveedor</th><th>Motivo</th><th>Guardar<input type="hidden" name="plazausuario" id="plazausuario" value="'.$_POST['plazausuario'].'"></th></tr>';
	...
	echo '<script>
		...
		function guardarLinea(linea){
			if($("#tipo_"+linea).val() == "0"){
				...
				atcr("reporte_lineas.php","_blank",2,linea);
			}
		}
	function reporte_correctivo(linea){
					$.ajax({
					  ...
					  data: {
						...
						success: function(data) {
							$("#dialog").html(data);
							$("#dialog").dialog("open");
							$(".entero").bind("keypress", function(e) {
					...
					buttons: {
						"Guardar": function(){
							$.ajax({
							  ...
							  data: {
								ajax: 4,
								campos: JSON.stringify($("#dialog").serializeForm())
								},
								success: function(data) {
	...
	if(document.getElementById("camposi[tipo_cargo]").value==1 ){
		$(".monto_").show();
	}
	if(document.getElementById("camposi[tipo_cargo]").value==2 || document.getElementById("camposi[tipo_cargo]").value==3){
		$(".monto_").hide();
	}
	}
				</script>';
