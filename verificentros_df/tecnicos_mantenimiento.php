...
if($_POST['ajax']==1) {
		//Listado de plazas
		$select= " SELECT * FROM tecnicos_mantenimiento WHERE 1 ";
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			echo '<th>Nombre</th><th>Empresa</th>';
...
if ($_POST['cmd']==2) {
	...
	if($_POST['reg']) {
			...
			$update = " UPDATE tecnicos_mantenimiento 
						SET nombre='".$_POST['nombre']."',empresa='".$_POST['empresa']."'
						WHERE cve='".$_POST['reg']."' " ;
			$ejecutar = mysql_query($update);			
	} else {
			//Insertar el Registro
			$insert = " INSERT INTO tecnicos_mantenimiento 
						(nombre,empresa)
						VALUES 
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		$select=" SELECT * FROM tecnicos_mantenimiento WHERE cve='".$_POST['reg']."' ";
		$res=mysql_query($select);
		...
		echo '
			<tr>';
			if(nivelUsuario()>1)
				echo '<td><a href="#" onClick="$(\'#panel\').show(); atcr(\'tecnicos_mantenimiento.php\',\'\',\'2\',\''.$row['cve'].'\');"><img src="images/guardar.gif" border="0">&nbsp;Guardar</a></td><td>&nbsp;</td>';
			echo '<td><a href="#" onClick="$(\'#panel\').show();atcr(\'tecnicos_mantenimiento.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver</a></td><td>&nbsp;</td>
			</tr>';
		echo '</table>';
		echo '<br>';
		...
		echo '<tr><th>Empresa</th><td><select name="empresa" id="empresa" class="textField"><option value="">---Seleccione---</option>';
...
/*** PAGINA PRINCIPAL **************************************************/
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr>
				...
				<td><a href="#" onClick="atcr(\'tecnicos_mantenimiento.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;Nuevo</td><td>&nbsp;</td>
			 </tr>';
		echo '</table>';
		echo '<table>';
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","tecnicos_mantenimiento.php",true);
