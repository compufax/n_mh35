<?php
/*echo '<h2>Pagina en mantenimiento<br>Disculpe las molestias</h2>';
exit();*/
				...
				if ($oSoapClient->fault) {
					...
					if ($err){
						...
						if($respuesta['resultado']){
							...
							echo '<h2>Se genero el folio de factura '.$fserie." ".$ffolio.'  <a href="#" onClick="atcr(\'facturacion_web.php\',\'_blank\',\'200\',\''.$plaza.'|'.$cvefact.'|1\');"><img src="images/zip_grande.png" border="0" width="45px" height="45px" title="Descargar">Descarga aqui su factura</a></h2>';
						}
						else
							$strmsg=$respuesta['mensaje'];
						//print_r($respuesta);	
...
//if(date("H:i:s")>'19:30:00' || date('H:i:s')<'01:00:00' || date('Y-m-d') == '2015-10-29' || date('w') == 0){
...
/*	else
		echo '<script>$("#panel").show();
		alert("El horario de facturación es de 01:00 a 19:30 horas de lunes a sabado");
		</script>';
}*/
?>