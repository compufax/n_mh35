...
if($_POST['ajax']==1){
	...
	if(mysql_num_rows($res)>0) 
	{
		...
		<th>Usuario</th><th>Monto</th><th>Observaciones</th>';
		echo '</tr>';
		$t=0;
		$c=0;
		while($row=mysql_fetch_array($res)) {
			...
			if($row['estatus']=='C'){
				...
				//if(nivelUsuario()>1)
				if($_POST['cveusuario'] == 1 || ($nivelUsuario == 3 && $row['fecha']==date('Y-m-d')))
...
if ($_POST['cmd']==2) {
	if($_POST['plazausuario'] > 0){
		...
		if($saldocaja < $totaldesglose){
			echo '<script>alert("Sobrepaso el saldo en caja");</script>';
		}
		else{

			$res=mysql_query("SELECT IFNULL(MAX(folio)+1,1) FROM desglose_dinero WHERE plaza='".$_POST['plazausuario']."' AND fecha='".$_POST['fecha']."'");
			$row=mysql_fetch_array($res);
			$folio=$row[0];
			$insert = " INSERT desglose_dinero 
							...
							obs='".$_POST['obs']."',fecha_creacion='".fechaLocal()."'";
			mysql_query($insert);
...
if($_POST['cmd']==1){
	if ($_POST['cmd']==1) {
		...
		echo '<tr><td>Observaciones</td><td colspan="3"><textarea name="obs" id="obs" rows="3" cols="50"  class="textField"></textarea></td></tr>';
		echo '</table>';
