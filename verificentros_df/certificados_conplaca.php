...
if($_POST['ajax']==1) {
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			echo '<tr bgcolor="#E9F2F8"><th>&nbsp;</th><th><span style="color: #0000FF; cursor: pointer;" onClick="SortTable(1,\'C\',\'tabla1\')">Folio</span></th><th>Fecha</th><th>Ticket</th><th>Tipo de Venta</th><th>Tipo de Pago</th><th>Placa</th><th style="display:none;">Problemas con OBDII</th>
...
if($_POST['ajax']==2){
	...
	$res = mysql_query("SELECT * FROM cobro_engomado WHERE plaza='".$_POST['plazausuario']."' AND placa='".$_POST['placa']."' AND estatus!='C' ORDER BY cve DESC LIMIT 1");
	$row = mysql_fetch_array($res);
	$folio = $row['cve'];
	...
	if(mysql_num_rows($res)==0){
		...
		if($row=mysql_fetch_array($res)){
			...
			elseif($row['tipo_venta'] == 3){
				...
				$fecha = $row['fecha'];
				if($row['monto']==0){
					$res=mysql_query("SELECT cve,monto FROM cobro_engomado WHERE plaza='".$_POST['plazausuario']."' AND placa='".$row['placa']."' AND estatus!='D' AND estatus!='C' AND cve<'$folio' AND monto>0 ORDER BY cve DESC LIMIT 1");
					$row = mysql_fetch_array($res);
				}
				if($row['monto'] > 0)
				{
...
if($_POST['ajax']==9){
	...
	if(mysql_num_rows($res)==0){
		...
		if(mysql_num_rows($res)>0){
			...
			WHERE a.plaza='".$_POST['plazausuario']."' AND a.estatus!='C' AND b.folio='".intval($_POST['certificado'])."' ORDER BY b.cve DESC LIMIT 1");
			if($row = mysql_fetch_array($res)){
				...
				if($row=mysql_fetch_array($res)){
					...
					if(mysql_num_rows($res)>0 || $ValidarCertificados == 0){
						$insert = " INSERT certificados 
										...
										problema_obdii='".$_POST['problema_obdii']."'";
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		echo '<table>';
		echo '<tr><th align="left">Placa</th><td><input type="text" name="placa" id="placa" class="textField placas" style="font-size:20px" size="10" value="" onKeyUp="if(event.keyCode==13){ traeTicket(); document.forma.ticket.focus();} else{ this.value = this.value.toUpperCase();}">&nbsp;&nbsp;<font color="RED">Dar enter para traer el ticket de la placa</td></tr>';
		echo '<tr style="display:none;"><th align="left">Ticket</th><td><input type="text" name="ticket" id="ticket" class="readOnly enteros" style="font-size:20px" size="10" value="" onKeyUp="/*if(event.keyCode==13){ traeTicket();} else{ this.value = this.value.toUpperCase();}*/" readOnly>&nbsp;&nbsp;<!--<font color="RED">Dar enter para traer el engomado del ticket</font>--></td></tr>';
		...
		echo '<tr><th align="left">Problemas con OBDII</th><td><select name="problema_obdii" id="problema_obdii">';
		...
		echo '</select></td></tr>';*/
		echo '<tr><th align="left">Tecnico</th><td>
