...
if($_POST['ajax']==1) {
		//Listado de plazas
		$select= " SELECT * FROM tipo_plaza WHERE 1 ";
...
if ($_POST['cmd']==2) {
	...
	if($_POST['reg']) {
			...
			$update = " UPDATE tipo_plaza 
						SET nombre='".$_POST['nombre']."'
						WHERE cve='".$_POST['reg']."' " ;
			$ejecutar = mysql_query($update);			
	} else {
			//Insertar el Registro
			$insert = " INSERT INTO tipo_plaza 
						(nombre)
						VALUES 
						('".$_POST['nombre']."')";
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		$select=" SELECT * FROM tipo_plaza WHERE cve='".$_POST['reg']."' ";
		$res=mysql_query($select);
		...
		echo '
			<tr>';
			if(nivelUsuario()>1)
				echo '<td><a href="#" onClick="$(\'#panel\').show();atcr(\'tipo_plaza.php\',\'\',\'2\',\''.$row['cve'].'\');"><img src="images/guardar.gif" border="0">&nbsp;Guardar</a></td><td>&nbsp;</td>';
			echo '<td><a href="#" onClick="$(\'#panel\').show();atcr(\'tipo_plaza.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver</a></td><td>&nbsp;</td>
			</tr>';
		echo '</table>';
		echo '<br>';
		...
		//Formulario 
		echo '<table>';
		echo '<tr><td class="tableEnc">Edicion Tipo Plaza</td></tr>';
		echo '</table>';
...
/*** PAGINA PRINCIPAL **************************************************/
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr>
				...
				<td><a href="#" onClick="atcr(\'tipo_plaza.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;Nuevo</td><td>&nbsp;</td>
			 </tr>';
		echo '</table>';
		echo '<table>';
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","tipo_plaza.php",true);
