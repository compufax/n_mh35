...
$array_tipo_cortesia = array(1=>'Autorizada', 2=>'20x1');
...
$res=mysql_query("SELECT local,vende_seguros,intentoporcertificadodif,num_intentos,num_intentosanticipados,bloquear_impresion,maneja_medio_pago FROM plazas WHERE cve='".$_POST['plazausuario']."'");
...
while($row=mysql_fetch_array($res)){
	if($row['cve'] == 3 && $_POST['plazausuario']==3) $row['costo'] = 200;
...
/////////////////////////////////////////////////////////////////////////////////////////////////
		...
		//Listado de plazas
		$select= " SELECT a.*, f.cve as referencia ,b.cve as certificado, b.certificado as holograma,b.engomado as engomado_entrega, CONCAT(b.fecha,' ',b.hora) as fechaentrega, TIMEDIFF(IFNULL(CONCAT(b.fecha,' ',b.hora),NOW()),CONCAT(a.fecha,' ',a.hora)) as diferencia FROM cobro_engomado a LEFT JOIN certificados b ON a.plaza=b.plaza AND a.cve=b.ticket AND b.estatus!='C' 
		LEFT JOIN depositantes c ON c.cve = a.depositante AND c.plaza = a.plaza 
...
//			echo''.$select.'';
			...
			if($_POST['cveusuario']==1) echo '<th>Ticket</th>';
			echo '<th>Nota</th><th>Fecha</th><!--<th>Referencia Maquina Registradora--></th>';
...
if($_POST['ajax']==71){
	if($_POST['tipo_vale'] == 1){
		...
		$res = mysql_query("SELECT a.depositante FROM pagos_caja a INNER JOIN vales_pago_anticipado b ON a.plaza = b.plaza AND a.cve = b.pago WHERE a.plaza='".$_POST['plazausuario']."' AND a.estatus!='C' AND b.estatus!='C' AND b.cve= '".$_POST['vale_pago_anticipado']."'");
		if($row = mysql_fetch_array($res)){
...
if($_POST['cmd']==103.9){
	...
	$resPlaza = mysql_query("SELECT nombre FROM plazas WHERE cve='".$_POST['plazausuario']."'");
	$rowPlaza = mysql_fetch_array($resPlaza);
	echo '<h1>'.$array_plaza[$_POST['plazausuario']].'<br>'.$rowPlaza['nombre'].'<br>CORTE VENTA CERTIFICADO<br>'.fechaLocal().' '.horaLocal();
...
if($_POST['cmd']==191){
	...
	$resPlaza = mysql_query("SELECT nombre FROM plazas WHERE cve='".$_POST['plazausuario']."'");
	$rowPlaza = mysql_fetch_array($resPlaza);
	echo '<h1>'.$array_plaza[$_POST['plazausuario']].'<br>'.$rowPlaza['nombre'].'<br>CORTE INTENTO CON IMPORTE<br>'.fechaLocal().' '.horaLocal();
	if($_POST['fecha_ini']==$_POST['fecha_fin']) echo "<br>FECHA: ".$_POST['fecha_ini'];
	else echo "<br>FECHA INICIO: ".$_POST['fecha_ini']."<br>FECHA FIN: ".$_POST['fecha_fin'];
	if ($_POST['usuario']!=""){ 
		$filtro.=" AND a.usuario='".$_POST['usuario']."' "; 
...
if($_POST['cmd']==103){
	...
	$texto.='<br><br>';
	$res = mysql_query("SELECT COUNT(cve), SUM(IF(estatus='C',1,0)), SUM(IF(estatus='A',copias,0)) FROM cobro_engomado a WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' ".$filtro."");
	$row = mysql_fetch_array($res);
	...
	$res1=mysql_query("SELECT IF(tipo_venta=0,tipo_pago,IF(tipo_venta=1,-2,IF(tipo_venta=2,-1,-3))) as tipo_pago, SUM(IF(estatus!='C',monto,0)), COUNT(cve), SUM(copias)  FROM cobro_engomado a WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."'  ".$filtro." GROUP BY IF(tipo_venta=0,tipo_pago,IF(tipo_venta=1,-2,IF(tipo_venta=2,-1,-3)))") or die(mysql_error());
	$cantidad = 0;
	$total = 0;
	$copias=0;
	while($row1=mysql_fetch_array($res1)){
	...
	echo '<tr><td>TOTAL:</td><td align="right">'.$cantidad.'</td><td align="right">'.number_format($total,2).'</td></tr><tr><td></td></tr>';

	if($maneja_medio_pago==1){
		echo '<tr><td colspan="3"><h2>DETALLE DE COBRO DE MEDIOS PAGOS</td></tr>';
		$res1=mysql_query("SELECT tipo_pago, SUM(monto), COUNT(cve), SUM(copias)  FROM cobro_engomado a WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND tipo_venta=0 AND descuento50=1 AND estatus!='C' ".$filtro." GROUP BY tipo_venta");
		$cantidad = 0;
		$total = 0;
		while($row1=mysql_fetch_array($res1)){
			echo '<tr><td>'.$array_tipo_pago[$row1['tipo_pago']].'</td><td align="right">'.$row1[2].'</td><td align="right">'.number_format($row1[1],2).'</td></tr>';
	...
	echo '<tr><td colspan="3"><h2>DETALLE DE COBRO DE REPOSICIONES</td></tr>';
	$res1=mysql_query("SELECT tipo_pago, SUM(monto), COUNT(cve), SUM(copias)  FROM cobro_engomado a WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND tipo_venta=3 AND estatus!='C' ".$filtro." GROUP BY tipo_venta");
	$cantidad = 0;
	$total = 0;
	while($row1=mysql_fetch_array($res1)){
		echo '<tr><td>'.$array_tipo_pago[$row1['tipo_pago']].'</td><td align="right">'.$row1[2].'</td><td align="right">'.number_format($row1[1],2).'</td></tr>';
...
if($_POST['cmd']==101){
	...
	$resPlaza = mysql_query("SELECT nombre,'1' as bloqueada_sat FROM plazas WHERE cve='".$row['plaza']."'");
	$rowPlaza = mysql_fetch_array($resPlaza);
	...
	$texto.=chr(27).'!'.chr(8)." NOTA: ".sprintf("%05s", $row['referencia']);
	$texto.='|';
	...
	$texto.='|SI EL IMPORTE COBRADO ES DIFERENTE AL DEL TICKET FAVOR DE REPORTARLO|SI REQUIERE FACTURA, SOLICITELA EN ATENCION A CLIENTES|';
...
if($rowPlaza=mysql_fetch_array($resPlaza)){
	echo '<h1>Plaza: '.$rowPlaza['numero'].' '.$rowPlaza['nombre'].'</h1>';
}
if($_POST['cmd']==22){
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		if($_POST['reg']==0){
			...
			echo '><th align="left">50% de Descuento</th><td><input type="checkbox" name="descuento50" id="descuento50" value="1" onClick="descontar()"></td></tr>';
			echo '<tr';
		...
		echo '<script>
				...
				function descontar(){
					...
					if(document.forma.descuento50.checked)
						monto = document.forma.monto.value/2;
					else
...
//	}
	...
	echo '<br>';
	if($PlazaLocal==1)echo '<h1>La captura de la venta en esta plaza es de forma local</h1>';
	//echo '<h1>Las modificaciones a la operación del sistema se deberán enviar al correo: <font color="BLUE">contacto@hilosnegros.com.mx</font><!-- con el L.C. Arturo Galicia de la O--></h1>';
	//Listado
