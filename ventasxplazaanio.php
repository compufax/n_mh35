...
$rsUsuario=mysql_query("SELECT a.cve,a.numero,a.nombre FROM plazas a inner join datosempresas b on a.cve = b.plaza where a.estatus!='I' and b.localidad_id = 2 ORDER BY b.localidad_id, a.lista, a.numero");
while($Usuario=mysql_fetch_array($rsUsuario)){
...
if($_POST['cmd']==100){
	...
	echo '<th>Total 2014</th><th>Total 2015</th><th>Total Venta</th></tr>'; 
	...
	foreach($array_plazas as $k=>$v){
		...
		//	if($_POST['plaza']=='all' || $k==$_POST['plaza']){
				rowb();
				$select= " SELECT COUNT(a.cve),SUM(IF(a.estatus!='C' AND a.tipo_pago NOT IN (2,6),a.monto,0)),SUM(IF(a.estatus!='C' AND a.tipo_pago NOT IN (2,6) AND a.anio=1,a.monto,0))
				FROM cobro_engomado as a WHERE a.plaza='".$k."' AND a.fecha>='".$_POST['fecha_ini']."' AND a.fecha<='".$_POST['fecha_fin']."' ";
				$res=mysql_query($select) or die(mysql_error());
				$row=mysql_fetch_array($res);
				$res1=mysql_query("SELECT SUM(monto) FROM pagos_caja WHERE plaza='$k' AND fecha>='".$_POST['fecha_ini']."' AND fecha<='".$_POST['fecha_fin']."' AND estatus!='C'");
				$row1=mysql_fetch_array($res1);
				$row[1]+=$row1[0];
				$res1=mysql_query("SELECT SUM(recuperacion) FROM recuperacion_certificado WHERE plaza='$k' AND fecha>='".$_POST['fecha_ini']."' AND fecha<='".$_POST['fecha_fin']."' AND estatus!='C'");
				$row1=mysql_fetch_array($res1);
				$row[1]+=$row1[0];
				echo '<td>'.htmlentities(utf8_encode($v)).'</td>';
				echo '<td align="right">'.number_format($row[2],0).'</td>';
				echo '<td align="right">'.number_format($row[1]-$row[2],0).'</td>';
...
top($_SESSION);
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr>
				...
				echo 'atcr(\'ventasxplazaanio.php\',\'_blank\',100,0);"><img src="images/b_print.png" border="0">&nbsp;&nbsp;Imprimir</a>&nbsp;&nbsp;</td>';
		echo '</tr>';
		echo '</table>';
		echo '<table>';
		...
		/*if($_POST['plazausuario']>0){
			...
			echo '<tr style="display:none;"><td align="left">Localidad</td><td><select name="localidad" id="localidad"><option value="2">Puebla</option>';
			echo '</select>';
...
echo '  document.getElementById("Resultados").innerHTML = "<img src=\'images/ajaxtrabajando.gif\' border=\'0\' align=\'absmiddle\'> Espere un momento, buscando registros...";
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","ventasxplazaanio.php",true);
