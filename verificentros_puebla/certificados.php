		...
		if(mysql_num_rows($res)>0) 
		{
			...
			while($row=mysql_fetch_array($res)) {
				...
				if($row['estatus']=='C'){
					...
					if(nivelUsuario()>1 && $row1['factura']==0 && $PlazaLocal != 1){
...
if($_POST['ajax']==3){
	//echo '0';
	//exit();
	$res = mysql_query("SELECT cve FROM certificados_cancelados WHERE plaza='".$_POST['plazausuario']."' AND engomado='".$_POST['engomado']."' AND CAST(certificado AS UNSIGNED)='".intval($_POST['certificado'])."' AND estatus!='C'");
	if(mysql_num_rows($res)==0){
...
if($_POST['ajax']==2){
	...
	$res = mysql_query("SELECT cve, certificado FROM certificados WHERE plaza='".$_POST['plazausuario']."' AND ticket='".$folio."' AND estatus!='C'");
	if(mysql_num_rows($res)==0){
		$res = mysql_query("SELECT cve,fecha,placa,anio FROM cobro_engomado WHERE plaza='".$_POST['plazausuario']."' AND cve='".$folio."' ORDER BY cve DESC LIMIT 1");
		if($row=mysql_fetch_array($res)){
			if($row['estatus']=='C'){
				echo '-2|';
			}
			elseif($row['anio']==1){
				...
				$res1 = mysql_query("SELECT SUM(recuperacion) FROM recuperacion_certificado WHERE a.plaza='".$_POST['plazausuario']."' AND a.estatus!='C' AND a.ticket = '".$row['cve']."'");
				$row1 = mysql_fetch_array($res1);
				echo round($row['monto']+$row1[0],2).'|'.$placa;
			}
		}
		else{
			echo "0|";
	...
	if($_POST['engomado']>0){
		...
		if(mysql_num_rows($res)==0){
			...
			if(mysql_num_rows($res)==0){
				...
				if($row=mysql_fetch_array($res)){
					$res = mysql_query("SELECT b.estatus FROM compra_certificados a INNER JOIN compra_certificados_detalle b ON a.plaza = b.plaza AND a.cve = b.cvecompra WHERE a.plaza='".$_POST['plazausuario']."' AND a.engomado = '".$_POST['engomado']."' AND b.folio='".intval($_POST['certificado'])."' AND b.tipo=0");
					if(mysql_num_rows($res)>0 || $ValidarCertificados == 0){
						$insert = " INSERT certificados 
										...
										usuario='".$_POST['cveusuario']."',estatus='A',ticket='".$_POST['ticket']."',tecnico='".$_POST['tecnico']."',entregado='".$_POST['entregado']."'";
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		echo '
			<tr>';
			if(nivelUsuario()>1)
				echo '<td><a href="#" onClick="$(\'#dialog\').dialog(\'open\');"><img src="images/guardar.gif" border="0">&nbsp;Guardar</a></td><td>&nbsp;</td>';
		...
		echo '</table>';
		echo '<input type="hidden" name="entregado" id="entregado" value="">';
		...
		echo '<script>
				...
				function validarCertificado(){
					...
					if (objeto.readyState != 0) {
						...
						objeto.send("ajax=3&engomado="+document.getElementById("engomado").value+"&certificado="+document.getElementById("certificado").value+"&plazausuario="+document.getElementById("plazausuario").value);
...
<Script language="javascript">
	...
	function buscarRegistros(btn)
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","certificados.php",true);
			objeto.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
			objeto.send("ajax=1&btn="+btn+"&estatus="+document.getElementById("estatus").value+"&entregado="+document.getElementById("entregado").value+"&ticket="+document.getElementById("ticket").value+"&certificado="+document.getElementById("certificado").value+"&fecha_ini="+document.getElementById("fecha_ini").value+"&fecha_fin="+document.getElementById("fecha_fin").value+"&placa="+document.getElementById("placa").value+"&engomado="+document.getElementById("engomado").value+"&usuario="+document.getElementById("usuario").value+"&cveusuario="+document.getElementById("cveusuario").value+"&cvemenu="+document.getElementById("cvemenu").value+"&plazausuario="+document.getElementById("plazausuario").value+"&numeroPagina="+document.getElementById("numeroPagina").value);
			objeto.onreadystatechange = function()
			{
				if (objeto.readyState==4)
				{
