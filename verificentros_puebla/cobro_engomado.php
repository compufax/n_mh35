...
if($_POST['ajax']==1) {
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			<th>Tiene Multa</th><th>Tipo de Certificado</th><th>Monto</th><th>A&ntilde;o Certificacion</th><th>Tipo de Pago</th><th>Tipo Combustible</th><!--<th>Documento</th>--><th>Factura</th><th>Entrega Certificado</th><th>Holograma Entregado</th><th>Usuario</th>';
			echo '</tr>';
			$t=0;
			while($row=mysql_fetch_array($res)) {
				rowb();
				...
				if($row['estatus']=='C'){
					...
					if(nivelUsuario()>1 && $PlazaLocal != 1 && $row['factura']==0 && $row['certificado']=="" && ($row['fecha']==fechaLocal() || $_POST['cveusuario']==1))
			...
			echo '	
				...
				<td colspan="7" bgcolor="#E9F2F8">&nbsp;</td>
				</tr>
			</table>';
...
if($_POST['cmd']==103){
	...
	$res=mysql_query("SELECT engomado,SUM(IF(tipo_pago!=6,monto,0)),COUNT(cve),SUM(IF(tipo_pago IN (2),monto,0)),SUM(IF(tipo_pago IN (2),1,0)) FROM cobro_engomado WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND estatus!='C' GROUP BY engomado");
	while($row=mysql_fetch_array($res)){
		echo '<h2>'.$array_engomado[$row['engomado']].'</h2>';
		echo '<table><tr><th>Tipo Pago</th><th>Cantidad</th><th>Importe</th></tr>';
		$res1=mysql_query("SELECT tipo_pago,SUM(monto),COUNT(cve)  FROM cobro_engomado WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND engomado='".$row['engomado']."' AND estatus!='C' GROUP BY tipo_pago");
	...
	echo '<table><tr><th>Tipo Pago</th><th>Cantidad</th><th>Importe</th></tr>';
	$t31=$t32=0;
	$res3=mysql_query("SELECT tipo_pago,SUM(monto),COUNT(cve)  FROM pagos_caja WHERE plaza='".$_POST['plazausuario']."' AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND estatus!='C' $filtro GROUP BY tipo_pago");
	while($row3=mysql_fetch_array($res3)){
		echo '<tr><td>'.$array_tipo_pago[$row3['tipo_pago']].'</td><td>'.$row3[2].'</td><td>'.number_format($row3[1],2).'</td></tr>';
		$t31+=$row3[1];
		$t32+=$row3[2];
	}
...
if($_POST['cmd']==102){
	...
	$texto.=' GRAN TOTAL IMP: '.number_format($t2+$row2[0]-$row1[0],2).'|';
	$texto.=' TOTAL A DEPOSITAR IMP: '.number_format($t2-$t4+$row2[0]-$row1[0],2).'||';
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		if($_POST['reg']==0){
			...
			echo '<tr><th align="left">Año Certificado</th><td><select name="anio" id="anio" style="font-size:20px"><option value="0">Seleccione</option>';
