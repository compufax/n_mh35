...
$rsUsuario=mysql_query("SELECT a.cve,a.numero,a.nombre FROM plazas a inner join datosempresas b on a.cve = b.plaza where a.estatus!='I' and b.localidad_id ORDER BY a.numero");
	...
	if($_POST['mostrar']==1){
		...
		foreach($array_meses as $k=>$v){
			if($k>0){
				rowb();
				echo '<td>'.$v.'</td>';
				echo '<td align="right">'.number_format($array_montos[$k][1],0).'</td>';
				echo '<td align="right">'.number_format($array_montos[$k][0],2).'</td>';
				echo '<td align="right">'.number_format($array_montos[$k][2],2).'</td>';
				echo '<td align="right">'.number_format($array_montos[$k][0]-$array_montos[$k][2],2).'</td>';
				echo '</tr>';
