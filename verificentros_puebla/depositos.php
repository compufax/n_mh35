...
$array_depositante = array();
$res = mysql_query("SELECT * FROM depositantes ORDER BY nombre");
while($row = mysql_fetch_array($res)){
	$array_depositante[$row['cve']]=$row['nombre'];
}
...
if($_POST['cmd']==101){
	...
	include("../numlet.php");
	$res=mysql_db_query($base,"SELECT * FROM depositos WHERE cve='".$_POST['reg']."'");
	...
	$pdf->Ln();
	$pdf->Cell(95,10,'Depositos',0,0,'L');
	...
	$pdf->MultiCell(190,5,"Deposite la cantidad de ".numlet($row['monto']),0,"R");
	$pdf->Ln();
	...
	$pdf->Cell(60,5,'');
	$pdf->MultiCell(70,5,$array_depositante[$row['depositante']],0,'C');
	$pdf->Ln();
	$pdf->SetFont('Arial','',12);
	$pdf->Cell(190,5,"Deposite",0,0,'C');
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Ln();
	$pdf->Ln();
...
if($_POST['ajax']==1){
	...
	$select = "SELECT * FROM depositos WHERE plaza IN (".$plazas_ids.") AND fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."'";
	...
	if($_POST['depositante']!="all") $select .= " AND depositante='".$_POST['depositante']."'";
	...
	if(mysql_num_rows($res)>0) 
	{
		...
		<th>Cuenta</th><th>Referencia</th><th>Motivo</th><th>Depositante</th><th>Monto</th><th>Concepto</th><th>Usuario</th>';
		echo '</tr>';
		$t=0;
		while($row=mysql_fetch_array($res)) {
			rowb();
			...
			if($row['estatus']=='C'){
				...
				echo '<a href="#" onClick="atcr(\'depositos.php\',\'_blank\',\'101\','.$row['cve'].')"><img src="images/b_print.png" border="0" title="Imprimir '.$row['cve'].'"></a>';
				if(nivelUsuario()>1)
					echo '<a href="#" onClick="if(confirm(\'Esta seguro de cancelar el deposito\')) atcr(\'depositos.php\',\'\',\'3\','.$row['cve'].')"><img src="images/validono.gif" border="0" title="Cancelar '.$row['cve'].'"></a>';
			}	
			echo '</td>';
			...
			echo '<td align="left">'.htmlentities($array_motivo[$row['motivo']]).'</td>';
			echo '<td align="left">'.htmlentities($array_depositante[$row['depositante']]).'</td>';
...
if($_POST['ajax']==3){
	$res = mysql_query("SELECT cve,nombre FROM depositantes WHERE plaza='".$_POST['plaza']."'");
	if(mysql_num_rows($res)!=1) echo '<option value="0">Seleccione</option>';
...
if($_POST['cmd']==3){
	mysql_query("UPDATE depositos SET estatus='C',usucan='".$_POST['cveusuario']."',fechacan='".fechaLocal()." ".horaLocal()."' WHERE cve='".$_POST['reg']."'");
	$_POST['cmd']=0;
}
...
if ($_POST['cmd']==2) {
	$rsAbono=mysql_db_query($base,"SELECT IFNULL(MAX(folio)+1,1) FROM depositos WHERE plaza='".$_POST['plaza']."'");
	$Abono=mysql_fetch_array($rsAbono);
	$folio=$Abono[0];
	$insert = " INSERT depositos 
					SET 
					...
					depositante='".$_POST['depositante']."',monto='".$_POST['monto']."',concepto='".$_POST['concepto']."',
...
if ($_POST['cmd']==1) {
	...
	$select=" SELECT * FROM depositos WHERE cve='".$_POST['reg']."' ";
	$res=mysql_query($select);
	$row=mysql_fetch_array($res);			
	if($_POST['reg']==0){
	...
	echo '
		...
		echo '<td><a href="#" onClick="$(\'#panel\').show();atcr(\'depositos.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver</a></td><td>&nbsp;</td>
		</tr>';
	echo '</table>';
	echo '<br>';
	...
	echo '</select></td></tr>';
	echo '<tr><th align="left">Depositante</th><td><select name="depositante" id="depositante">';
	if($_POST['plazausuario']==0 && $row['plaza']==0){
		echo '<option value="0">Seleccione Plaza</option>';
	}
	else{
		if($_POST['plazausuario']>0)
			$res1 = mysql_query("SELECT cve,nombre FROM depositantes WHERE plaza='".$_POST['plazausuario']."'");
		else
			$res1 = mysql_query("SELECT cve,nombre FROM depositantes WHERE plaza='".$row['plaza']."'");
		if(mysql_num_rows($res1)!=1){
			echo '<option value="0">Seleccione</option>';
		}
		while($row1 = mysql_fetch_array($res1)){ 
			echo '<option value="'.$row1['cve'].'"';
			if($row1['cve']==$row['beneficiario']) echo ' selected';
	...
	echo '<script>
			function validar(){
				if(document.forma.plaza.value=="0"){
					...
					$("#panel").hide();
					alert("Necesita seleccionar un depositante");
				}
			...
			function traerCuentas(){
				if(document.forma.plaza.value=="0"){
					...
					$("#depositante").html(\'<option value="0">Seleccione Plaza</option>\');
				}
				else{
					$.ajax({
						url: "depositos.php",
			...
			function traerDepositantes(){
				$.ajax({
					url: "depositos.php",
					type: "POST",
					data: "ajax=3&plaza="+document.forma.plaza.value,
					success: function(resultado)
					{
						$("#depositante").html(resultado);
					}
				});
			}
...
if($_POST['cmd']==0){
	...
	echo '<tr>
			...
			<td><a href="#" onClick="atcr(\'depositos.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;Nuevo</td><td>&nbsp;</td>
		 </tr>';
	echo '</table>';
	echo '<table>';
	...
	if($_POST['plazausuario']>0){
		$res=mysql_query("SELECT cve,nombre FROM depositantes WHERE plaza='".$_POST['plazausuario']."' ORDER BY nombre");
		while($row=mysql_fetch_array($res)){
	...
	if($_POST['plazausuario']==0)
		$res=mysql_query("SELECT b.cve,b.usuario FROM depositos a INNER JOIN usuarios b ON a.usuario = b.cve WHERE 1 GROUP BY a.usuario ORDER BY b.usuario");
	else
		$res=mysql_query("SELECT b.cve,b.usuario FROM depositos a INNER JOIN usuarios b ON a.usuario = b.cve WHERE a.plaza='".$_POST['plazausuario']."' GROUP BY a.usuario ORDER BY b.usuario");
	while($row=mysql_fetch_array($res)){
...
<Script language="javascript">
	...
	function traerCuentas(){
		if(document.forma.plaza.value=="0"){
			...
			$("#depositante").html(\'<option value="all">Todos</option>\');
		}
		else{
			$.ajax({
				url: "depositos.php",
