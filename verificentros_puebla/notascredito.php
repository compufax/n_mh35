...
if($_POST['cmd']==101){
	generaNotaCreditoPdf($_POST['plazausuario'],$_POST['reg'],1);
	exit();
}
...
if($_POST['ajax']==1){
	$filtro="";
	$select= " SELECT a.* FROM notascredito as a WHERE a.plaza='".$_POST['plazausuario']."' AND a.fecha>='".$_POST['fecha_ini']."' AND a.fecha<='".$_POST['fecha_fin']."' ";
	...
	if(mysql_num_rows($rsabonos)>0) {
		...
		while ($Abono=mysql_fetch_array($rsabonos)){	
			...
			if($Abono['estatus']=='C'){
				...
				if(file_exists('cfdi/comprobantes/notacreditoc_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf')){
					echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'cfdi/comprobantes/notacreditoc_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf\',\'_blank\',\'0\',\''.$Abono['cve'].'\');"><img src="images/b_print.png" border="0" title="Imprimir '.$Abono['folio'].'"></a>';
					if($_POST['cveusuario']==1){
						echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'notascredito.php\',\'_blank\',\'103\',\'cfdi/comprobantes/notacreditoc_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf\');"><img src="images/basura.gif" border="0" title="Borrar PDF '.$Abono['folio'].'"></a>';
					}
				}
				else{
					echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'notascredito.php\',\'_blank\',\'101\',\''.$Abono['cve'].'\');"><img src="images/b_print.png" border="0" title="Imprimir '.$Abono['folio'].'"></a>';
				}
				echo '</td>';
				if($_POST['cveusuario']==1){
					echo '<td>&nbsp;</td>';
			...
			elseif($Abono['respuesta1']==""){
				...
				echo '<a href="#" onClick="if(confirm(\'Esta seguro que desea timbrar?\')){$(\'#panel\').show();atcr(\'notascredito.php\',\'\',\'5\',\''.$Abono['cve'].'\');}"><img src="images/validosi.gif" border="0" title="Timbrar '.$Abono['folio'].'"></a>';
				echo '&nbsp;&nbsp;<a href="#" onClick="if(confirm(\'Esta seguro de cancelar?\')){$(\'#panel\').show();atcr(\'notascredito.php\',\'\',\'3\',\''.$Abono['cve'].'\');}"><img src="images/validono.gif" border="0" title="Cancelar '.$Abono['folio'].'"></a>';
				echo '</td>';
				if($_POST['cveusuario']==1){
					echo '<td>&nbsp;</td>';
				}
				...
				if(file_exists('../cfdi/comprobantes/notacredito_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf')){
					echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'../cfdi/comprobantes/notacredito_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf\',\'_blank\',\'0\',\''.$Abono['cve'].'\');"><img src="images/b_print.png" border="0" title="Imprimir '.$Abono['folio'].'"></a>';
					if($_POST['cveusuario']==1){
						echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'notascredito.php\',\'_blank\',\'103\',\'cfdi/comprobantes/notacredito_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf\');"><img src="images/basura.gif" border="0" title="Borrar PDF '.$Abono['folio'].'"></a>';
					}
				}
				else{
					echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'notascredito.php\',\'_blank\',\'101\',\''.$Abono['cve'].'\');"><img src="images/b_print.png" border="0" title="Imprimir '.$Abono['folio'].'"></a>';
...
if($_POST['cmd']==6){
	...
	foreach($_POST['checksf'] as $cvefact){
		...
		$mail->Subject = "Nota de Credito ".$cvefact;
		$mail->Body = "Nota de Credito ".$cvefact;
		//$mail->AddAddress(trim($emailenvio));
		$correos = explode(",",trim($emailenvio));
		foreach($correos as $correo)
			$mail->AddAddress(trim($correo));
		$mail->AddAttachment("../cfdi/comprobantes/notacredito_".$_POST['plazausuario']."_".$cvefact.".pdf", "Nota de Credito ".$cvefact.".pdf");
		$mail->AddAttachment("../cfdi/comprobantes/cfdin_".$_POST['plazausuario']."_".$cvefact.".xml", "Nota de Credito ".$cvefact.".xml");
		$mail->Send();
		if($rowempresa['email']!=""){
			$mail = new PHPMailer();
			$mail->Host = "localhost";
		...
		if ($oSoapClient->fault) {
			...
			if ($err){
				...
				if($respuesta['resultado']){
					...
					generaNotaCreditoPdf($_POST['plazausuario'],$cvefact);
					...
					$filename='cfdin_'.$_POST['plazausuario'].'_'.$cvefact;
					...
					if ($err){
						...
						if($respuesta['resultado']){
							...
							if($emailenvio!=""){
								...
								$mail->From = "vereficentros@vereficentros.com";
								$mail->FromName = "Vereficentros";
								$mail->Subject = "Cancelacion Nota de Credito ".$cvefact;
								$mail->Body = "Cancelacion Nota de Credito ".$cvefact;
								//$mail->AddAddress(trim($emailenvio));
								$correos = explode(",",trim($emailenvio));
								foreach($correos as $correo)
									$mail->AddAddress(trim($correo));
								$mail->AddAttachment("../cfdi/comprobantes/notacreditoc_".$_POST['plazausuario']."_".$cvefact.".pdf", "Nota de Credito ".$cvefact.".pdf");
								$mail->AddAttachment("../cfdi/comprobantes/cfdin_".$_POST['plazausuario']."_".$cvefact.".xml", "Nota de Credito ".$cvefact.".xml");
								$mail->Send();
							}	
							if($rowempresa['email']!=""){
...
if($_POST['cmd']==2){
	require_once("../phpmailer/class.phpmailer.php");
	$res = mysql_query("SELECT folio_inicial FROM foliosiniciales WHERE plaza='".$_POST['plazausuario']."' AND tipo=0 AND tipodocumento=2");
	$row = mysql_fetch_array($res);
	$res1 = mysql_query("SELECT cve FROM notascredito WHERE plaza='".$_POST['plazausuario']."'");
	if(mysql_num_rows($res1) > 0){
		mysql_query("INSERT notascredito SET plaza='".$_POST['plazausuario']."',fecha='".$_POST['fecha']."',fecha_creacion='".fechaLocal()."',hora='".horaLocal()."',obs='".$_POST['obs']."',
		...
		tipopago_cliente='".$_POST['tipopago_cliente']."',banco_cliente='".$_POST['banco_cliente']."',cuenta_cliente='".$_POST['cuenta_cliente']."'") or die("INSERT notascredito SET plaza='".$_POST['plazausuario']."',fecha='".$_POST['fecha']."',fecha_creacion='".fechaLocal()."',hora='".horaLocal()."',obs='".$_POST['obs']."',
		...
		mysql_query("INSERT notascredito SET plaza='".$_POST['plazausuario']."',cve='".$row['folio_inicial']."',fecha='".$_POST['fecha']."',fecha_creacion='".fechaLocal()."',hora='".horaLocal()."',obs='".$_POST['obs']."',
		cliente='".$_POST['cliente']."',tipo_pago='".$_POST['tipo_pago']."',usuario='".$_POST['cveusuario']."',baniva_retenido='".$_POST['baniva_retenido']."',banisr_retenido='".$_POST['banisr_retenido']."',
		carta_porte='".$_POST['carta_porte']."',load_cliente='".$_POST['load']."',nombre_cliente='".$_POST['nombre_cliente']."',direccion_cliente='".$_POST['direccion_cliente']."',
		...
		if ($oSoapClient->fault) {
			...
			if ($err){
				...
				if($respuesta['resultado']){
					...
					sellotimbre='".$respuesta['sellotimbre']."',cadenaoriginal='".$respuesta['cadenaoriginal']."',
					fechatimbre='".substr($respuesta['fechatimbre'],0,10)." ".substr($respuesta['fechatimbre'],-8)."'
					WHERE empresa='".$_POST['plazausuario']."' AND cve=".$cvefact);
					//Tomar la informacion de Retorno
					$dir="../cfdi/comprobantes/";
					//$dir=dirname(realpath(getcwd()))."/solucionesfe_facturacion/cfdi/comprobantes/";
					//el zip siempre se deja fuera
...
top($_SESSION);
	...
	if($_POST['cmd']==1){
		...
		if(nivelUsuario()>1){
			...
			if(document.forma.cliente.value==\'0\'){
				...
				atcr(\'notascredito.php\',\'\',2,\'0\');
			}
			"><img src="images/guardar.gif" border="0">&nbsp;Guardar</a></td><td>&nbsp;</td>';
		}
		echo '<td><a href="#" onclick="$(\'#panel\').show();atcr(\'notascredito.php\',\'\',0,\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;&nbsp;Regresar</a></td>';
		echo '</tr></table>';
		echo '<br>';
		echo '<table>';
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr>
				...
				<td><a href="#" onClick="atcr(\'notascredito.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0">&nbsp;Nuevo</a></td><td>&nbsp;</td>';
		if($_POST['cveusuario']==1){
			echo '<td><a href="#" onClick="atcr(\'notascredito.php\',\'\',\'6\',\'0\');"><img src="images/nuevo.gif" border="0">&nbsp;Reenviar Archivos</a></td><td>&nbsp;</td>';
		}
		echo '</tr>';
		echo '</table>';
		echo '<table>';
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","notascredito.php",true);
