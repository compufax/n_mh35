...
if($_POST['cmd']==100) {
		...
		$select= " SELECT a.cve,a.estatus,b.cve,b.rfc,b.puesto,a.plaza,b.nombre,
		...
		echo '<h1>Asistencia del dia '.fecha_letra($_POST['fecha']).'</h1>';
		if(mysql_num_rows($rspersonal)>0) 
		{
			...
			echo '<th>Asistio</th><th>Hora Entrada</th><th>Hora Salida</th></tr></thead><tbody>';//<th>P.Costo</th><th>P.Venta</th>
			$total=0;
			$i=0;
			while($Personal=mysql_fetch_array($rspersonal)) {
				...
				echo '<td align="center">'.substr($Personal['entrada'],11,8).'</td>';
				if($Personal['entrada']==$Personal['salida']) echo '<td align="center">&nbsp;</td>';
				else echo '<td align="center">'.substr($Personal['salida'],11,8).'</td>';
				echo '</tr>';
			}
...
if($_POST['ajax']==1) {
		...
		if ($_POST['num']!="") { $select.=" AND b.cve='".$_POST['num']."'"; }
		if ($_POST['puesto']!="all") { $select.=" AND b.puesto='".$_POST['puesto']."'"; }
		if ($_POST['plaza']!="all") { $select.=" AND b.plaza='".$_POST['plaza']."'"; }
		$select.=" GROUP BY a.cve ORDER BY b.cve";
		$rspersonal=mysql_query($select);
		$totalRegistros = mysql_num_rows($rspersonal);
		echo '<h1>Asistencia del dia '.fecha_letra($_POST['fecha']).'</h1>';
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr>
				<td><a href="#" onclick="buscarRegistros();"><img src="images/buscar.gif" border="0"></a>&nbsp;&nbsp;Buscar&nbsp;&nbsp;
					<a href="#" onClick="atcr(\'asistencia_personal.php\',\'_blank\',\'100\',\'0\');"><img src="images/b_print.png" border="0"></a>&nbsp;Imprimir</td><td>&nbsp;</td>
			 </tr>';
		echo '</table>';
		echo '<table>';
