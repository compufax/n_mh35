...
if($_POST['ajax']==1){
	$filtro="";
	$select= " SELECT a.* FROM gastos_xml as a WHERE a.plaza='".$_POST['plazausuario']."' AND a.fecha>='".$_POST['fecha_ini']."' AND a.fecha<='".$_POST['fecha_fin']."' ";
	//if($_POST['tipo']!="all") $select.=" AND a.tipo='".$_POST['tipo']."'";
	if($_POST['cliente']!="") $select.=" AND (a.rfc='".$_POST['cliente']."' OR a.nombre LIKE '%".$_POST['cliente']."%')";
	...
	if(mysql_num_rows($rsabonos)>0) {
		...
		echo '<th>Folio</th><th>Fecha</th><th>RFC Receptor</th>
		<th>Emisor</th><th>RFC Emisor</th><th>Estado</th><th>Tipo Pago</th><th>Subtotal</th>
		<th>Iva</th><th>Total</th><th>UUID</th><th>Cheques</th>
		<th>Usuario<br>';
		echo '<select name="usuario" onchange="document.forma.usu.value=this.value;buscarRegistros();"><option value="all">---Todos---</option>';
		$res1=mysql_query("SELECT a.usuario FROM gastos_xml as a WHERE plaza='".$_POST['plazausuario']."' GROUP BY a.usuario ORDER BY a.usuario");
		...
		while ($Abono=mysql_fetch_array($rsabonos)){	
			...
			if($Abono['estatus']=='C'){
				...
				if(file_exists('../xmls/cfdig_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf')){
					echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'../xmls/cfdig_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf\',\'_blank\',\'0\',\''.$Abono['cve'].'\');"><img src="images/b_print.png" border="0" title="Imprimir '.$Abono['folio'].'"></a>';
				}
				elseif(file_exists('../xmls/gastoc_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf')){
					echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'../xmls/gastoc_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf\',\'_blank\',\'0\',\''.$Abono['cve'].'\');"><img src="images/b_print.png" border="0" title="Imprimir '.$Abono['folio'].'"></a>';
					if($_POST['cveusuario']==1){
						echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'gastos_xml.php\',\'_blank\',\'103\',\'xmls/gastoc_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf\');"><img src="images/basura.gif" border="0" title="Borrar PDF '.$Abono['folio'].'"></a>';
					}
				}
				else{
					echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'gastos_xml.php\',\'_blank\',\'101\',\''.$Abono['cve'].'\');"><img src="images/b_print.png" border="0" title="Imprimir '.$Abono['folio'].'"></a>';
				}
				echo '</td>';
				if($_POST['cveusuario']==1){
				...
				elseif(file_exists('../xmls/gasto_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf')){
					echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'../xmls/gasto_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf\',\'_blank\',\'0\',\''.$Abono['cve'].'\');"><img src="images/b_print.png" border="0" title="Imprimir '.$Abono['folio'].'"></a>';
					if($_POST['cveusuario']==1){
						echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'gastos_xml.php\',\'_blank\',\'103\',\'xmls/gasto_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf\');"><img src="images/basura.gif" border="0" title="Borrar PDF '.$Abono['folio'].'"></a>';
					}
				}
				else{
					echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'gastos_xml.php\',\'_blank\',\'101\',\''.$Abono['cve'].'\');"><img src="images/b_print.png" border="0" title="Imprimir '.$Abono['folio'].'"></a>';
				...
				if(nivelUsuario()>2)
					echo '&nbsp;&nbsp;<a href="#" onClick="if(confirm(\'Esta seguro de cancelar?\')){$(\'#panel\').show();atcr(\'gastos_xml.php\',\'\',\'3\',\''.$Abono['cve'].'\');}"><img src="images/validono.gif" border="0" title="Cancelar '.$Abono['folio'].'"></a>';
				echo '</td>';
				if($_POST['cveusuario']==1){
					echo '<td><input type="checkbox" class="checks" name="checksf[]" value="'.$Abono['cve'].'"></td>';
				}
...
if($_POST['cmd']==6){
	...
	foreach($_POST['checksf'] as $cvefact){
		$res = mysql_query("SELECT * FROM gastos_xml WHERE plaza='".$_POST['plazausuario']."' AND cve='".$cvefact."'");
		...
		$mail->Subject = "Factura de Gasto ".$foliofact;
		$mail->Body = "Factura de Gasto ".$foliofact;
		//$mail->AddAddress(trim($emailenvio));
		$correos = explode(",",trim($emailenvio));
		foreach($correos as $correo)
			$mail->AddAddress(trim($correo));
		if($row['estatus']=='C')
			$mail->AddAttachment("../xmls/gastoc_".$_POST['plazausuario']."_".$cvefact.".pdf", "Factura ".$foliofact.".pdf");
		else
			$mail->AddAttachment("../xmls/gasto_".$_POST['plazausuario']."_".$cvefact.".pdf", "Factura ".$foliofact.".pdf");
		$mail->AddAttachment("../xmls/cfdig_".$_POST['plazausuario']."_".$cvefact.".xml", "Factura ".$foliofact.".xml");
		$mail->Send();
		if($rowempresa['email']!=""){
			$mail = new PHPMailer();
			$mail->Host = "localhost";
	...
	foreach($_POST['checksf'] as $cvefact){
		...
		if($row['estatus']!='C'){
			...
			/*if($row['uuid']!=""){
					...
					if ($oSoapClient->fault) {
						...
						if ($err){
							...
							if($respuesta['resultado']){
								mysql_query("UPDATE gastos_xml SET estatus='C',usucan='".$_POST['cveusuario']."',fechacan='".fechaLocal()." ".horaLocal()."',respuesta2='".$respuesta['mensaje']."' WHERE plaza='".$_POST['plazausuario']."' AND cve='".$cvefact."'");
				...
				mysql_query("UPDATE gastos_xml SET estatus='C',usucan='".$_POST['cveusuario']."',fechacan='".fechaLocal()." ".horaLocal()."' WHERE plaza='".$_POST['plazausuario']."' AND cve='".$cvefact."'");
				generaGastoPdf($_POST['plazausuario'],$cvefact);
			//}
		}
	}
	$_POST['cmd']=0;
...
if($_POST['cmd']==3){
	require_once("../phpmailer/class.phpmailer.php");
	$res = mysql_query("SELECT * FROM gastos_xml WHERE plaza='".$_POST['plazausuario']."' AND cve='".$_POST['reg']."'");
	$row = mysql_fetch_array($res);
	if($row['estatus']!='C'){
		$cvefact=$row['cve'];
		$foliofact=$row['folio'];
		/*if($row['uuid']!=""){
			...
			if($err!="")
				...
				if ($oSoapClient->fault) {
					...
					if ($err){
						...
						if($respuesta['resultado']){
							mysql_query("UPDATE gastos_xml SET estatus='C',usucan='".$_POST['cveusuario']."',fechacan='".fechaLocal()." ".horaLocal()."',respuesta2='".$respuesta['mensaje']."' WHERE plaza='".$_POST['plazausuario']."' AND cve='".$_POST['reg']."'");
							generaFacturaPdf($_POST['plazausuario'],$cvefact);
							if($emailenvio!=""){
								$mail = new PHPMailer();
								$mail->Host = "localhost";
			...
			mysql_query("UPDATE gastos_xml SET estatus='C',usucan='".$_POST['cveusuario']."',fechacan='".fechaLocal()." ".horaLocal()."' WHERE plaza='".$_POST['plazausuario']."' AND cve='".$_POST['reg']."'");
			generaGastoPdf($_POST['plazausuario'],$cvefact);
		//}
	}
	$_POST['cmd']=0;
...
top($_SESSION);
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr><td align="left">Emisor</td><td><input type="text" name="cliente" id="cliente"  size="30" class="textField" value=""></td></tr>';
		echo '<tr><td align="left">Estado</td><td><input type="text" name="estado" id="estado"  size="30" class="textField" value=""></td></tr>';
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","gastos_xml.php",true);
			objeto.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
			objeto.send("ajax=1&estado="+document.getElementById("estado").value+"&estatus="+document.getElementById("estatus").value+"&cliente="+document.getElementById("cliente").value+"&fecha_ini="+document.getElementById("fecha_ini").value+"&fecha_fin="+document.getElementById("fecha_fin").value+"&usu="+document.getElementById("usu").value+"&numeroPagina="+document.getElementById("numeroPagina").value+"&plazausuario="+document.getElementById("plazausuario").value+"&cveusuario="+document.getElementById("cveusuario").value+"&cvemenu="+document.getElementById("cvemenu").value);
