...
if($_POST['ajax']==1) {
		//Listado de plazas
		$select= " SELECT * FROM sat_empresas WHERE 1 ";
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			echo '<tr bgcolor="#E9F2F8"><th>Editar</th><th>Nombre</th><th>RFC</th>';
			...
			while($row=mysql_fetch_array($res)) {
				...
				echo '<td>'.htmlentities(utf8_encode($row['rfc'])).'</td>';
				echo '</tr>';
			}
			echo '	
				<tr>
...
if ($_POST['cmd']==2) {
	...
	if($_POST['reg']) {
			...
			$update = " UPDATE sat_empresas 
						SET nombre='".$_POST['nombre']."',rfc='".$_POST['rfc']."'
						WHERE cve='".$_POST['reg']."' " ;
			$ejecutar = mysql_query($update);			
	} else {
			//Insertar el Registro
			$insert = " INSERT INTO sat_empresas 
						(nombre,rfc)
						VALUES 
						('".$_POST['nombre']."','".$_POST['rfc']."')";
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		$select=" SELECT * FROM sat_empresas WHERE cve='".$_POST['reg']."' ";
		$res=mysql_query($select);
		...
		echo '<tr><th align="left">RFC</th><td><input type="text" name="rfc" id="rfc" class="textField" size="50" value="'.$row['cuenta'].'"></td></tr>';
		echo '</table>';
...
/*** PAGINA PRINCIPAL **************************************************/
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr>
				...
				<td><a href="#" onClick="atcr(\'sat_empresas.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;Nuevo</td><td>&nbsp;</td>
			 </tr>';
		echo '</table>';
		echo '<table>';
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","sat_empresas.php",true);
