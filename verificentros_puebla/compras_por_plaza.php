...
if($_POST['cmd']==100){
	...
	foreach($array_plazas as $k=>$v){
		...
		//	if($_POST['plaza']=='all' || $k==$_POST['plaza']){
				...
				WHERE a.plaza='".$k."' AND a.estatus!='C' GROUP BY a.engomado";
				$res=mysql_query($select) or die(mysql_error());
				$array_montoengomados=array();
				while($row=mysql_fetch_array($res)){
					$array_montoengomados[$row[0]][0]=$row[1];
					$array_montoengomados[$row[0]][1]=$row[2];
					$array_montoengomados[$row[0]][2]=$row[1]-$row[2];
				}
...
top($_SESSION);
	...
	if ($_POST['cmd']<1) {
		...
		echo '</table>';
		echo '<table>';
		echo '<tr style="display:none;"><td align="left">Fecha Inicial</td><td><input type="text" name="fecha_ini" id="fecha_ini"  size="15" class="readOnly" value="'.substr(fechaLocal(),0,8).'01" readonly>&nbsp;<a href="#" onClick="displayCalendar(document.forms[0].fecha_ini,\'yyyy-mm-dd\',this,true)"><img src="images/calendario.gif" border="0"></a></td></tr>';
