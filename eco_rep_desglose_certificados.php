...
$rsUsuario=mysql_query("SELECT a.* FROM plazas a inner join datosempresas b on a.cve = b.plaza where a.estatus!='I' and b.localidad_id=2 ORDER BY a.numero");
while($Usuario=mysql_fetch_array($rsUsuario)){
...
if($_POST['cmd']==100){
	...
	if($_POST['engomado'] != 'all')
		$res = mysql_query("SELECT * FROM engomados WHERE cve = '".$_POST['engomado']."' and entrega=1 ORDER BY nombre");
	else
	...
	if($_POST['certificado'] != "") $filtro .= " AND CAST(certificado as UNSIGNED) = '".intval($_POST['certificado'])."'";
	if($_POST['anio'] != "all") $filtro .= " AND anio = '".$_POST['anio']."'";
	if($_POST['engomado'] != "all") $filtro .= " AND engomado = '".$_POST['engomado']."'";

	$res = mysql_query("SELECT engomado, plaza, certificado, fecha, anio FROM (
			SELECT a.engomado, a.plaza, a.certificado, a.fecha, a.anio FROM certificados a 
			...
			UNION ALL
			SELECT a.engomado, a.plaza, a.certificado, a.fecha, a.anio FROM certificados_cancelados a 
			INNER JOIN datosempresas d ON a.plaza = d.plaza
			WHERE a.plaza IN (".$_POST['plaza'].") AND a.fecha BETWEEN '".$_POST['fecha_ini']."' AND '".$_POST['fecha_fin']."' AND a.estatus!='C' AND d.localidad_id = '".$_POST['localidad']."'
		) a ".$filtro."
	...
	foreach($array_engomados as $k=>$datos){
		if($_POST['plazausuario']==0){
			echo '<th colspan="4">'.$datos['nombre'].'</th>';
		}
		else{
			echo '<th colspan=3>'.$datos['nombre'].'</th>';
		}
	...
	foreach($array_engomados as $k=>$datos){
		...
		echo '<th>Fecha<br>Entrega</th><th>Certificado</th><th>A&ntilde;o<br>Certificaci&oacute;n</th>';
	}
	echo '</tr>';
	...
	for($i=0;$i<$maximo;$i++){
		...
		foreach($array_engomados as $k=>$datos){
			...
			echo '<td align="center">'.$datos['certificados'][$i]['fecha'].'</td>';
			echo '<td align="center">'.$datos['certificados'][$i]['certificado'].'</td>';
			echo '<td align="center">'.$datos['certificados'][$i]['anio'].'</td>';
		}
		echo '</tr>';
	}
	echo '</table>';
...
top($_SESSION);
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr>
				...
				echo 'atcr(\'eco_rep_desglose_certificados.php\',\'_blank\',100,0);}"><img src="images/b_print.png" border="0">&nbsp;&nbsp;Imprimir</a>&nbsp;&nbsp;</td>';
		echo '</tr>';
		echo '</table>';
		echo '<table>';
		...
		echo '<tr><td align="left">Certificado</td><td><input type="text" name="certificado" id="certificado"  size="30" class="textField" value=""></td></tr>';
		if($_POST['plazausuario']>0){
			...
			echo '<tr style="display:none;"><td align="left">Localidad</td><td><select name="localidad" id="localidad"><option value="all">Seleccione</option>';
		...
		echo '<tr><td align="left">A&ntilde;o de Certificacion</td><td><select name="anio" id="anio"><option value="all">Todos</option>';
		foreach($array_anios as $k=>$v){
			echo '<option value="'.$k.'">'.$v.'</option>';
		}
		echo '</select></td></tr>';
		echo '<tr><td align="left">Tipo de Certificado</td><td><select name="engomado" id="engomado"><option value="all">Todos</option>';
...
echo '  document.getElementById("Resultados").innerHTML = "<img src=\'images/ajaxtrabajando.gif\' border=\'0\' align=\'absmiddle\'> Espere un momento, buscando registros...";
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","eco_rep_desglose_certificados.php",true);
			objeto.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
			objeto.send("ajax=1&anio="+document.getElementById("anio").value+"&engomado="+document.getElementById("engomado").value+"&certificado="+document.getElementById("certificado").value+"&localidad="+document.getElementById("localidad").value+"&plaza="+document.getElementById("plaza").value+"&fecha_ini="+document.getElementById("fecha_ini").value+"&fecha_fin="+document.getElementById("fecha_fin").value+"&numeroPagina="+document.getElementById("numeroPagina").value+"&plazausuario="+document.getElementById("plazausuario").value+"&cveusuario="+document.getElementById("cveusuario").value+"&cvemenu="+document.getElementById("cvemenu").value);
