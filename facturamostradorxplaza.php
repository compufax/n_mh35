...
if($_POST['ajax']==1){
	...
	echo '<th>Acumulado Vendido</th><th>Acumulado Facturado</th><th>Acumulado Restante</th>';
	echo '</tr>';
	$x=0;
	...
	foreach($array_plazas as $k=>$v){
		...
		while($fecha <= $_POST['fecha_fin']){
			...
			$sumacargo[$c]+=$array_importes[$k][$fecha]['vendido'];
			$acumulado['vendido'] += $array_importes[$k][$fecha]['vendido'];
			$c++;
			
			$sumacargo[$c]+=$array_importes[$k][$fecha]['facturado'];
			$acumulado['facturado'] += $array_importes[$k][$fecha]['facturado'];
			$c++;
			
			$sumacargo[$c]+=$array_importes[$k][$fecha]['restante'];
			$acumulado['restante'] += $array_importes[$k][$fecha]['restante'];
			$c++;
			
			$fecha=date( "Y-m-d" , strtotime ( "+1 day" , strtotime($fecha) ) );
		}
			echo '<td align="right">'.number_format($acumulado['vendido'],2).'</td>';
			echo '<td align="right">'.number_format($acumulado['facturado'],2).'</td>';
			echo '<td align="right"><font color="RED">'.number_format($acumulado['restante'],2).'</font></td>';
		echo '</tr>';
		$x++;
		// totales de acumulados
		$total_acumulado['vendido'] += $acumulado['vendido'];
		$total_acumulado['facturado'] += $acumulado['facturado'];
		$total_acumulado['restante'] += $acumulado['restante'];
	}
	echo '<tr>';
	echo '<td bgcolor="#E9F2F8" align="right">&nbsp;Total</td>';
	foreach($sumacargo as $k=>$v){
	...
	echo '<td bgcolor="#E9F2F8" align="right">&nbsp;'.number_format($total_acumulado['vendido'],2).'</td>';
	echo '<td bgcolor="#E9F2F8" align="right">&nbsp;'.number_format($total_acumulado['facturado'],2).'</td>';
	echo '<td bgcolor="#E9F2F8" align="right">&nbsp;'.number_format($total_acumulado['restante'],2).'</td>';
	echo '</tr>';
	echo '</table>';
	exit();
}
