...
$res=mysql_query("SELECT local, validar_certificado, validar_certificado_anterior FROM plazas WHERE cve='".$_POST['plazausuario']."'");
$row=mysql_fetch_array($res);
$PlazaLocal=$row[0];
$ValidarCertificados=$row[1];
$ValidarCertificadosAnterior=$row[2];
	...
	if(mysql_num_rows($res)==0){
		...
		if(mysql_num_rows($res)>0){
			...
			if($ValidarCertificados == 1){
				if($_POST['engomado'] == 9 || $_POST['engomado'] == 6)
					$res = mysql_query("SELECT b.estatus, b.tipo FROM compra_certificados a INNER JOIN compra_certificados_detalle b ON a.plaza = b.plaza AND a.cve = b.cvecompra WHERE a.plaza='".$_POST['plazausuario']."' AND a.engomado = '".$_POST['engomado']."' AND b.folio='".intval($_POST['certificado'])."'");
				else
					$res = mysql_query("SELECT b.estatus, b.tipo FROM compra_certificados a INNER JOIN compra_certificados_detalle b ON a.plaza = b.plaza AND a.cve = b.cvecompra WHERE a.plaza='".$_POST['plazausuario']."' AND a.engomado = '".$_POST['engomado']."' AND a.anio='".$_POST['anio']."' AND b.folio='".intval($_POST['certificado'])."'");
				if($row = mysql_fetch_array($res)){
					if($row['tipo']==0)
						echo '0';
					else
	...
	if(mysql_num_rows($res)==0){
		...
		if($row=mysql_fetch_array($res)){
			...
			elseif($row['anio']==1){
				...
				$row1 = mysql_fetch_array($res1);
				echo $cve.'|'.$fecha.'|'.round($row['monto']+$row1[0],2).'|'.$placa.'|'.$anio.'|'.$tiene_multa;
			}
		}
		else{
			echo "0|";
...
if($_POST['ajax']==5){
	mysql_query("UPDATE certificados SET engomado='".$_POST['engomado']."' WHERE plaza='".$_POST['plazausuario']."' AND cve='".$_POST['folio']."'");
	exit();
}
...
if($_POST['ajax']==7){
	...
	if($ValidarCertificadosAnterior == 1){
		$res = mysql_query("SELECT a.engomado, a.certificado FROM certificados a INNER JOIN datosempresas b ON a.plaza = b.plaza 
			WHERE TRIM(a.placa) = '".trim($_POST['placa'])."' AND a.estatus!='C' AND b.localidad_id = 2 ORDER BY a.fecha DESC, a.hora DESC");
		if($row = mysql_fetch_array($res)){
			if($row['engomado'] != $_POST['engomado_anterior'] || intval($row['certificado']) != intval($_POST['folio_anterior'])){
				$resultado['error'] = 1;
				$resultado['mensaje'] = 'El certificado anterior no coincide con el capturado';
			}
		}
	}
	...
	if($_POST['engomado']>0){
		...
		if(mysql_num_rows($res)==0){
			...
			if(mysql_num_rows($res)==0){
				...
				if($row=mysql_fetch_array($res)){
					if($_POST['engomado'] == 9 || $_POST['engomado'] == 6)
						$res = mysql_query("SELECT b.estatus, b.tipo FROM compra_certificados a INNER JOIN compra_certificados_detalle b ON a.plaza = b.plaza AND a.cve = b.cvecompra WHERE a.plaza='".$_POST['plazausuario']."' AND a.engomado = '".$_POST['engomado']."' AND b.folio='".intval($_POST['certificado'])."' AND b.tipo=0");
					else
						$res = mysql_query("SELECT b.estatus FROM compra_certificados a INNER JOIN compra_certificados_detalle b ON a.plaza = b.plaza AND a.cve = b.cvecompra WHERE a.plaza='".$_POST['plazausuario']."' AND a.engomado = '".$_POST['engomado']."' AND a.anio='".$_POST['anio']."' AND b.folio='".intval($_POST['certificado'])."' AND b.tipo=0");
					if(mysql_num_rows($res)>0 || $ValidarCertificados == 0){
						$insert = " INSERT certificados 
										...
										usuario='".$_POST['cveusuario']."',estatus='A',ticket='".$_POST['ticket']."',tecnico='".$_POST['tecnico']."',entregado='1'";
						mysql_query($insert) or die(mysql_error());
						if($_POST['engomado'] == 9 || $_POST['engomado'] == 6)
							mysql_query("UPDATE compra_certificados a INNER JOIN compra_certificados_detalle b ON a.plaza = b.plaza AND a.cve = b.cvecompra SET b.estatus=1 WHERE a.plaza='".$_POST['plazausuario']."' AND a.engomado = '".$_POST['engomado']."' AND b.folio='".intval($_POST['certificado'])."' AND b.tipo=0");
						else
							mysql_query("UPDATE compra_certificados a INNER JOIN compra_certificados_detalle b ON a.plaza = b.plaza AND a.cve = b.cvecompra SET b.estatus=1 WHERE a.plaza='".$_POST['plazausuario']."' AND a.engomado = '".$_POST['engomado']."' AND a.anio='".$_POST['anio']."' AND b.folio='".intval($_POST['certificado'])."' AND b.tipo=0");
					}
				}
			}
		}
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		echo '<input type="hidden" name="tiene_multa" id="tiene_multa" value="">';
		echo '<table>';
		...
		/*echo '<tr><th align="left">Engomado</th><td><select name="engomado" id="engomado" style="font-size:20px"><option value="0">Seleccione</option>';
		foreach($array_engomado as $k=>$v){
			echo '<option value="'.$k.'">'.$v.'</option>';
		}
		echo '</select></td></tr>';*/
		echo '<tr><th align="left">Tipo de Certificado</th><td><input type="hidden" name="engomado" id="engomado" value="0"><input type="hidden" name="monto" id="monto" value="0"><table><tr>';
		$i=0;
		foreach($array_engomado as $k=>$v){
			if($i==4){
				echo '</tr><tr>';
			...
			echo '<td><input type="radio" name="auxengomado" value="'.$k.'" onClick="if(this.checked){document.forma.engomado.value=this.value; document.forma.monto.value=\''.$array_engomadoprecio[$k].'\';}">'.$v.'&nbsp;&nbsp;&nbsp;</td>';
			$i++;
		}
		echo '</td></tr></table></td></tr>';
		//echo '<tr><th align="left">Engomado</td><td><input type="hidden" name="engomado" id="engomado" value="">
		//<input type="text" name="nomengomado" id="nomengomado" class="readOnly" style="font-size:20px" size="10" value="" readOnly></td></tr>';
		echo '<tr><th align="left">Tecnico</th><td><select name="tecnico" id="tecnico"><option value="0">Seleccione</option>';
		//$res1=mysql_query("SELECT * FROM personal WHERE plaza='".$_POST['plazausuario']."' AND puesto='14' AND estatus='1' ORDER BY nombre");
		...
		echo '<tr><th align="left">Tipo de Certificado Anterior</th><td><input type="hidden" name="engomado_anterior" id="engomado_anterior" value="0"><table><tr>';
		$i=0;
		foreach($array_engomado as $k=>$v){
			if($i==4){
				echo '</tr><tr>';
			...
			echo '<td><input type="radio" name="auxengomadoanterior" value="'.$k.'" onClick="if(this.checked){document.forma.engomado_anterior.value=this.value; }">'.$v.'&nbsp;&nbsp;&nbsp;</td>';
			$i++;
		}
		echo '</td></tr></table></td></tr>';
		echo '<tr><th align="left">Holograma Anterior</th><td><input type="text" name="folio_anterior" id="folio_anterior" class="textField" style="font-size:20px" size="30" value=""></td></tr>';
		...
		echo '<script>
				...
				function validar(){
					...
					else if(!validarCertificadoAnterior()){
						$("#panel").hide();
					}
					else{
						validarCertificado();
				...
				function validarCertificadoAnterior(){
					if(document.forma.tiene_multa.value == "1"){
						return true;
					}
					regresar = false;
					$.ajax({
					  ...
					  data: {
						...
						engomado_anterior: document.getElementById("engomado_anterior").value,
						folio_anterior: document.getElementById("folio_anterior").value,
				...
				function validarCertificado(){
					...
					if (objeto.readyState != 0) {
						...
						objeto.send("ajax=3&anio="+document.getElementById("anio").value+"&engomado="+document.getElementById("engomado").value+"&certificado="+document.getElementById("certificado").value+"&plazausuario="+document.getElementById("plazausuario").value);
				...
				function traeTicket(){
					if(document.forma.ticket.value==""){
						...
						document.forma.tiene_multa.value="";
					}
					else{
						objeto=crearObjeto();
						if (objeto.readyState != 0) {
							...
							objeto.onreadystatechange = function()
							{
								if (objeto.readyState==4)
								{
									...
									if(datos[0]=="-1"){
										...
										document.forma.tiene_multa.value=datos[5];
									}
								}
							}
						}
...
/*** PAGINA PRINCIPAL **************************************************/
	...
	if ($_POST['cmd']<1) {
		...
		echo '<br>';
		if($PlazaLocal==1)echo '<h1>La captura de la entrega de certificados en esta plaza es de forma local</h1>';
		//Listado
