...
if($_POST['ajax']==1) {
		...
		FROM clientes a INNER JOIN facturas b on a.cve = b.cliente and b.estatus!='C' and b.fecha>='2016-09-01' 
		LEFT JOIN plazas c ON c.cve = b.plaza";
		$select .= " WHERE a.rfc='".$_POST['rfc']."'";
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			<th>Iva</th><th>Total</th><th>Ticket</th><th>Fecha Ticket</th><th>Placa Ticket</th>
			<th>Usuario</th></tr>'; 
			$t=$t2=0;
...
if ($_POST['cmd']<1) {
	...
	echo '</table>';
	echo '<table>';
	echo '<tr><td>RFC</td><td><input type="text" name="rfc" id="rfc" size="10" class="textField" value=""></td></tr>';
	echo '</table>';
	echo '<br>';
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.send("ajax=1&rfc="+document.getElementById("rfc").value+"&cveusuario="+document.getElementById("cveusuario").value+"&cvemenu="+document.getElementById("cvemenu").value+"&plazausuario="+document.getElementById("plazausuario").value+"&numeroPagina="+document.getElementById("numeroPagina").value);
