...
function generaFacturaPdf($plaza,$cvefact,$mostrar=0){
	...
	$pdf->AddPage();
	$res = mysql_query("SELECT * FROM facturas_xml WHERE plaza='".$plaza."' AND cve='".$cvefact."'");
	...
	$pdf->Cell(52,4,':'.$numeroPlaza.' - '.$row['folio'],0,0,"L",0);
	$pdf->Ln();
	$pdf->SetX(122.5);
	...
	$pdf->Ln();
	$pdf->SetX(110);
	$pdf->MultiCell(97.5,4,$row['nombre'],0,"L",0);
	//$pdf->Ln();
	$pdf->SetX(110);
	$pdf->Cell(97.5,4,$row['rfc'],0,0,"L",0);
	$rr=$row1['rfc'];
	$pdf->Ln();
	$pdf->SetX(110);
	$pdf->Cell(97.5,4,$row['calle'].' '.$row['numexterior'].' '.$row['numinterior'],0,0,'L',0);
	$pdf->Ln();
	$pdf->SetX(110);
	$pdf->Cell(97.5,4,$row['colonia'],0,0,"L",0);
	$pdf->Ln();
	$pdf->SetX(110);
	if($row1['localidad']!="")
		$pdf->Cell(97.5,4,$row['localidad'].' C.P. '.$row['codigopostal'].', '.$row['municipio'].', '.$row['estado'],0,0,"L",0);
	else
		$pdf->Cell(97.5,4,'C.P. '.$row['codigopostal'].', '.$row['municipio'].', '.$row['estado'],0,0,"L",0);
	$pdf->Ln();
	//$pdf->SetX(110);
	...
	$pdf->Ln();
	$y=$pdf->GetY();
	$res2 = mysql_query("SELECT * FROM facturasmov_xml WHERE plaza='".$plaza."' AND cvefact='".$cvefact."'");
	...
	QRcode::png("?re=".$re."&rr=".$rr."&tt=".$tt."&id=".$row['uuid'],"xmls/barcode_".$row['plaza'].'_'.$row['cve'].".png","L",4,0);
	if(file_exists("xmls/barcode_".$row['plaza'].'_'.$row['cve'].".png")) $pdf->Image("xmls/barcode_".$row['plaza'].'_'.$row['cve'].".png",10,$y,34,34);
	if($row['estatus']=='C'){
		$pdf->Output("xmls/facturac_".$row['plaza']."_".$row['cve'].".pdf","F");
	}
	else{
		$pdf->Output("xmls/factura_".$row['plaza']."_".$row['cve'].".pdf","F");
	}
	if(file_exists("xmls/barcode_".$row['plaza'].'_'.$row['cve'].".png")) unlink("xmls/barcode_".$row['plaza'].'_'.$row['cve'].".png");
...
function generaGastoPdf($plaza,$cvefact,$mostrar=0){
	global $base,$array_tipo_pago;
	$color="#F0F0F0";
	$pdf = new PDF_MC_Table('P','mm','LETTER');
	$pdf->AddPage();
	$res = mysql_query("SELECT * FROM gastos_xml WHERE plaza='".$plaza."' AND cve='".$cvefact."'");
	$row = mysql_fetch_array($res);
	$pdf->SetFont("Arial","",8);
	$pdf->SetFillColor(240,240,240);
	$res1 = mysql_query("SELECT * FROM plazas WHERE cve='".$row['plaza']."'");
	...
	if($row['tipo_factura']==1)
		...
		$pdf->Cell(85,6.5,"FACTURA GASTOS",1,0,"C",1);
	$pdf->Ln();
	$pdf->SetX(122.5);
	$pdf->Cell(85,4,$row['uuid'],0,0,"C",0);
	$pdf->Ln();
	...
	$pdf->Ln();
	$y=$pdf->GetY();
	$pdf->Cell(100,6.5,"DATOS DEL RECEPTOR",1,0,"C",1);
	$pdf->Ln();
	$pdf->Cell(100,4,$row1['nombre'],0,0,"L",0);
	$pdf->Ln();
	$pdf->Cell(100,4,$row1['rfc'],0,0,"L",0);
	...
	$y2=$pdf->GetY();
	$pdf->SetXY(110,$y);
	$pdf->Cell(97.5,6.5,"DATOS DEL EMISOR",1,0,"C",1);
	$pdf->Ln();
	$pdf->SetX(110);
	$pdf->MultiCell(97.5,4,$row['nombre'],0,"L",0);
	//$pdf->Ln();
	...
	$pdf->Ln();
	$y=$pdf->GetY();
	$res2 = mysql_query("SELECT * FROM gastosmov_xml WHERE plaza='".$plaza."' AND cvefact='".$cvefact."'");
	while($row2 = mysql_fetch_array($res2)){
		$pdf->SetXY(10,$y);
		$pdf->Cell(30,4,$row2['unidad'],0,0,"C",0);
		$y3=$pdf->GetY();
	...
	QRcode::png("?re=".$re."&rr=".$rr."&tt=".$tt."&id=".$row['uuid'],"xmls/barcodeg_".$row['plaza'].'_'.$row['cve'].".png","L",4,0);
	if(file_exists("xmls/barcodeg_".$row['plaza'].'_'.$row['cve'].".png")) $pdf->Image("xmls/barcodeg_".$row['plaza'].'_'.$row['cve'].".png",10,$y,34,34);
	if($row['estatus']=='C'){
		$pdf->Output("xmls/gastosc_".$row['plaza']."_".$row['cve'].".pdf","F");
	}
	else{
		$pdf->Output("xmls/gastos_".$row['plaza']."_".$row['cve'].".pdf","F");
	}
	if(file_exists("xmls/barcodeg_".$row['plaza'].'_'.$row['cve'].".png")) unlink("xmls/barcodeg_".$row['plaza'].'_'.$row['cve'].".png");
