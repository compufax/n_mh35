...
function generaPreFacturaPdf($plaza,$cvefact,$mostrar=0){
	global $base,$array_tipo_pago,$array_forma_pago;
	$color="#F0F0F0";
	$pdf = new PDF_MC_Table('P','mm','LETTER');
	$pdf->AddPage();
	$res = mysql_query("SELECT * FROM prefacturas WHERE plaza='".$plaza."' AND cve='".$cvefact."'");
	...
	if($row['tipo_factura']==1)
		...
		$pdf->Cell(85,6.5,"SOLICITUD FACTURA",1,0,"C",1);
	$pdf->Ln();
	$pdf->SetX(122.5);
	...
	$pdf->Ln();
	$y=$pdf->GetY();
	$res2 = mysql_query("SELECT * FROM prefacturasmov WHERE plaza='".$plaza."' AND cvefact='".$cvefact."'");
