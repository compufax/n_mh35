...
if($_POST['ajax']==1) {
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			echo '<th>Nombre</th><th>Lugar</th><th>Propietario</th><th>Email</th><th>Telefonos</th>';
...
if ($_POST['cmd']==2) {
	...
	if($_POST['reg']) {
			...
			$update = " UPDATE depositantes 
						...
						bloquear_saldo_negativo='".$_POST['bloquear_saldo_negativo']."'
						WHERE cve='".$_POST['reg']."' " ;
			$ejecutar = mysql_query($update);			
	} else {
			//Insertar el Registro
			$insert = " INSERT INTO depositantes 
						(nombre,plaza,edo_cuenta,tipo,agencia,propietario,email,telefono,bloquear_saldo_negativo)
						VALUES 
						('".$_POST['nombre']."','".$_POST['plaza']."','1','".$_POST['tipo']."','".$_POST['agencia']."',
							'".$_POST['propietario']."','".$_POST['email']."','".$_POST['telefono']."',
							'".$_POST['bloquear_saldo_negativo']."')";
