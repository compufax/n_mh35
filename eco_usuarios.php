...
if($_POST['ajax']==1) {
		...
		if($_SESSION['CveUsuario']>1){
			$select= " SELECT * FROM eco_usuarios WHERE estatus!='I' AND cve>'1'";
		}
		else{
			$select= " SELECT * FROM eco_usuarios WHERE estatus!='I'";
		}
...
if($_POST['ajax']==2){
	$res=mysql_query("SELECT * FROM eco_usuarios WHERE usuario='".$_POST['usuario']."' AND estatus!='I' AND cve!='".$_POST['cveusu']."'");
...
if ($_POST['cmd']==3) {
	$delete= "UPDATE eco_usuarios SET estatus='I' WHERE cve='".$_POST['reg']."' ";
	$ejecutar=mysql_query($delete);
	mysql_query("INSERT eco_historial SET menu='".$_POST['cvemenu']."',cveaux='".$_POST['reg']."',fecha='".fechaLocal()." ".horaLocal()."',
...
if ($_POST['cmd']==2) {
	if($_POST['reg']) {
		$select=" SELECT * FROM eco_usuarios WHERE cve='".$_POST['reg']."' ";
		...
		$insert = " INSERT INTO eco_usuarios (nombre,usuario,password,tipo,permitir_mas_dias)
					VALUES 
		...
		mysql_query("INSERT eco_historial SET menu='".$_POST['cvemenu']."',cveaux='".$cveusu."',fecha='".fechaLocal()." ".horaLocal()."',
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		echo '
			<td><a href="#" onClick="$(\'#panel\').show();atcr(\'eco_usuarios.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver</a></td><td>&nbsp;</td>
			</tr>';
		echo '</table>';
		echo '<br>';
		...
		echo '<script>
				function validarUsuario(reg)
				{
					...
					if (objeto.readyState != 0) {
						...
						objeto.open("POST","eco_usuarios.php",true);
...
/*** PAGINA PRINCIPAL **************************************************/
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr>
				...
				<td><a href="#" onClick="atcr(\'eco_usuarios.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;Nuevo</td><td>&nbsp;</td>
			 </tr>';
		echo '</table>';
