...
function generarNotaCargoPdf($plaza,$cvefact,$mostrar=0){
	global $base,$array_tipo_pago;
	if($empresa==21) $array_tipo_pago[2]='TRANSFERENCIA ELECTRONICA';
	$color="#F0F0F0";
	$pdf = new PDF_MC_Table('P','mm','LETTER');
	$pdf->AddPage();
	$res = mysql_query("SELECT * FROM notascargo WHERE plaza='".$plaza."' AND cve='".$cvefact."'");
	...
	$pdf->Cell(85,6.5,"NOTA DE CARGO",1,0,"C",1);
	$pdf->Ln();
	$pdf->SetX(122.5);
	...
	$pdf->Ln();
	$y=$pdf->GetY();
	$res2 = mysql_query("SELECT * FROM notascargomov WHERE plaza='".$plaza."' AND cvefact='".$cvefact."'");
	...
	QRcode::png("?re=".$re."&rr=".$rr."&tt=".$tt."&id=".$row['uuid'],"cfdi/comprobantes/barcodenc_".$row['plaza'].'_'.$row['cve'].".png","L",4,0);
	if(file_exists("cfdi/comprobantes/barcodenc_".$row['plaza'].'_'.$row['cve'].".png")) $pdf->Image("cfdi/comprobantes/barcodenc_".$row['plaza'].'_'.$row['cve'].".png",10,$y,34,34);
	if($row['estatus']=='C'){
		$pdf->Output("cfdi/comprobantes/notacargoc_".$row['plaza']."_".$row['cve'].".pdf","F");
	}
	else{
		$pdf->Output("cfdi/comprobantes/notacargo_".$row['plaza']."_".$row['cve'].".pdf","F");
	}
	if(file_exists("cfdi/comprobantes/barcodenc_".$row['plaza'].'_'.$row['cve'].".png")) unlink("cfdi/comprobantes/barcodenc_".$row['plaza'].'_'.$row['cve'].".png");
