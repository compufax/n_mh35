...
if($_POST['cmd']==2){
	foreach($_POST as $k=>$v){
		if($k!='plaza' && $k!='placat' && $k!='ticket'){
			$_POST[$k] = mb_strtoupper(utf8_encode($v));
		}
	}
	$plaza = $_POST['plaza'][0];
	...
	if($row=mysql_fetch_array($res)){
		...
		if(count($array_detalles)>0){
			...
			if($copias>0){
				...
				precio='".round(round($monto-$importe_iva,6)/$copias,2)."',importe='".round($monto-$importe_iva,6)."',
    ...
    function validar(){
        if(confirm(" Nombre: "+document.getElementById("nombre").value+"\n Email: "+document.getElementById("email").value+"\n Los datos son correctos?")){
            if(document.getElementById("nombre").value==""){
                ...
                $('#panel').hide();
                alert("Necesita seleccionar el regimen fiscal");
            }
