...
if($_POST['ajax']==1) {
		...
		echo '<tr><td bgcolor="#E9F2F8" colspan="7">'.mysql_num_rows($rsusuarios).' Registro(s)</td></tr>';
			echo '<tr bgcolor="#E9F2F8"><th>Accesos</th><th>Nombre</th><th>Tipo</th><th>Usuario</th><th>Categoria</th><th>Plazas</th><th>Borrar</th></tr>';
			while($Usuario=mysql_fetch_array($rsusuarios)) {
				...
				if($Usuario['cve']==1 || $Usuario['tipo']==1)
					...
					$res = mysql_query("SELECT a.cve,a.numero,a.nombre FROM plazas a INNER JOIN usuario_accesos b ON a.cve=b.plaza AND b.usuario='".$Usuario['cve']."' AND b.acceso>0 LEFT JOIN datosempresas c on a.cve = c.plaza WHERE a.estatus!='I' GROUP BY a.cve ORDER BY c.localidad_id, a.numero");
...
if ($_POST['cmd']==2) {
	if($_POST['reg']) {
		...
		if($_POST['plazausuario']>0){
			$res = mysql_query("SELECT * FROM menu WHERE cve>1 ORDER BY CVE");
			while($row = mysql_fetch_array($res)){
