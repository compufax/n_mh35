...
if($_POST['recargado'] != 1){
	echo '<form name="forma" id="forma" method="POST" action="facturacion_web2.php" enctype="multipart/form-data" class="form-horizontal">
				...
				if($encontrado){
					...
					elseif($row['factura']>0 && $row['notacredito']==0){
						...
						if($row1['respuesta1']==""){
							...
							$resultado['html'] .=  '&nbsp;&nbsp;<a href="#" onClick="atcr(\'facturacion_web2.php\',\'_blank\',\'200\',\''.$row1['plaza'].'|'.$row1['cve'].'|1\');"><img src="images/zip_grande.png" border="0" width="30px" height="30px" title="Descargar"></a>';
							if($row1['estatus'] == 'D') $resultado['html'] .= '<br>DEVUELTA';
							//else $resultado['html'] .=  '&nbsp;&nbsp;<a href="#" onClick="if(confirm(\'Esta seguro de generar la nota de credito de descuento? Se generara por el total de la factura.\'))atcr(\'facturacion_web2.php\',\'\',\'3\',\''.$row1['plaza'].'|'.$row1['cve'].'\');"><img src="images/cerrar.gif" border="0" width="15px" height="15px" title="Generar Nota de Credito"></a>';
						}
						$resultado['html'] .=  '</td>
						...
						if($row['notacredito']>0){
							...
							if($row1['respuesta1']==""){
								...
								$resultado['html'] .=  '&nbsp;&nbsp;<a href="#" onClick="atcr(\'facturacion_web2.php\',\'_blank\',\'200\',\''.$row1['plaza'].'|'.$row1['cve'].'|2\');"><img src="images/zip_grande.png" border="0" width="25px" height="25px" title="Descargar"></a>';
							}
							$resultado['html'] .=  '</td>
							<td align="left" style="padding: 10px!important">'.$row['nomplaza'].'</td>
...
if($_POST['ajax']==1){
	...
	if($row=mysql_fetch_array($res)){
		...
		if($repetido){
			...
			elseif($row['fecha']<=$limite2 || ($row['fecha']<=$limite1 && date('d')>'07')){
				$resultado['error'] = 1;
				$resultado['mensaje'] .= 'Vencio el limite de facturación'."\n";
			}
...
if($_POST['ajax']=='prefactura'){
	...
	<tr><td valign="top">
		<table>
		<tr><th align="left">Cliente:</th><td colspan=3>'.utf8_encode($datos['nombre']).'</td></tr>
		<tr><th align="left">R.F.C.:</th><td colspan=3>'.utf8_encode($datos['rfc']).'</td></tr>
		<tr><th align="left">Domicilio:</th><td colspan=3>'.utf8_encode($datos['calle'].' No. '.$datos['numexterior'].' '.$datos['numinterior']).'</td></tr>
		<tr><th align="left">Colonia:</th><td colspan=3>'.utf8_encode($datos['colonia']).'</td></tr>
		<!--<tr><th align="left">Localidad:</th><td colspan=3>'.utf8_encode($datos['localidad']).'</td></tr>-->
		<tr><th align="left">Municipio:</th><td colspan=3>'.utf8_encode($datos['municipio']).'</td></tr>
		<tr><th align="left">Estado:</th><td colspan=3>'.utf8_encode($datos['estado']).'</td></tr>
		<tr><th align="left">C.P.:</th><td>'.utf8_encode($datos['codigopostal']).'</td><th>PAIS:</th><td>MEXICO</td></tr>
	...
	echo '<tr><td colspan="3" align="left">'.utf8_encode(numlet($total)).'</td></tr></table><br />';
	echo '<p class="text-center"><input type="button" id="aceptar_generacion" class="btn btn-primary" value="Generar" onClick="confirmar_generacion()">
	&nbsp;&nbsp;&nbsp;&nbsp;<input type="button" id="aceptar_generacion" class="btn btn-primary" value="Editar" onClick="volver_editar()"></p><br /><br />		';
	exit();
}
					...
					if ($oSoapClient->fault) {
						...
						if ($err){
							...
							if($respuesta['resultado']){
								...
								echo '<h1>Se genero el folio de factura '.$fserie." ".$ffolio.'  <a href="#" onClick="atcr(\'facturacion_web2.php\',\'_blank\',\'200\',\''.$plaza.'|'.$cvefact.'|1\');"><img src="images/zip_grande.png" border="0" width="45px" height="45px" title="Descargar">Descarga aqui su factura</a></h1>';
							}
							else
								$strmsg=$respuesta['mensaje'];
							//print_r($respuesta);	
                ...
                success: function(data) {   
                	...
                	if(data.error == 1){
                		...
                		if(confirm("Ticket ya facturado desea descargar la factura?")){
                			atcr('facturacion_web2.php','_blank','200',''+data.plaza+'|'+data.factura+'|1');
                		}
