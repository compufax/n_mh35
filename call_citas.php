...
if($_POST['cmd']==10){
	...
	echo '<tr><th align="left">N&uacute;mero tarjeta circulaci&oacute;n</th><td>'.$row['tarjeta_circulacion'].'</td></tr>';
	...
	if($row['requiere_factura']==1){
		...
		echo '<tr><th align="left">Email</th><td>'.$row['email'].'</td></tr>';
		echo '<tr><th align="left">Calle</th><td>'.$row['calle'].'</td></tr>';
		echo '<tr><th align="left">Numero Exterior</th><td>'.$row['numexterior'].'</td></tr>';
		echo '<tr><th align="left">Numero Interior</th><td>'.$row['numinterior'].'</td></tr>';
		...
		echo '<tr><th align="left">Codigo Postal</th><td>'.$row['codigopostal'].'</td></tr>';
	}	
	...
	if($_POST['numeroverificacion1']!="" && $_POST['numeroverificacion1']==$_POST['numeroverificacion2']){
		...
		if(mysql_num_rows($res)==0){
			...
			if($disponibles>0){
				...
				$html.= '<tr><th align="left">N&uacute;mero tarjeta circulaci&oacute;n</th><td>'.$row['tarjeta_circulacion'].'</td></tr>';
				...
				if($row['requiere_factura']==1){
					...
					$html.= '<tr><th align="left">Email</th><td>'.$row['email'].'</td></tr>';
					$html.= '<tr><th align="left">Calle</th><td>'.$row['calle'].'</td></tr>';
					$html.= '<tr><th align="left">Numero Exterior</th><td>'.$row['numexterior'].'</td></tr>';
					$html.= '<tr><th align="left">Numero Interior</th><td>'.$row['numinterior'].'</td></tr>';
					...
					$html.= '<tr><th align="left">Codigo Postal</th><td>'.$row['codigopostal'].'</td></tr>';
				}	
...
if($_POST['cmd']==2){
	...
	echo '<tr><th align="left">N&uacute;mero tarjeta circulaci&oacute;n</th><td><input type="text" class="textField" name="camposi[tarjeta_circulacion]" id="tarjeta_circulacion" value="" size="20" maxlength="30" onKeyUp="this.value=this.value.toUpperCase();"></td></tr>';
	...
	echo '<tr style="display:none;"><th align="left" style="color:#FF0000">Requiere Factura</th><td><input type="radio" name="camposi[requiere_factura]" id="requiere_factura_0" value="0" onClick="$(\'.cfactura\').hide()" checked>No&nbsp;&nbsp;
	...
	echo '<tr style="display:none;" class="cfactura"><th align="left">Numero Exterior</th><td><input type="text" class="textField" name="camposi[numexterior]" id="numexterior" value="'.$row['numexterior'].'" size="10" onKeyUp="this.value=this.value.toUpperCase();"></td></tr>';
	echo '<tr style="display:none;" class="cfactura"><th align="left">Numero Interior</th><td><input type="text" class="textField" name="camposi[numinterior]" id="numinterior" value="'.$row['numinterior'].'" size="10" onKeyUp="this.value=this.value.toUpperCase();"></td></tr>';
	...
	echo '<tr style="display:none;" class="cfactura"><th align="left">Codigo Postal</th><td><input type="text" class="textField" name="camposi[codigopostal]" id="codigopostal" value="'.$row['codigopostal'].'" size="50"></td></tr>';
...
if($_POST['cmd']==1){
	echo '<table><tr><td><input type="button" style="font-size:20px" value="Ver Mes" onClick="atcr(\'call_citas.php\',\'\',0,0);"></td></tr></table><br>';
	echo '<table style="font-size:20px">';
	echo '<tr><th align="left">Tipo de Combustible</th><td><select style="font-size:20px" name="tipocombustible" id="tipocombustible" onChange="$(\'#panel\').show();atcr(\'call_citas.php\',\'\',0,0);"><option value="0">Todos los tipos de combustible</option>';
	...
	echo '</select></td></tr>';
	echo '<tr><th align="left">Centro de Verificaci&oacute;n</th><td><select style="font-size:20px" name="plaza" id="plaza" onChange="$(\'#panel\').show();atcr(\'call_citas.php\',\'\',0,\''.$_POST['reg'].'\');"><option value="0">Seleccione centro de verificación</option>';
	$filtro = '';
	if($_POST['tipocombustible'] > 0) $filtro = " AND tipocombustible LIKE '%|".$_POST['tipocombustible']."|%'";
	$res = mysql_query("SELECT * FROM datosempresas WHERE maneja_callcenter=1 $filtro ORDER BY nombre_callcenter");
	while($row=mysql_fetch_array($res)){
		echo '<option value="'.$row['plaza'].'"';
		if($row['plaza']==$_POST['plaza']) echo ' selected';
		echo '>'.$row['nombre_callcenter'].'</option>';
	}
	echo '</select></td></tr></table>';
	if($_POST['plaza']>0){
		$Usuario = mysql_fetch_array(mysql_query("SELECT permitir_mas_dias FROM call_usuarios WHERE cve='".$_POST['cveusuario']."'"));
		if($Usuario[0]==1) $diasusuario=15;
		else $diasusuario=5;
		...
		echo '<table width="100%"><td width="50%" align="left"><input type="button" value="Anterior" style="font-size:20px" onClick="atcr(\'call_citas.php\',\'\',1,\''.date( "Y-m-d" , strtotime ( "-1 day" , strtotime($_POST['reg']) ) ).'\');">
		<td width="50%" align="right"><input type="button" value="Siguiente" style="font-size:20px" onClick="atcr(\'call_citas.php\',\'\',1,\''.date( "Y-m-d" , strtotime ( "+ 1 day" , strtotime($_POST['reg']) ) ).'\');"></td></tr></table>';
		...
		$res1=mysql_query("SELECT fecha, hora, COUNT(cve) FROM call_citas WHERE plaza='".$_POST['plaza']."' AND fecha BETWEEN '".date( "Y-m-d" , strtotime ( "+ 1 day" , strtotime(date("Y-m-d")) ) )."' AND '".date( "Y-m-d" , strtotime ( "+ ".$diasusuario." day" , strtotime(date("Y-m-d")) ) )."' AND estatus!='C' GROUP BY fecha, hora");
		while($row1=mysql_fetch_array($res1)){
			$array_fechas_citas[$row1['fecha'].' '.substr($row1['hora'],0,6)] = $row1[2];
		}

		$array_fechas_conf = array();
		$res1=mysql_query("SELECT fecha, estatus FROM call_configuracion_dias WHERE fecha BETWEEN '".$fecha_ini."' AND '".$fecha_fin."'");
		while($row1=mysql_fetch_array($res1)){
			$array_fechas_conf[$row1['fecha']] = $row1[1];
		}
		...
		while($hora<$row['horafin'].':00'){
			...
			while($fecha<=$fecha_fin){
				...
				//if($fecha<=date("Y-m-d") || $fecha=='2015-05-05' || $fecha=='2015-04-04' || $fecha=='2015-05-01' || ($dia == 0 && $fecha!='2015-09-13') || $fecha>date( "Y-m-d" , strtotime ( "+ ".$diasusuario." day" , strtotime(date("Y-m-d")) ) )){
				if($fecha<=date("Y-m-d") || intval($array_fechas_conf[$fecha])==1 || ($dia == 0 && intval($array_fechas_conf[$fecha])!=2) || $fecha>date( "Y-m-d" , strtotime ( "+ ".$diasusuario." day" , strtotime(date("Y-m-d")) ) )){
					...
					elseif($disponibles>=($lineas/2)){
						echo '<td bgcolor="#00FF00"><a href="#" onClick="atcr(\'call_citas.php\',\'\',2,\''.$fecha.' '.$hora.'\');">'.$disponibles.' Disponible(s)</a></td>';
					}
					else{
						echo '<td bgcolor="#FFFF00"><a href="#" onClick="atcr(\'call_citas.php\',\'\',2,\''.$fecha.' '.$hora.'\');">'.$disponibles.' Disponible(s)</a></td>';
					}
				}
				$fecha = date( "Y-m-d" , strtotime ( "+ 1 day" , strtotime($fecha) ) );
			}
...
if($_POST['cmd']==0){
	...
	echo '</select></td></tr>';
	echo '<tr><th align="left">Centro de Verificaci&oacute;n</th><td><select style="font-size:20px" name="plaza" id="plaza" onChange="$(\'#panel\').show();atcr(\'call_citas.php\',\'\',0,0);"><option value="0">Seleccione centro de verificación</option>';
	$filtro = '';
	if($_POST['tipocombustible'] > 0) $filtro = " AND tipocombustible LIKE '%|".$_POST['tipocombustible']."|%'";
	$res = mysql_query("SELECT * FROM datosempresas WHERE maneja_callcenter=1 $filtro ORDER BY nombre_callcenter");
	while($row=mysql_fetch_array($res)){
	...
	if($_POST['plaza']>0){
		...
		echo '<table width="100%"><td width="50%" align="left"><input type="button" value="Anterior" style="font-size:20px" onClick="document.forma.mes.value=\''.date( "Y-m" , strtotime ( "-1 month" , strtotime($fecha) ) ).'\';atcr(\'call_citas.php\',\'\',0,0);"></td>
		<td width="50%" align="right"><input type="button" value="Siguiente" style="font-size:20px" onClick="document.forma.mes.value=\''.date( "Y-m" , strtotime ( "+ 1 month" , strtotime($fecha) ) ).'\';atcr(\'call_citas.php\',\'\',0,0);"></td></tr></table>';
		echo '<table style="font-size:20px" width="100%" border="1">
		...
		$array_fechas_citas = array();
		$res=mysql_query("SELECT fecha, COUNT(cve) FROM call_citas WHERE plaza='".$_POST['plaza']."' AND fecha BETWEEN '".date( "Y-m-d" , strtotime ( "+ 1 day" , strtotime(date("Y-m-d")) ) )."' AND '".date( "Y-m-d" , strtotime ( "+ ".$diasusuario." day" , strtotime(date("Y-m-d")) ) )."' AND estatus!='C' GROUP BY fecha");
		while($row=mysql_fetch_array($res)){
		...
		for($i=$dia;$fec<=$fecultima;$i++){
			...
			//if($fec<=date("Y-m-d") || $fec=='2015-05-05' || $fec=='2015-04-04' || $fec=='2015-05-01' || ($dia == 0 && $fec!='2015-09-13') || $fec>date( "Y-m-d" , strtotime ( "+ ".$diasusuario." day" , strtotime(date("Y-m-d")) ) )){
			if($fec<=date("Y-m-d") || intval($array_fechas_conf[$fec])==1 || ($dia == 0 && intval($array_fechas_conf[$fec])!=2) || $fec>date( "Y-m-d" , strtotime ( "+ ".$diasusuario." day" , strtotime(date("Y-m-d")) ) )){
				...
				echo '><table width="100%"><tr><td width="100%">&nbsp;</td></tr><tr><td align="center"><a href="#" onClick="atcr(\'call_citas.php\',\'\',1,\''.$fec.'\');">'.substr($fec,8,2).'</a></td></tr><tr><td style="font-size:10px" align="right">'.$disponible.' Disponible(s)</td></tr></table></td>';
			}
...
bottom();
if($cita_id>0){
	echo '<script>atcr("call_citas.php","_blank",10,'.$cita_id.');</script>';
}
?>
