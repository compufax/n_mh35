...
if($_POST['recargado'] != 1){
	...
	<input type="hidden" name="fechahoracarga" id="fechahoracarga" value="'.date('Y-m-d H:i:s').'">
...
$res = mysql_query("SELECT TIMEDIFF(NOW(), '".$_POST['fechahoracarga']."') as diferencia");
$row = mysql_fetch_array($res);
if(($_POST['idcliente'] == 0 || $row['diferencia'] > '00:10:00') && !isset($_POST['ajax'])){
	if($row['idcliente'] > 0) $error = 1;
	echo '<form name="forma" id="forma" method="POST" action="login_web.php" enctype="multipart/form-data" class="form-horizontal">
...
if($_POST['ajax']==1){
	...
	$res = mysql_query("SELECT a.plaza, a.cve as ticket, a.tipo_pago FROM cobro_engomado a INNER JOIN claves_facturacion b ON a.plaza = b.plaza AND a.cve = b.ticket WHERE a.placa = '".$_POST['placa']."' AND a.cve = '".$_POST['folio']."' AND b.cve = '".$_POST['clavefacturacion']."'");
	if($row=mysql_fetch_array($res)){
...
if($_POST['ajax']=='prefactura'){
	...
	<tr><td valign="top">
		...
		<tr><th align="left">Domicilio:</th><td colspan=3>'.($datos['calle'].' No. '.$datos['numexterior'].' '.$datos['numinterior']).'</td></tr>
		<tr><th align="left">Colonia:</th><td colspan=3>'.($datos['colonia']).'</td></tr>
		<!--<tr><th align="left">Localidad:</th><td colspan=3>'.($datos['localidad']).'</td></tr>-->
		<tr><th align="left">Municipio:</th><td colspan=3>'.($datos['municipio']).'</td></tr>
		<tr><th align="left">Estado:</th><td colspan=3>'.($datos['estado']).'</td></tr>
		<tr><th align="left">C.P.:</th><td>'.($datos['codigopostal']).'</td><th>PAIS:</th><td>MEXICO</td></tr>
	...
	M&eacute;todo pago: '.$array_tipo_pago[$tipo_pago].'<br>
	...
	<div class="row">
		...
		<div class="col-sm-6">
			...
			<div class="form-group">
				...
				<div class="col-sm-7">
					<input type="text" class="required mayusculas form-control input-sm" id="rfc" name="rfc" maxlength="13" onKeyUp="this.value=this.value.toUpperCase();" value="<?php echo $row['rfc'];?>" <?php echo $readOnly; ?>>
