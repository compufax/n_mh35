...
if($_POST['cmd']==110){
	...
	$Plaza=mysql_fetch_array($res);
	$filtro="";
	$select= " SELECT a.*, b.concepto FROM notascredito as a INNER JOIN notascreditomov b ON a.plaza = b.plaza AND a.cve = b.cvefact WHERE a.plaza='".$_POST['plazausuario']."'";
	...
	if(mysql_num_rows($rsabonos)>0) {
		...
		header("Content-Disposition: filename=NotasCredito.xls");
		header("Pragma: no-cache");
		header("Expires: 0");
		echo '<h1>Notas de Credito '.$Plaza['numero'].' '.$Plaza['nombre'].' del dia '.$_POST['fecha'].'</h1>';
		$res=mysql_query("SELECT CONCAT(serie,' ',folio) as folio FROM notascredito WHERE plaza='".$_POST['plazausuario']."' AND fecha>='".$_POST['fecha_ini']."' AND fecha<='".$_POST['fecha_fin']."' ORDER BY fecha,hora LIMIT 1");
		$row = mysql_fetch_array($res);
		echo '<h1>Folio Inicial: '.$row['folio'].'</h1>';
		$res=mysql_query("SELECT CONCAT(serie,' ',folio) as folio FROM notascredito WHERE plaza='".$_POST['plazausuario']."' AND fecha>='".$_POST['fecha_ini']."' AND fecha<='".$_POST['fecha_fin']."' ORDER BY fecha DESC,hora DESC LIMIT 1");
		$row = mysql_fetch_array($res);
		echo '<h1>Folio Final: '.$row['folio'].'</h1>';
		...
		while ($Abono=mysql_fetch_array($rsabonos)){	
			...
			echo '<td>'.htmlentities($Abono['concepto']).'</td>';
...
if($_POST['cmd']==101){
	generaFacturaPdf($_POST['plazausuario'],$_POST['reg'],1,2);
	exit();
}
...
if($_POST['cmd']==16){
	...
	if($zip->open("cfdi/zipcfdis".$fecha.".zip",ZipArchive::CREATE)){
		foreach($_POST['checksf'] as $cvefact){
			$res = mysql_query("SELECT * FROM notacredito WHERE plaza='".$_POST['plazausuario']."' AND cve='".$cvefact."'");
			$row = mysql_fetch_array($res);
			if($row['estatus']=='C')
				$zip->addFile("cfdi/comprobantes/ncc_".$_POST['plazausuario']."_".$cvefact.".pdf","NotaCredito_".$_POST['plazausuario']."_".$cvefact.".pdf");
			else
				$zip->addFile("cfdi/comprobantes/nc_".$_POST['plazausuario']."_".$cvefact.".pdf","NotaCredito_".$_POST['plazausuario']."_".$cvefact.".pdf");
			$zip->addFile("cfdi/comprobantes/cfdinc_".$_POST['plazausuario']."_".$cvefact.".xml","NotaCredito_".$_POST['plazausuario']."_".$cvefact.".xml");
		}
		$zip->close(); 
...
if($_POST['ajax']==1){
	$filtro="";
	$select= " SELECT a.* FROM notascredito as a LEFT JOIN facturas c ON c.plaza = a.plaza AND c.cve = a.factura
	LEFT JOIN cobro_engomado as b ON a.plaza = b.plaza AND a.cve = b.notacredito WHERE a.plaza='".$_POST['plazausuario']."'";
	if($_POST['folio'] != ""){
		$select .= " AND a.folio='".$_POST['folio']."' GROUP BY a.cve";
	}
	elseif($_POST['placa'] != '')
	{
		$select .= " AND b.placa = '".$_POST['placa']."' GROUP BY a.cve";
	}
	else{
		$select .= " AND a.fecha>='".$_POST['fecha_ini']."' AND a.fecha<='".$_POST['fecha_fin']."' ";
	...
	if(mysql_num_rows($rsabonos)>0) {
		...
		<th>Ticket</th><th>Fecha Ticket</th><th>Placa Ticket</th><th>Usuario<br>';
		echo '<select name="usuario" onchange="document.forma.usu.value=this.value;buscarRegistros();"><option value="all">---Todos---</option>';
		$res1=mysql_query("SELECT a.usuario FROM notascredito as a WHERE plaza='".$_POST['plazausuario']."' GROUP BY a.usuario ORDER BY a.usuario");
		...
		while ($Abono=mysql_fetch_array($rsabonos)){	
			...
			if($Abono['estatus']=='C'){
				...
				if(file_exists('cfdi/comprobantes/ncc_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf')){
					echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'cfdi/comprobantes/ncc_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf\',\'_blank\',\'0\',\''.$Abono['cve'].'\');"><img src="images/b_print.png" border="0" title="Imprimir '.$Abono['folio'].'"></a>';
					if($_POST['cveusuario']==1){
						echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'notacredito.php\',\'_blank\',\'103\',\'cfdi/comprobantes/ncc_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf\');"><img src="images/basura.gif" border="0" title="Borrar PDF '.$Abono['folio'].'"></a>';
					}
				}
				else{
					echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'notacredito.php\',\'_blank\',\'101\',\''.$Abono['cve'].'\');"><img src="images/b_print.png" border="0" title="Imprimir '.$Abono['folio'].'"></a>';
				}
				echo '</td>';
				if(nivelUsuario()>0){
			...
			elseif($Abono['respuesta1']==""){
				echo '<td align="center" width="40" nowrap>';
				echo '<input type="button" style=" font-size:25px;" value="TIMBRAR" onClick="if(confirm(\'Esta seguro que desea timbrar?\')){$(\'#panel\').show();atcr(\'notacredito.php\',\'\',\'5\',\''.$Abono['cve'].'\');}"><br>';
				//echo '<a href="#" onClick="if(confirm(\'Esta seguro que desea timbrar?\')){$(\'#panel\').show();atcr(\'notacredito.php\',\'\',\'5\',\''.$Abono['cve'].'\');}"><img src="images/validosi.gif" border="0" title="Timbrar '.$Abono['folio'].'"></a>';
				//if(nivelUsuario()>2){
				if($_POST['cveusuario'] == 1 || $_SESSION['TipoUsuario']){
					echo '&nbsp;&nbsp;<a href="#" onClick="if(confirm(\'Esta seguro de cancelar?\')){$(\'#panel\').show();atcr(\'notacredito.php\',\'\',\'3\',\''.$Abono['cve'].'\');}"><img src="images/validono.gif" border="0" title="Cancelar '.$Abono['folio'].'"></a>';
				}
				echo '</td>';
				if(nivelUsuario()>0){
					echo '<td>&nbsp;</td>';
				...
				echo '<td align="center" width="40" nowrap>';
				//<a href="#" onClick="atcr(\'cfdi/comprobantes/cfdinc_'.$_POST['cveempresa'].'_'.$Abono['cve'].'.pdf\',\'_blank\',\'\',\'\');"><img src="images/b_print.png" border="0" title="Imprimir '.$Abono['folio'].'"></a>';
				if(file_exists('cfdi/comprobantes/nc_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf')){
					echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'cfdi/comprobantes/nc_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf\',\'_blank\',\'0\',\''.$Abono['cve'].'\');"><img src="images/b_print.png" border="0" title="Imprimir '.$Abono['folio'].'"></a>';
					if($_POST['cveusuario']==1){
						echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'notacredito.php\',\'_blank\',\'103\',\'cfdi/comprobantes/nc_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.pdf\');"><img src="images/basura.gif" border="0" title="Borrar PDF '.$Abono['folio'].'"></a>';
					}
				}
				else{
					echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'notacredito.php\',\'_blank\',\'101\',\''.$Abono['cve'].'\');"><img src="images/b_print.png" border="0" title="Imprimir '.$Abono['folio'].'"></a>';
				...
				//if($_POST['cveusuario']==1)
					echo '&nbsp;&nbsp;<a href="#" onClick="atcr(\'cfdi/comprobantes/cfdinc_'.$_POST['plazausuario'].'_'.$Abono['cve'].'.xml\',\'_blank\',\'0\',\''.$Abono['cve'].'\');"><img src="images/b_print.png" border="0" title="Imprimir XML '.$Abono['folio'].'"></a>';
				echo '</td>';
				if(nivelUsuario()>0){
					echo '<td><input type="checkbox" class="checks" name="checksf[]" value="'.$Abono['cve'].'"></td>';
				}
			...
			$array_tickets=array();
			$res2=mysql_query("SELECT * FROM cobro_engomado WHERE plaza='".$Abono['plaza']."' AND notacredito='".$Abono['cve']."' AND estatus!='C'");
			...
			if(count($array_tickets) > 0){
				...
				echo '</td><td align="center">';
				foreach($array_tickets as $ticket=>$datosticket){
					$res3=mysql_query("SELECT a.cve FROM notascredito a INNER JOIN cobro_engomado b ON a.plaza=b.plaza AND a.cve=b.notacredito WHERE a.plaza='".$Abono['plaza']."' AND a.cve!='".$Abono['cve']."' AND b.placa = '".$datosticket['placa']."' AND ABS(DATEDIFF('".$Abono['fecha']."',fecha))<=60");
...
if($_POST['ajax']==3){
	...
	if($row=mysql_fetch_array($res)){
		if($row['estatus']=='C'){
			$regresar = array('error'=>1,'mensaje'=>'La factura esta cancelada');
		}
		elseif($row['estatus']=='D'){
			$regresar = array('error'=>1,'mensaje'=>'La factura ya esta devuelta');
		}
	}
	else{
		$regresar = array('error'=>1,'mensaje'=>'No se encontro el folio de factura');
	}
...
if($_POST['ajax']==16){
	mysql_query("UPDATE notascredito SET fecha='".$_POST['fecha']."' WHERE plaza='".$_POST['plazausuario']."' AND cve='".$_POST['folio']."'");
	exit();
}
...
if($_POST['cmd']==6){
	...
	foreach($_POST['checksf'] as $cvefact){
		$res = mysql_query("SELECT * FROM notascredito WHERE plaza='".$_POST['plazausuario']."' AND cve='".$cvefact."'");
		$row = mysql_fetch_array($res);
		...
		$mail->Subject = "Nota de Credito ".$row['serie']." ".$row['folio'];
		$mail->Body = "Nota de Credito ".$row['serie']." ".$row['folio'];
		...
		if($row['estatus']=='C')
			$mail->AddAttachment("cfdi/comprobantes/ncc_".$_POST['plazausuario']."_".$cvefact.".pdf", "Nota de Credito ".$row['serie']." ".$row['folio'].".pdf");
		else
			$mail->AddAttachment("cfdi/comprobantes/nc_".$_POST['plazausuario']."_".$cvefact.".pdf", "Nota de Credito ".$row['serie']." ".$row['folio'].".pdf");
		$mail->AddAttachment("cfdi/comprobantes/cfdinc_".$_POST['plazausuario']."_".$cvefact.".xml", "Nota de Credito ".$row['serie']." ".$row['folio'].".xml");
		$mail->Send();
		if($rowempresa['email']!=""){
			$mail = new PHPMailer();
			$mail->Host = "localhost";
...
if($_POST['cmd']==5){
	...
	$res = mysql_query("SELECT * FROM notascredito WHERE plaza='".$_POST['plazausuario']."' AND cve='".$_POST['reg']."'");
	$row = mysql_fetch_array($res);
	$fserie=$row['serie'];
	$ffolio=$row['folio'];
	...
	$res2 = mysql_query("SELECT * FROM notascreditomov WHERE plaza='".$_POST['plazausuario']."' AND cvefact='".$cvefact."'");
		...
		if ($oSoapClient->fault) {
			...
			if ($err){
				...
				if($respuesta['resultado']){
					...
					mysql_query("UPDATE facturas SET estatus='D' WHERE plaza='".$_POST['plazausuario']."' AND cve='".$row['factura']."'");
...
if($_POST['cmd']==3){
	...
	if($row['estatus']!='C'){
		...
		if($row['respuesta1']!=""){
			...
			if($err!="")
				...
				if ($oSoapClient->fault) {
					...
					if ($err){
						...
						if($respuesta['resultado']){
							mysql_query("UPDATE notascredito SET estatus='C',usucan='".$_POST['cveusuario']."',fechacan='".fechaLocal()." ".horaLocal()."',respuesta2='".$respuesta['mensaje']."' WHERE plaza='".$_POST['plazausuario']."' AND cve='".$_POST['reg']."'");
							generaFacturaPdf($_POST['plazausuario'],$cvefact,0,2);
							mysql_query("UPDATE facturas SET estatus='A' WHERE plaza='".$_POST['plazausuario']."' AND cve='".$row['factura']."'");
							mysql_query("UPDATE cobro_engomado SET notacredito=0 WHERE plaza='".$_POST['plazausuario']."' AND notacredito='".$_POST['reg']."'");
							if($emailenvio!=""){
								$mail = new PHPMailer();
								$mail->Host = "localhost";
								$mail->From = "verificentros@verificentros.net";
								$mail->FromName = "Verificentros Plaza ".$array_plaza[$_POST['plazausuario']];
								$mail->Subject = "Cancelacion Nota de Credito ".$fserie." ".$ffolio;
								$mail->Body = "Cancelacion Nota de Credito ".$fserie." ".$ffolio;
								//$mail->AddAddress(trim($emailenvio));
								$correos = explode(",",trim($emailenvio));
								foreach($correos as $correo)
									$mail->AddAddress(trim($correo));
								...
								$mail->AddAttachment("cfdi/comprobantes/ncc_".$_POST['plazausuario']."_".$cvefact.".pdf", "Nota de Credito ".$fserie." ".$ffolio.".pdf");
								$mail->AddAttachment("cfdi/comprobantes/cfdinc_".$_POST['plazausuario']."_".$cvefact.".xml", "Nota de Credito ".$fserie." ".$ffolio.".xml");
								$mail->Send();
							}	
							if($rowempresa['email']!=""){
			...
			mysql_query("UPDATE notascredito SET estatus='C',usucan='".$_POST['cveusuario']."',fechacan='".fechaLocal()." ".horaLocal()."' WHERE plaza='".$_POST['plazausuario']."' AND cve='".$_POST['reg']."'");
			generaFacturaPdf($_POST['plazausuario'],$cvefact,0,2);
			mysql_query("UPDATE cobro_engomado SET notacredito=0 WHERE plaza='".$_POST['plazausuario']."' AND notacredito='".$_POST['reg']."'");
			mysql_query("UPDATE facturas SET estatus='A' WHERE plaza='".$_POST['plazausuario']."' AND cve='".$row['factura']."'");
		}
...
if($_POST['cmd']==2){
	...
	$resFactura = mysql_query("SELECT cve,estatus FROM facturas WHERE plaza='".$_POST['plazausuario']."' AND serie='".$_POST['seriefactura']."' AND folio='".$_POST['foliofactura']."'");
	$Factura=mysql_fetch_array($resFactura);
	$insert = "INSERT notascredito SET plaza='".$_POST['plazausuario']."',serie='".$row['serie']."',folio='".$row['folio_inicial']."',fecha='".$_POST['fecha']."',fecha_creacion='".fechaLocal()."',hora='".horaLocal()."',obs='".$_POST['obs']."',
	...
	factura='".$Factura['cve']."',engomado='".$_POST['engomado']."',banco='".$ClienteCuenta['banco']."',cuenta_cheque='".$ClienteCuenta['cuenta']."',tiene_descuento='".$_POST['tiene_descuento']."'".$datossucursal;
	...
	foreach($_POST['cant'] as $k=>$v){
		if($v>0){
			...
			mysql_query("INSERT notascreditomov SET plaza='".$_POST['plazausuario']."',cvefact='$cvefact',cantidad='".$v."',concepto='".$_POST['concepto'][$k]."',
			precio='".$_POST['precio'][$k]."',descuento='".$_POST['descuento'][$k]."',importe='".$_POST['importe'][$k]."',iva='".$_POST['ivap'][$k]."',importe_iva='$importe_iva',unidad='".$_POST['unidad'][$k]."',
	...
	mysql_query("UPDATE notascredito SET subtotal='".$_POST['subtotal']."',iva='".$_POST['iva']."',total='".$_POST['total']."',
	...
	mysql_query("UPDATE cobro_engomado SET notacredito = '$cvefact' WHERE plaza='".$_POST['plazausuario']."' AND factura='".$Factura['cve']."' AND factura>0");
	mysql_query("UPDATE facturas SET estatus = 'D' WHERE plaza='".$_POST['plazausuario']."' AND cve='".$Factura['cve']."'");
	mysql_query("UPDATE venta_engomado_factura SET notacredito = '$cvefact' WHERE plaza='".$_POST['plazausuario']."' AND factura='".$Factura['cve']."'");
...
top($_SESSION);
	...
	if($_POST['cmd']==1){
		...
		if(nivelUsuario()>1){
			...
			if(document.forma.cliente.value==\'0\'){
				...
				alert(\'La nota de credito debe de ser mayor a cero\');
				$(\'#panel\').hide();
			}
			else if(validarFactura()){
				atcr(\'notacredito.php\',\'\',2,\'0\');
			}
			"><img src="images/guardar.gif" border="0">&nbsp;Guardar</a></td><td>&nbsp;</td>';
		}
		echo '<td><a href="#" onclick="$(\'#panel\').show();atcr(\'notacredito.php\',\'\',0,\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;&nbsp;Regresar</a></td>';
		echo '</tr></table>';
		echo '<br>';
		echo '<table>';
		...
		echo '<tr><th align="left">Factura</th><td><select name="seriefactura" id="seriefactura"><option value="">Seleccione</option>';
		$res1=mysql_query("SELECT serie FROM facturas WHERE plaza='".$_POST['plazausuario']."' GROUP BY serie ORDER BY serie");
		while($row1=mysql_fetch_array($res1)) echo '<option value="'.$row1[0].'">'.$row1[0].'</option>';
		echo '</select>&nbsp;&nbsp;<input type="text" name="foliofactura" id="foliofactura"  size="15" class="textField" value=""></td></tr>';
		echo '<tr style="display:none;"><td>Nota de Credito de Engomado</td><td><input type="checkbox" name="engomado" id="engomado" value="1" onClick="facturar_engomado();"></td></tr>';
		echo '<tr style="display:none;" id="id_descuento"><td>Tiene 10% Descuento</td><td><input type="checkbox" name="tiene_descuento" id="tiene_descuento" value="1" onClick="aplicar_descuento();sumarproductos();"></td></tr>';
		...
		echo '<th>Cantidad</th>';
		echo '<th id="encabezadoengomado" style="display:none;">Engomado</th>';
		echo '<th>Unidad</th>';
		...
		if($i==0){
			...
			echo '<td align="center" style="display:none;"><select name="engomado_id['.$i.']" id="engomado_id'.$i.'" onChange="seleccionengomado('.$i.')">';
			...
			foreach($array_engomado as $k=>$v){
				echo '<option value="'.$k.'" nombre="VERIFICACION VEHICULAR" precio="'.$array_engomadoprecio[$k].'">'.$v.'</option>';
				$cadenaengomado.='<option value="'.$k.'" nombre="VERIFICACION VEHICULAR" precio="'.$array_engomadoprecio[$k].'">'.$v.'</option>';
			}
			echo '</select></td>';
			...
			echo '<td align="center"><input type="text" class="textField" size="10" name="precio['.$i.']" id="precio'.$i.'" value=""  onKeyUp="sumarproductos()">
			<input type="hidden" class="descuentos" name="descuento['.$i.']" id="descuento'.$i.'" value=""></td>';
		...
		echo '<script>
			function facturar_engomado(){
				if(document.forma.engomado.checked==false){
					...
					colspan="4";
					$("#id_descuento").hide();
					if($("#tiene_descuento").is(":checked")){
						$("#tiene_descuento").removeAttr("checked");
						aplicar_descuento();
					}
				}
				else{
			...
			function aplicar_descuento(){
				if($("#tiene_descuento").is(":checked")){
					$(".descuentos").each(function(){
						ren = $(this).parents("tr:first").attr("ren");
						descuento = $("#precio"+ren).val()*0.10;
						$(this).val(descuento.toFixed(2));
						nuevoprecio = ($("#precio"+ren).val()/1)-($(this).val()/1);
						$("#precio"+ren).val(nuevoprecio.toFixed(2));
					});
				}
				else{
					$(".descuentos").each(function(){
			...
			var ac_config = {
				source: "notacredito.php?ajax=2&plazausuario='.$_POST['plazausuario'].'",
			...
			function agregarproducto(){
				...
				<td align="center"><input type="text" class="\'+clase+\'" size="10" name="precio[\'+num+\']" id="precio\'+num+\'" value=""  onKeyUp="sumarproductos()"\'+bloqueo+\'>\
				<input type="hidden" class="descuentos" name="descuento[\'+num+\']" id="descuento\'+num+\'" value=""></td>\
			...
			function seleccionengomado(linea){
				...
				if($("#tiene_descuento").is(":checked")){
					descuento = $("#precio"+linea).val()*0.10;
					$("#descuento"+linea).val(descuento.toFixed(2));
					nuevoprecio = ($("#precio"+linea).val()/1)-($("#descuento"+linea).val()/1);
					$("#precio"+linea).val(nuevoprecio.toFixed(2));
				}
				sumarproductos();
			}
			...
			function validarFactura(){
				var regresar = true;
				if((document.forma.foliofactura.value/1)>0){
					$.ajax({
		                url: "notacredito.php",
		                type: "POST",
		                async: false,
		                ...
		                data: {
		                	ajax: 3,
		                	serie: document.forma.seriefactura.value,
		                	folio: document.forma.foliofactura.value,
	...
	if ($_POST['cmd']<1) {
		...
		echo '<tr>
				...
				<td><a href="#" onClick="atcr(\'notacredito.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0">&nbsp;Nuevo</a></td><td>&nbsp;</td>';
		if(nivelUsuario()>0){
			echo '<td><a href="#" onClick="atcr(\'notacredito.php\',\'\',\'6\',\'0\');"><img src="images/nuevo.gif" border="0">&nbsp;Reenviar Archivos</a></td><td>&nbsp;</td>';
			echo '<td><a href="#" onClick="atcr(\'notacredito.php\',\'_blank\',\'16\',\'0\');"><img src="images/zip_grande.png" border="0" width="15px" height="15px" title="Descargar">&nbsp;Descargar Archivos</a></td><td>&nbsp;</td>';
			echo '<td><a href="#" onClick="atcr(\'notacredito.php\',\'_blank\',\'110\',\'0\');"><img src="images/b_print.png" border="0" width="15px" height="15px" title="Descargar">&nbsp;Descargar Excel</a></td><td>&nbsp;</td>';
		}
		echo '</tr>';
		echo '</table>';
		echo '<table>';
		...
		echo '<tr><td align="left">Placa</td><td><input type="text" name="placa" id="placa"  size="15" class="textField" value=""></td></tr>';
...
<Script language="javascript">
	...
	function buscarRegistros()
	{
		...
		if (objeto.readyState != 0) {
			...
			objeto.open("POST","notacredito.php",true);
			objeto.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
			objeto.send("ajax=1&folio="+document.getElementById("folio").value+"&placa="+document.getElementById("placa").value+"&estatus="+document.getElementById("estatus").value+"&cliente="+document.getElementById("cliente").value+"&fecha_ini="+document.getElementById("fecha_ini").value+"&fecha_fin="+document.getElementById("fecha_fin").value+"&usu="+document.getElementById("usu").value+"&numeroPagina="+document.getElementById("numeroPagina").value+"&plazausuario="+document.getElementById("plazausuario").value+"&cveusuario="+document.getElementById("cveusuario").value+"&cvemenu="+document.getElementById("cvemenu").value);
