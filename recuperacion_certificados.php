...
if($_POST['cmd']==101){
	...
	$res=mysql_query("SELECT * FROM recuperacion_certificado WHERE plaza='".$_POST['plazausuario']."' AND cve='".$_POST['reg']."'");
	$row=mysql_fetch_array($res);
	$texto=chr(27)."@";
	$texto.='|';
	...
	$texto.='|';
	$texto.=chr(27).'!'.chr(8)." RECUPERACION DE CERTIFICADO";
	$texto.='|';
	...
	$texto.='|';
	$texto.=chr(27).'!'.chr(8)." RECUPERACION: ".number_format($row['recuperacion'],2);
	$texto.='|';
	$texto.='|';
...
if($_POST['cmd']==100) {
	...
	echo '<h1>Listado de Recuperacion de Certificados de la Plaza '.$array_plaza[$_POST['plazausuario']].' del '.$_POST['fecha_ini'].' al '.$_POST['fecha_fin'].'</h1>';
		//Listado de plazas
		$select= " SELECT * FROM recuperacion_certificado WHERE plaza='".$_POST['plazausuario']."'";
		if ($_POST['certificado']!="") { $select.=" AND certificado='".$_POST['certificado']."' "; }
		...
		if(mysql_num_rows($res)>0) 
		{
			...
			<th>Tipo de Certificado Venta</th><th>Monto Venta</th><th>Tipo Certificado Entregado</th><th>Monto</th><th>Recuperacion</th>
			<th>Usuario</th>';
			echo '</tr>';
			$t=0;
			while($row=mysql_fetch_array($res)) {
				...
				if($row['estatus']=='C'){
					...
					if(nivelUsuario()>1)
						echo '<a href="#" onClick="if(confirm(\'Esta seguro de cancelar el registro\')) atcr(\'recuperacion_certificados.php\',\'\',\'3\','.$row['cve'].')"><img src="images/validono.gif" border="0" title="Cancelar '.$row['cve'].'"></a>';
				}	
				echo '</td>';
...
if($_POST['ajax']==2){
	$res = mysql_query("SELECT a.cve as ticket,a.placa,a.engomado,a.monto as monto_venta, b.cve as folioentrega, b.engomado as engomadoentrega, b.certificado, b.entregado FROM cobro_engomado a LEFT JOIN certificados b ON a.plaza = b.plaza AND a.cve = b.ticket AND b.estatus!='C' WHERE a.plaza='".$_POST['plazausuario']."' AND a.cve = '".$_POST['ticket']."' AND a.estatus!='C' AND a.factura=0 ORDER BY a.cve DESC LIMIT 1");
	if($row=mysql_fetch_array($res)){
		if($row['monto_venta']>0){
			$res1=mysql_query("SELECT cve FROM recuperacion_certificado WHERE plaza='".$_POST['plazausuario']."' AND ticket='".$row['ticket']."' AND estatus!='C'");
...
if($_POST['ajax']==2.1){
	$res = mysql_query("SELECT a.cve as ticket,a.engomado,a.monto as monto_venta, b.cve as folioentrega, b.engomado as engomadoentrega, b.certificado FROM cobro_engomado a INNER JOIN certificados b ON a.plaza = b.plaza AND a.cve = b.ticket AND b.estatus!='C' WHERE a.plaza='".$_POST['plazausuario']."' AND a.placa = '".$_POST['placa']."' AND a.estatus!='C' AND a.factura=0 ORDER BY a.cve DESC LIMIT 1");
	if($row=mysql_fetch_array($res)){
		$res1=mysql_query("SELECT cve FROM recuperacion_certificado WHERE plaza='".$_POST['plazausuario']."' AND ticket='".$row['ticket']."' AND estatus!='C'");
		if(mysql_num_rows($res1)==0){
...
if($_POST['cmd']==3){
	mysql_query("UPDATE recuperacion_certificado SET estatus='C',usucan='".$_POST['cveusuario']."',fechacan='".fechaLocal()." ".horaLocal()."' WHERE plaza='".$_POST['plazausuario']."' AND cve='".$_POST['reg']."'");
	$_POST['cmd']=0;
}
...
if ($_POST['cmd']==2) {
	...
	$res = mysql_query("SELECT cve FROM recuperacion_certificado WHERE plaza='".$_POST['plazausuario']."' AND ticket='".$_POST['ticket']."' AND estatus!='C'");
	if(mysql_num_rows($res)==0){
			...
			if($row=mysql_fetch_array($res)){
				$insert = " INSERT recuperacion_certificado 
								...
								engomadoentrega='".$_POST['engomadoentrega']."',montoentrega='".$_POST['montoentrega']."'";
				mysql_query($insert);
				$cverecuperacion=mysql_insert_id();
			}
	}
	$_POST['cmd']=0;
}
...
/*** EDICION  **************************************************/
	...
	if ($_POST['cmd']==1) {
		...
		echo '
			...
			echo '<td><a href="#" onClick="$(\'#panel\').show();atcr(\'recuperacion_certificados.php\',\'\',\'0\',\'0\');"><img src="images/flecha-izquierda.gif" border="0">&nbsp;Volver</a></td><td>&nbsp;</td>
			</tr>';
		echo '</table>';
		echo '<br>';
		...
		//Formulario 
		echo '<table>';
		echo '<tr><td class="tableEnc">RECUPERACION DE CERTIFICADO</td></tr>';
		echo '</table>';
		echo '<input type="hidden" name="ticket" id="ticket" value="">';
		echo '<table>';
		echo '<tr><th align="left">Ticket</th><td><input type="text" name="fticket" id="fticket" class="textField" style="font-size:20px" size="10" value="" onKeyUp="if(event.keyCode==13){ traeTicket();} else{ this.value = this.value.toUpperCase();}">&nbsp;&nbsp;<font color="RED">Dar enter para traer el engomado del ticket</font></td></tr>';
		echo '<tr><th align="left">Placa</th><td><input type="text" name="placa" id="placa" class="readOnly" style="font-size:20px" size="10" value="" onKeyUp="if(event.keyCode==13){ traeTicket();} else{ this.value = this.value.toUpperCase();}" readOnly></td></tr>';
		...
		echo '<tr><th align="left">Recuperacion</th><td><input type="text" name="recuperacion" id="recuperacion" class="textField" style="font-size:20px" size="10" value=""></td></tr>';
		echo '<tr><th align="left">Motivo de Recuperacion</th><td><textarea name="motivo" id="motivo" class="textField" rows="3" cols="50"></textarea></td></tr>';
		echo '</table>';
		...
		echo '<script>
				function validar(){
					...
					else if((document.forma.recuperacion.value/1)==0){
						$("#panel").hide();
						alert("La recuperacion no puede ser cero");
					}
					else if($.trim(document.forma.motivo.value)==""){
						$("#panel").hide();
						alert("Necesita ingresar el motivo de la recuperacion");
					}
					else{
						atcr("recuperacion_certificados.php","",2,0);
					}
				}
				
				
				function traeTicket(){
					if(document.forma.fticket.value==""){
						...
						document.forma.recuperacion.value="";
					}
					else{
						objeto=crearObjeto();
						if (objeto.readyState != 0) {
							...
							objeto.open("POST","recuperacion_certificados.php",true);
							objeto.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
							objeto.send("ajax=2&ticket="+document.getElementById("fticket").value+"&plazausuario="+document.getElementById("plazausuario").value);
							objeto.onreadystatechange = function()
							{
								if (objeto.readyState==4)
								{
									...
									if(datos[0]=="-1"){
										...
										alert("Ya se le hizo la recuperacion al ticket");
										document.forma.placa.value="";
										document.forma.engomado.value="";
										document.forma.nomengomado.value="";
										document.forma.ticket.value="";
										...
										alert("No se pueden recuperar intentos");
										document.forma.placa.value="";
										document.forma.engomado.value="";
										document.forma.nomengomado.value="";
										document.forma.ticket.value="";
										...
										document.forma.placa.value=datos[1];
...
/*** PAGINA PRINCIPAL **************************************************/
	...
	if ($_POST['cmd']<1) {
		...
		if($PlazaLocal!=1)
			echo '<td><a href="#" onClick="atcr(\'recuperacion_certificados.php\',\'\',\'1\',\'0\');"><img src="images/nuevo.gif" border="0"></a>&nbsp;Nuevo</td><td>&nbsp;</td>';
		echo '<td><a href="#" onClick="atcr(\'recuperacion_certificados.php\',\'_blank\',\'100\',\'0\');"><img src="images/b_print.png" border="0"></a>&nbsp;Imprimir</td><td>&nbsp;</td>
			 </tr>';
		echo '</table>';
		echo '<table>';
		...
		$res=mysql_query("SELECT b.cve,b.usuario FROM recuperacion_certificado a INNER JOIN usuarios b ON a.usuario = b.cve WHERE a.plaza='".$_POST['plazausuario']."' GROUP BY b.cve ORDER BY b.usuario");
		while($row=mysql_fetch_array($res)){
...
if($cverecuperacion>0){
		echo '<script>atcr(\'recuperacion_certificados.php\',\'_blank\',\'101\','.$cverecuperacion.');</script>';
	}
?>
