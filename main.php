...
$array_modulos=array(3=>"Catalogos Generales",1=>"Catalogos Locales",20=>"Gastos",2=>"Ventas",23=>'Cometra',4=>"Personal",6=>"Chequeras",9=>"Compras",
7=>"Facturacion",8=>"Auditoria",12=>"Direcci&oacute;n",99=>"Administracion");
...
function top($SESSION,$enter=0) {
	...
	<table width="100%" height="50" border="0" cellpadding="0" cellspacing="0">
	  ...
	  <tr>
	   ...
	   <td background="images/bannertop-bg.png"><span class="whiteText17">GVERIFICENTROS '.$textoPlaza.'</span></td>
...
function menuppal2($SESSION) {
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="3">
		<tr><td height="20" bgcolor="#CC9933"><span class="style1">Menu '.$array_plaza[$_POST['plazausuario']].'</span></td></tr>
		...
		<tr><td><a href="#" onClick="document.forma.cvemenu.value=1;atcr(\'consulta_placa.php\',\'\',\'\',\'\')">-Consulta de Placas</a></td></tr>
		<tr><td><a href="#" onClick="document.forma.cvemenu.value=1;atcr(\'consulta_certificado.php\',\'\',\'\',\'\')">-Consulta de Certificado</a></td></tr>';
		...
		foreach($array_modulos as $k=>$v){ 
			...
			if($k!=19 || $_POST['cveusuario'] == 1 || $_POST['cveusuario'] == 420){
				...
				elseif($_POST['cveusuario']==2){
					$rs=mysql_query("SELECT * FROM menu WHERE modulo='$k' AND cve!=179 AND cve!='40' AND cve!='169' AND cve!='2' AND cve!='107' AND cve!='124' AND cve!='129' AND cve!=38 AND cve!=2 AND cve!=130  AND cve!=57 ORDER BY orden");
				}
				elseif($SESSION['TipoUsuario']==1){
					$rs=mysql_query("SELECT * FROM menu WHERE modulo='$k' AND cve!=179 AND cve!='40' AND cve!='169' AND cve!='2' AND cve!='107' AND cve!='124' AND cve!='129' AND cve!=38 AND cve!=2 AND cve!=130  AND cve!=57 ORDER BY orden");
				}
				else{
					$rs=mysql_query("SELECT a.* FROM menu as a INNER JOIN usuario_accesos as b ON (b.menu=a.cve AND b.usuario='".$_POST['cveusuario']."' AND b.plaza='".$_POST['plazausuario']."' AND b.acceso>0) WHERE a.modulo='$k' AND a.cve!=179 AND a.cve!='169' AND a.cve!='107' AND a.cve!='2' AND a.cve!='124' AND a.cve!=38 AND a.cve!=130 AND a.cve!=2 AND a.cve!=57 AND a.cve!=129 ORDER BY a.orden");
				}
		...
		foreach($array_modulos as $k=>$v){ 
			if($k==99 || $k==3 || $k==4 || $k==5 || $k==6 || $k==8 || $k==10 || $k==11 || $k==12 || $k==13 || $k==15 || $k==21 || $k==22 || ($k==19 && ($_POST['cveusuario'] == 1 || $_POST['cveusuario'] == 420))){
				if($_POST['cveusuario']==1)
					...
					$rs=mysql_query("SELECT * FROM menu WHERE modulo='$k' AND cve!=7 AND cve!='143' AND cve!='2' AND cve!='107' AND cve!='124' AND cve!='125' AND cve!=40 AND cve!=38 AND cve!=2 And cve!=1 AND cve!=57 ORDER BY orden");
				if(mysql_num_rows($rs)>0){
