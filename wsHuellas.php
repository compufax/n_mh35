...
set_time_limit(0);
$base='enero_aaz';
$namespace = "http://verificentrosgp1.net/huellaservices";
...
function RegistraChecada($usuario, $password, $lector, $operador, $tipo = 0)
{
	...
	if($respuesta['mensaje']=='')
	{
		/*if($tipo==2 && date('H:i:s')>'16:00:00'){
			...
			$limite=date( "Y-m-d H:i:s" , strtotime ( "-10 minute" , strtotime(date("Y-m-d H:i:s")) ) );
			$res = mysql_query("SELECT  a.fechahora FROM checada_lector a inner join motivos_checada b on a.tipo = b.cve WHERE a.cvepersonal='$operador' AND DATE(a.fechahora)='".date("Y-m-d")."' ORDER BY fechahora DESC");
