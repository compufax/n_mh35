		...
		if($encontrado){
			...
			elseif($row['factura']>0 && $row['notacredito']==0){
				...
				if($row1['respuesta1']==""){
					...
					$resultado['html'] .=  '&nbsp;&nbsp;<a href="#" onClick="atcr(\'facturacion_web.php\',\'_blank\',\'200\',\''.$row1['plaza'].'|'.$row1['cve'].'|1\');"><img src="images/zip_grande.png" border="0" width="15px" height="15px" title="Descargar"></a>';
				...
				if($row['notacredito']>0){
					...
					if($row1['respuesta1']==""){
						...
						$resultado['html'] .=  '&nbsp;&nbsp;<a href="#" onClick="atcr(\'facturacion_web.php\',\'_blank\',\'200\',\''.$row1['plaza'].'|'.$row1['cve'].'|2\');"><img src="images/zip_grande.png" border="0" width="15px" height="15px" title="Descargar"></a>';
					}
					$resultado['html'] .=  '</td>
					<td align="left" style="padding: 10px!important">'.$row['nomplaza'].'</td>
...
if($_POST['cmd']==2){
	...
	if($row=mysql_fetch_array($res)){
		...
		if(count($array_detalles)>0){
			...
			$insert = "INSERT facturas SET plaza='".$plaza."',serie='".$row['serie']."',folio='".$row['folio_inicial']."',fecha='".fechaLocal()."',fecha_creacion='".fechaLocal()."',hora='".horaLocal()."',obs='".$_POST['obs']."',ip='$ip',
				...
				if ($oSoapClient->fault) {
					...
					if ($err){
						...
						if($respuesta['resultado']){
							...
							echo '<h1>Se genero el folio de factura '.$fserie." ".$ffolio.'</h1>';
						}
						else
							$strmsg=$respuesta['mensaje'];
						//print_r($respuesta);	
					...
					<div class="row">
						<div class="col-sm-6">
							<div class="input-group">
								<input type="text" class="form-control input-sm" id="codigofactura" name="codigofactura" placeholder="Buscar por código de facturación" onKeyUp="this.value=this.value.toUpperCase();">
...
if(date("H:i:s")>'21:00:00' || date('H:i:s')<'01:00:00' || date('Y-m-d') == '2015-10-04'){
	if(date('Y-m-d') == '2015-10-04')
		...
		echo '<script>$("#panel").show();
		alert("El horario de facturación es de 01:00 a 21:00 horas");
		</script>';
}
?>