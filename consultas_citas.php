...
/*** PAGINA PRINCIPAL **************************************************/
	...
	if ($_POST['cmd']<1) {
		...
		echo '<table>';
		echo '<tr style="display:none;"><td>Fecha Inicial</td><td><input type="text" name="fecha_ini" id="fecha_ini" class="textField" size="12" value="'.fechaLocal().'" >&nbsp;<a href="#" onClick="displayCalendar(document.forms[0].fecha_ini,\'yyyy-mm-dd\',this,true)"><img src="images/calendario.gif" border="0"></a></td></tr>';
		...
		echo '<tr style="display:none;"><td>Folio</td><td><input type="text" name="folio" id="folio" size="10" class="textField"></td></tr>';		
		echo '<tr><td>Placa</td><td><input type="text" name="placa" id="placa" size="10" class="textField placas"></td></tr>';		
		echo '<tr style="display:none;"><td>Tipo de Certificado</td><td><select name="engomado" id="engomado"><option value="all">Todos</option>';
		...
		echo '</select></td></tr>';
		echo '<tr style="display:none;"><td>Sucursal</td><td><select name="plaza" id="plaza"><option value="all">Todos</option>';
		foreach($array_plazas as $k=>$v){
			echo '<option value="'.$k.'">'.$v.'</option>';
		}
		echo '</select></td></tr>';
		echo '<tr style="display:none;"><td>Mostrar</td><td><select name="mostrar" id="mostrar"><option value="all" selected>Todos</option>';
		...
		<Script language="javascript">
			...
			function buscarRegistros()
			{
				...
				if (objeto.readyState != 0) {
					...
					objeto.open("POST","consultas_citas.php",true);
					objeto.setRequestHeader("Content-Type","application/x-www-form-urlencoded");
					objeto.send("ajax=1&fecha_ini="+document.getElementById("fecha_ini").value+"&fecha_fin="+document.getElementById("fecha_fin").value+"&folio="+document.getElementById("folio").value+"&placa="+document.getElementById("placa").value+"&plaza="+document.getElementById("plaza").value+"&engomado="+document.getElementById("engomado").value+"&mostrar="+document.getElementById("mostrar").value+"&numeroPagina="+document.getElementById("numeroPagina").value+"&cveusuario="+document.getElementById("cveusuario").value+"&plazausuario="+document.getElementById("plazausuario").value);
